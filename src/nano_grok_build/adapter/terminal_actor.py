...
class ProcessLeaseV1:
    ...
    def canonical_bytes(self) -> bytes:
        """Publish the exact lease identity without exposing owner tokens."""
        payload = {
            "schema_version": "terminal-actor-process-lease-v1",
            "identities": [
                {
                    "task_id": identity.task_id,
                    "request_dir": identity.request_dir,
                    ...
                    "term_grace_ms": identity.term_grace_ms,
                    "kill_confirmation_timeout_ms": (
                        identity.kill_confirmation_timeout_ms
                    ),
                }
    ...
    def canonical_sha256(self) -> str:
        """Bind the exact lease without publishing its private owner tokens."""
...
if [ "$cleanup_verified" != "true" ]; then
  ...
  meta_format="${meta_format}"',"stdout_truncated":true,"stderr_truncated":true'
  meta_format="${meta_format}"',"cleanup_attempted":true,"term_sent":%s'
  meta_format="${meta_format}"',"kill_sent":%s,"cleanup_verified":false'
  meta_format="${meta_format}"',"census_verified":false,"survivor_count":%s}'
  printf "${meta_format}\n" \
    "$return_code" "$timed_out" "$term_sent" "$kill_sent" \
  ...
  sync "$request_dir" 2>/dev/null || true
  exit 76
fi
drain_forced=false
...
class RemoteTerminalActor:
    ...
    def settlement_state_v1(self) -> dict[str, object]:
        """Expose only structural foreground/snapshot settlement facts.

        Background tasks are deliberately excluded: their exact identities are
        frozen separately by ``seal_process_lease_v1``.  Foreground and snapshot
        entries are removed only after their remote zero-census is proved (or
        after a fully observed no-process path), so an empty set is the actor's
        mechanical dispatch/quiescence boundary.
        """

        quiescent = not self._active and not self._active_snapshots
        return {
            "schema_version": "terminal-actor-settlement-state-v1",
            "actor_dispatch": "revoked" if quiescent else "unverified",
            "actor_quiescent": quiescent,
            "remote_census_verified": quiescent,
            "remote_survivor_count": 0 if quiescent else None,
        }
        ...
        try:
            ...
            try:
                ...
                (
                    ...
                    recovery_subtype,
                    recovery_failure,
                ) = await self._recover_settled_foreground(request_dir, request)
                        ...
                        replace(
                            ...
                            actor_receipt=self._foreground_receipt(
                                ...
                                phase=(
                                    ...
                                    if recovery_failure.cleanup_verified is not True
                                ...
                                recovery_subtype=recovery_subtype,
                                execution_may_have_started=True,
                                cleanup_verified=(recovery_failure.cleanup_verified),
                                census_verified=recovery_failure.census_verified,
                                effective_cutoff_monotonic_ns=phase_cutoff_ns,
                            ),
                        )
    ...
    async def _execute_foreground_legacy(
        ...
        try:
            ...
            try:
                ...
                recovered, _, _ = await self._recover_settled_foreground(
                    request_dir,
                    request,
                )
    ...
    async def _recover_settled_foreground(
        ...
        try:
            ...
            return None, self._foreground_recovery_subtype(error), error.failure
        except Exception as error:
            return None, self._foreground_recovery_subtype(error), None
