...
async def _complete_shielded(
    ...
    *,
    ...
    cancellation_reserve_sec = min(0.05, total_timeout_sec * 0.25)
...
async def run_stdio_bridge(
    ...
    *,
    ...
    try:
        ...
        return BridgeOutcome(request_count=expected_seq, stderr=stderr)
        ...
        try:
            ...
            if legacy:
                (
                    ...
                    cleanup_awaitable,
                    timeout_sec=cleanup_budget,
                )
            else:

                async def cleanup_and_stop() -> tuple[bool, bool]:
                    cleanup_result, stop_result = await asyncio.gather(
                        cleanup_awaitable,
                        _allow_publication_then_stop_child(
                            process,
                    ...
                    return (
                        ...
                        not isinstance(stop_result, BaseException),
                    )

                joint_completed, joint_value, cleanup_cancel = await _complete_shielded(
                    cleanup_and_stop(),
                    timeout_sec=remaining,
                )
                cleanup_completed = joint_completed
                if joint_completed:
                    cleanup_value, stop_completed = joint_value
        finally:
            if legacy:
                stop_budget = hard_remaining()
                stop_completed, _, stop_cancel = await _complete_shielded(
                    _stop_child(process),
                    timeout_sec=stop_budget,
                )
        ...
        cleanup_ok = (
            execution_completed
            and execution_value is True
            and cleanup_completed
            and cleanup_value is True
            and stop_completed
        )
        if not cleanup_ok:
            components_verified = {
                "execution": execution_completed and execution_value is True,
                "cleanup": cleanup_completed and cleanup_value is True,
                "stop": stop_completed,
            }
            failed_component = next(
                name for name, verified in components_verified.items() if not verified
            )
            raise BridgeError(
                ...
                cleanup_receipt={
                    ...
                    "census_scope": "remote_actor_owned_processes",
