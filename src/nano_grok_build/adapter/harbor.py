...
class NanoGrokBuildAgent(BaseAgent):
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        self._bridge_closure_receipt_v1: BridgeClosureReceiptV1 | None = None
        self._runtime_cleanup_facts_v1: RuntimeCleanupFactsV1 | None = None
        self._runtime_cleanup_receipt_v1: RuntimeCleanupReceiptV1 | None = None
    ...
    def _git_history_background_lifecycle_state(
        ...
        if type(lease) is not ProcessLeaseV1 or binding is None:
            raise BridgeError("git_history_lifecycle_invalid")
        try:
            self._validate_persisted_process_lease_v1(lease)
    ...
    def _validate_persisted_process_lease_v1(self, lease: ProcessLeaseV1) -> str:
        """Read back the exact durable lease without materializing a missing one."""

        if type(lease) is not ProcessLeaseV1:
            raise BridgeError("terminal_actor_process_lease_invalid")
        payload = lease.canonical_bytes
        path = self._artifact_source_dir() / "runtime-process-lease-v1.json"
        try:
            persisted = _read_artifact_regular(
                path,
                256 * 1024,
                "terminal_actor_process_lease_invalid",
            )
        except BridgeError:
            raise
        except Exception as error:
            raise BridgeError("terminal_actor_process_lease_invalid") from error
        digest = hashlib.sha256(persisted).hexdigest()
        if persisted != payload or digest != lease.canonical_sha256:
            raise BridgeError("terminal_actor_process_lease_invalid")
        return digest

    def _persist_process_lease_v1(self, lease: ProcessLeaseV1) -> str:
        """Durably bind the exact private lease before downstream safety use."""
        ...
        try:
            if not (path.exists() or path.is_symlink()):
                self._write_immutable(path, lease.canonical_bytes)
        except Exception as error:
            raise BridgeError("terminal_actor_process_lease_invalid") from error
        return self._validate_persisted_process_lease_v1(lease)

    def _publish_background_handoff_v1(
        self,
        manifest_rows: list[dict[str, Any]],
    ) -> None:
        """Seal lease first, then publish the exact matching task manifest."""

        lease = self._actor.seal_process_lease_v1(manifest_rows)
        self._process_lease_v1 = lease
        self._persist_process_lease_v1(lease)
    ...
    async def _finalize_handoff(
        ...
        *,
        ...
        bridge_closure = getattr(self, "_bridge_closure_receipt_v1", None)
        if bridge_closure is None and outcome is not None and original_error is None:
            bridge_closure = outcome.closure_receipt
            if bridge_closure is None:
                # Compatibility for injected successful bridge doubles.  The
                # production bridge always supplies a typed receipt.
                bridge_closure = BridgeClosureReceiptV1(
        ...
        if bridge_closure is None and original_error is not None:
            raw_closure = getattr(original_error, "cleanup_receipt", None)
            if isinstance(raw_closure, Mapping):
                try:
                    bridge_closure = BridgeClosureReceiptV1.from_mapping(
                        dict(raw_closure)
                    )
                except BridgeError as error:
                    errors.append(error)
        self._bridge_closure_receipt_v1 = bridge_closure
        self._bridge_closed = bool(
            bridge_closure is not None and bridge_closure.stdio_bridge_closed
        )
        self._child_process_closed = bool(
            bridge_closure is not None and bridge_closure.runtime_child_closed
        )
        cleanup_attempted = original_error is not None
        cleanup_verified = bool(
            bridge_closure is not None and bridge_closure.process_safe
        )
        if bridge_closure is None or not bridge_closure.process_safe:
        ...
        try:
            ...
            self._publish_background_handoff_v1(manifest_rows)
        ...
        if not manifest_published and (
            bridge_closure is not None and bridge_closure.process_safe
        ):
            cleanup_attempted = True
            try:
                cleanup_verified = bool(await self._actor.cleanup_active())
            except Exception as error:
        ...
        if not manifest_published and manifest_error is not None:
            errors.append(manifest_error)
        if not manifest_published:
            failure_source = errors[0] if errors else manifest_error or original_error
            try:
                manifest_payload = canonical_json(
                    {
                        ...
                        # Without a durable exact lease + normal manifest,
                        # background cleanup is not identity-bound evidence.
                        "cleanup_verified": False,
                    }
                )
                self._write_immutable(
                ...
                errors.append(error)
        if type(getattr(self, "_trial_lifecycle", None)) is TrialLifecycleCoordinator:
            try:
                self._runtime_cleanup_facts_v1 = self._derive_runtime_cleanup_facts_v1()
                if not self._runtime_cleanup_facts_v1.quiescence_proven:
                    self._persist_runtime_cleanup_receipt_v1(
    ...
    def _derive_runtime_cleanup_facts_v1(self) -> RuntimeCleanupFactsV1:
        """Bind the bridge/actor/lease axes to exact private artifacts."""

        root = self._artifact_source_dir()
        closure = getattr(self, "_bridge_closure_receipt_v1", None)
        if closure is None:
            closure = BridgeClosureReceiptV1(
                handler_completion=HandlerCompletionStateV1.UNVERIFIED,
        ...
        if not run_path.exists() or run_path.is_symlink():
            run_path = root / "runtime-emergency.json"
        _run_raw, run_sha256 = self._artifact_sha256(
            run_path,
            16 * 1024 * 1024,
        )
        events_path = root / "runtime" / "events.jsonl"
        if events_path.exists() and not events_path.is_symlink():
            _events_raw, events_sha256 = self._artifact_sha256(
                events_path,
                64 * 1024 * 1024,
            )
        else:
        ...
        _exposure_raw, exposure_sha256 = self._artifact_sha256(root / EXPOSURE_RECEIPT)
        background_raw, background_sha256 = self._artifact_sha256(
            root / "runtime-background-manifest.json"
        )
        ...
        if (
            type(lease) is ProcessLeaseV1
            and type(binding) is tuple
            and len(binding) == 3
        ):
            try:
                durable_lease_sha256 = self._validate_persisted_process_lease_v1(lease)
            except BridgeError:
                durable_lease_sha256 = None
            process_lease_sealed = bool(
                durable_lease_sha256 == lease.canonical_sha256
                and binding[0] == background_raw
                and binding[1] == background_sha256
                and len(binding[2]) == lease.process_count
            )
        process_lease_sha256 = lease.canonical_sha256 if process_lease_sealed else None
        background_state = (
            BackgroundLeaseStateV1.EMPTY
            if process_lease_sealed and lease.process_count == 0
            else BackgroundLeaseStateV1.SEALED_RETAINED
            if process_lease_sealed
            else BackgroundLeaseStateV1.UNVERIFIED
        )
        return RuntimeCleanupFactsV1(
            run_id=str(self._run_spec["run_id"]),
            trial_id=str(self._run_spec["trial_id"]),
            attempt_id=str(self._run_spec["attempt_id"]),
            run_spec_sha256=rust_run_spec_sha256(self._run_spec),
            runtime_run_sha256=run_sha256,
            runtime_events_sha256=events_sha256,
            git_baseline_sha256=baseline_sha256,
            git_exposure_sha256=exposure_sha256,
            process_lease_sha256=process_lease_sha256,
            background_manifest_sha256=background_sha256,
            handler_completion=closure.handler_completion,
            actor_dispatch=closure.actor_dispatch,
            actor_quiescent=closure.actor_quiescent,
            remote_census_verified=closure.remote_census_verified,
            remote_survivor_count=closure.remote_survivor_count,
            process_lease_sealed=process_lease_sealed,
            background_state=background_state,
            stdio_bridge_closed=closure.stdio_bridge_closed,
            runtime_child_closed=closure.runtime_child_closed,
        )

    def _persist_runtime_cleanup_receipt_v1(
        self,
        *,
        workspace_after_sha256: str | None,
    ) -> RuntimeCleanupReceiptV1:
        facts = self._runtime_cleanup_facts_v1
        if type(facts) is not RuntimeCleanupFactsV1:
            facts = self._derive_runtime_cleanup_facts_v1()
            self._runtime_cleanup_facts_v1 = facts
        receipt = facts.finalize(workspace_after_sha256=workspace_after_sha256)
        existing = self._runtime_cleanup_receipt_v1
        if existing is not None:
            if existing != receipt:
                raise BridgeError("runtime_cleanup_receipt_conflict")
            return existing
        self._write_immutable(
            self._artifact_source_dir() / "runtime-cleanup-closure-v1.json",
            receipt.to_bytes(),
        )
        self._runtime_cleanup_receipt_v1 = receipt
        return receipt

    def _load_bound_runtime_cleanup_receipt_v1(
        self,
        *,
        workspace_after_sha256: str,
    ) -> RuntimeCleanupReceiptV1:
        try:
            raw = _read_artifact_regular(
                self._artifact_source_dir() / "runtime-cleanup-closure-v1.json",
                64 * 1024,
                "runtime_cleanup_receipt_invalid",
            )
            persisted = RuntimeCleanupReceiptV1.from_bytes(raw)
            facts = self._derive_runtime_cleanup_facts_v1()
            expected = facts.finalize(workspace_after_sha256=workspace_after_sha256)
        except Exception as error:
            raise BridgeError("runtime_cleanup_receipt_invalid") from error
        if persisted != expected or not persisted.verifier_permitted:
            raise BridgeError("runtime_cleanup_receipt_invalid")
        self._runtime_cleanup_facts_v1 = facts
        self._runtime_cleanup_receipt_v1 = persisted
        return persisted
    ...
    async def post_agent_workspace_snapshot_v1(
        ...
        *,
        ...
        lifecycle_enabled = (
            type(getattr(self, "_trial_lifecycle", None)) is TrialLifecycleCoordinator
        )
        if lifecycle_enabled:
            cleanup_facts = getattr(self, "_runtime_cleanup_facts_v1", None)
            if (
                type(cleanup_facts) is not RuntimeCleanupFactsV1
                or not cleanup_facts.quiescence_proven
            ):
                error = BridgeError("runtime_cleanup_quiescence_unverified")
                try:
                    self._persist_runtime_cleanup_receipt_v1(
                        workspace_after_sha256=None
                    )
                except BaseException as receipt_error:
                    error.__cause__ = receipt_error
                self._post_agent_snapshot_outcome = (None, error)
                raise error
        ...
        try:
            ...
            if lifecycle_enabled:
                try:
                    ...
                    liveness_error.__cause__ = receipt_error
            self._cleanup_git_history_escrow()
            raise
        ...
        if lifecycle_enabled:
            self._persist_runtime_cleanup_receipt_v1(
                workspace_after_sha256=persisted_receipt.canonical_sha256
            )
    ...
    async def complete_runtime_and_prove_safety_v1(
        ...
        *,
        ...
        cleanup_receipt: RuntimeCleanupReceiptV1 | None = None
        if snapshot_error is None:
            ...
            if workspace_complete:
                try:
                    cleanup_receipt = self._load_bound_runtime_cleanup_receipt_v1(
                        workspace_after_sha256=persisted.canonical_sha256
                    )
                except Exception:
                    cleanup_receipt = None
                    workspace_complete = False

        cleanup_process_safe = bool(
            cleanup_receipt is not None and cleanup_receipt.verifier_permitted
        )
        self._bridge_closed = bool(
            cleanup_process_safe and cleanup_receipt.stdio_bridge_closed
        )
        self._child_process_closed = bool(
            cleanup_process_safe and cleanup_receipt.runtime_child_closed
        )
        ...
        if workspace_complete and cleanup_process_safe:
            try:
                await self._rehydrate_git_history_for_verifier(
                    hard_deadline_monotonic_ns=hard_deadline_monotonic_ns,
                )
    ...
    def _publish_unsafe_diagnostic_v1(self) -> None:
        """Copy bounded closure/lifecycle evidence before private-root deletion."""

        lifecycle = self._trial_lifecycle
        plane = getattr(self, "_control_plane", None)
        if (
            type(lifecycle) is not TrialLifecycleCoordinator
            or lifecycle.phase is not TrialPhase.EXECUTION_FAILED_UNSAFE
            or not isinstance(plane, ControlPlane)
            or not plane.root.is_dir()
        ):
            raise BridgeError("unsafe_diagnostic_handoff_invalid")
        closure_raw = _read_artifact_regular(
            plane.root / "runtime-cleanup-closure-v1.json",
            64 * 1024,
            "unsafe_diagnostic_handoff_invalid",
        )
        try:
            closure = RuntimeCleanupReceiptV1.from_bytes(closure_raw)
        except ValueError as error:
            raise BridgeError("unsafe_diagnostic_handoff_invalid") from error
        manifests: list[dict[str, object]] = []
        for path in sorted(plane.root.glob("phase-*.json")):
            raw = _read_artifact_regular(
                path,
                1024 * 1024,
                "unsafe_diagnostic_handoff_invalid",
            ...
            try:
                ...
                raise BridgeError("unsafe_diagnostic_handoff_invalid") from error
            if type(manifest) is not dict or raw != canonical_json(manifest):
                raise BridgeError("unsafe_diagnostic_handoff_invalid")
            manifests.append(
                {
                    "name": path.name,
        ...
        if not any(
            row["manifest"].get("phase") == TrialPhase.EXECUTION_FAILED_UNSAFE.value
            for row in manifests
        ):
            raise BridgeError("unsafe_diagnostic_handoff_invalid")
        payload = canonical_json(
            {
                "schema_version": "nano-unsafe-execution-diagnostic-v1",
                "run_id": lifecycle.run_id,
                "trial_id": lifecycle.trial_id,
                "attempt_id": lifecycle.attempt_id,
                "run_spec_sha256": lifecycle.run_spec_sha256,
                "closure_receipt_sha256": hashlib.sha256(closure_raw).hexdigest(),
                "closure_receipt": closure.to_dict(),
                "lifecycle_manifests": manifests,
                "terminal_phase": TrialPhase.EXECUTION_FAILED_UNSAFE.value,
            }
        )
        self._write_immutable(
            plane.public_root / "unsafe-execution-diagnostic-v1.json",
            payload,
        )
    ...
    async def post_verifier_finalize_v1(
        ...
        *,
        ...
        plane = getattr(self, "_control_plane", None)
        if isinstance(plane, ControlPlane) and cleanup.succeeded:
            if lifecycle.phase is TrialPhase.EXECUTION_FAILED_UNSAFE:
                self._publish_unsafe_diagnostic_v1()
                plane.cleanup()
            elif (
                lifecycle.phase
                in {
                    TrialPhase.SUBMISSION_READY,
                    TrialPhase.COMPLETE_NOT_SUBMITTABLE,
                }
                and getattr(self, "_post_verifier_publication_error", None) is None
            ):
                plane.cleanup()
    ...
    async def _run_bound(
        ...
        *,
        ...
        try:
            ...
            self._bridge_closure_receipt_v1 = outcome.closure_receipt
        except BaseException as error:
            original_error = error
            raw_closure = getattr(error, "cleanup_receipt", None)
            if isinstance(raw_closure, Mapping):
                try:
                    self._bridge_closure_receipt_v1 = (
                        BridgeClosureReceiptV1.from_mapping(dict(raw_closure))
                    )
                except BridgeError:
                    self._bridge_closure_receipt_v1 = None
            elif not bridge_dispatch_started:
                self._bridge_closure_receipt_v1 = BridgeClosureReceiptV1(
                    handler_completion=HandlerCompletionStateV1.COMPLETED,
                    actor_dispatch=ActorDispatchStateV1.REVOKED,
                    actor_quiescent=True,
                    remote_census_verified=True,
