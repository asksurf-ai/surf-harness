...
class NanoGrokBuildAgent(BaseAgent):
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        self._artifact_publication_committed_v1 = False
    ...
    def _write_git_history_rehydration_receipt(
        ...
        *,
        ...
        coverage = run.get("provider_call_coverage") if isinstance(run, dict) else None
        tasks = background.get("tasks") if isinstance(background, dict) else None
        if (
            not isinstance(coverage, dict)
            or coverage.get("in_flight") != 0
            or run.get("run_spec_sha256") != rust_run_spec_sha256(self._run_spec)
    ...
    async def prepare_shared_verifier_v1(
        ...
        *,
        ...
        """Prepare once before publication; verifier entry only asserts its commit."""

        already_prepared = self._git_history_rehydration_outcome is not None
        await self._rehydrate_git_history_for_verifier(
            hard_deadline_monotonic_ns=hard_deadline_monotonic_ns,
        )
        if already_prepared and not getattr(
            self,
            "_artifact_publication_committed_v1",
            False,
        ):
            raise BridgeError("git_history_publication_uncommitted")
        ...
        try:
            ...
            if require_lifecycle:
                ...
                if admission not in {"started", "not_started"}:
                    raise BridgeError("git_history_publication_invalid")
                outcome = self._git_history_rehydration_outcome
                if admission == "started" and not (
                    type(outcome) is tuple
                    and len(outcome) == 2
                    and outcome[0]
                    in {"restored", "not_applicable", "deferred_background_active"}
            ...
            publication = publish_artifacts(
                logs_dir=source_dir,
                publication_dir=(
                    self.logs_dir if source_dir != self.logs_dir else None
                ),
                run_spec=self._run_spec,
                ...
                require_git_history_lifecycle=require_lifecycle,
                validated_runtime=validated_runtime,
            )
            # `publish_artifacts` returns only after ControlPlane has linked the
            # exact staged set and committed agent-run.json last. This in-memory
            # latch lets the verifier seam fail closed without reconstructing
            # authority from mutable public files after private cleanup.
            self._artifact_publication_committed_v1 = True
