...
_TERMINAL_VERIFIER_RESULT_KINDS = frozenset(
    {
        "passed",
        "assertion_failed",
...
def status_lattice(
    ...
    *,
    ...
    """Reduce census, verifier truth, and admission without conflating them."""

    expected = tuple(expected_task_ids)
    observed = tuple(str(row.get("task", "")) for row in rows)
    exact_census = (
        bool(expected)
        and len(expected) == TB21_TASK_COUNT
        and (
    ...
    unsafe = any(
        (
            isinstance(row.get("exception"), Mapping)
            and row["exception"].get("message") == "verifier_skipped_unsafe"  # type: ignore[index]
        )
        or row.get("verifier_status") == "SKIPPED_UNSAFE"
        or row.get("execution_status") == "EXECUTION_FAILED_UNSAFE"
        for row in rows
    )
    if unsafe:
        return {
            "execution_status": "EXECUTION_FAILED_UNSAFE",
            "verifier_status": "SKIPPED_UNSAFE",
            "submission_status": "NOT_EVALUATED",
        }
    verifier_terminal = exact_census and all(
        (
            row.get("verifier_result_kind") in _TERMINAL_VERIFIER_RESULT_KINDS
            or (
                "verifier_result_kind" not in row
                and type(row.get("reward")) in {int, float}
            )
        )
        and row.get("interruption_state") not in {"not_started", "incomplete"}
        for row in rows
    )
    if not verifier_terminal:
        return {
            "execution_status": "EXECUTION_INCOMPLETE",
            "verifier_status": "INCOMPLETE",
            "submission_status": "NOT_EVALUATED",
        }
    return {
        "execution_status": "COMPLETE",
        "verifier_status": "TERMINAL",
        "submission_status": (
            "SUBMISSION_READY" if submission_ready else "COMPLETE_NOT_SUBMITTABLE"
        ),
    }
...
class _SystematicSetupFailFast(RuntimeError):
    """The local K observed the allowlisted provider-free setup threshold."""


# Keep this allowlist closed to campaign-invalidating setup failures. Typed
# terminal trial exceptions are valid official denominator results and must not
# become fail-fast signatures because they repeat across tasks, families, or Ks.
_SYSTEMATIC_SETUP_SIGNATURE = ("RuntimeError", "git_history_baseline_failed")
_LOCAL_FAILFAST_SCHEMA = "nano-tb21-local-failfast-v1"
_LOCAL_FAILFAST_RECEIPT = "nano-local-failfast.json"
...
class _SystematicSetupFailFastHook:
    def __init__(self, *, job_dir: Path, selected_count: int, concurrency: int) -> None:
        self._job_dir = job_dir
        self._selected_count = selected_count
        self._concurrency = concurrency
        self._matched_trial_ids: list[str] = []
        self._matched_trial_id_set: set[str] = set()
        self._provider_started = False
        self._triggered = False

    async def agent_started(self, _event: object) -> None:
        self._provider_started = True

    async def __call__(self, event: object) -> None:
        if self._triggered or self._provider_started:
            return
        result = _event_field(event, "result")
        if (
            _event_field(result, "agent_execution") is not None
            or _event_field(result, "agent_result") is not None
        ):
            self._provider_started = True
            return
        exception = _event_field(result, "exception_info")
        setup = _event_field(result, "agent_setup")
        signature = (
            _event_field(exception, "exception_type"),
            _event_field(exception, "exception_message"),
        )
        if (
            signature != _SYSTEMATIC_SETUP_SIGNATURE
            or _event_field(setup, "started_at") is None
        ):
            return
        trial_id = _event_field(result, "trial_name")
        if (
            not isinstance(trial_id, str)
            or not trial_id
            or len(trial_id.encode("utf-8")) > 4096
            or any(ord(character) < 32 for character in trial_id)
            or trial_id in self._matched_trial_id_set
        ):
            return
        self._matched_trial_id_set.add(trial_id)
        self._matched_trial_ids.append(trial_id)
        if len(self._matched_trial_ids) < 2:
            return
        self._triggered = True
        value = {
            "schema_version": _LOCAL_FAILFAST_SCHEMA,
            "status": "triggered",
            "threshold": 2,
            "signature": {
                "exception_type": _SYSTEMATIC_SETUP_SIGNATURE[0],
                "exception_message": _SYSTEMATIC_SETUP_SIGNATURE[1],
            },
            "matched_trial_ids": self._matched_trial_ids[:2],
            "selected_count": self._selected_count,
            "concurrency": self._concurrency,
            "provider_started": int(self._provider_started),
            "attempt": 1,
            "retry": 0,
            "triggered_at": datetime.now(UTC).isoformat().replace("+00:00", "Z"),
        }
        path = self._job_dir / _LOCAL_FAILFAST_RECEIPT
        try:
            descriptor = os.open(
                path,
            ...
            raise TB21Error("local_failfast_receipt_publish_failed") from error
        raise _SystematicSetupFailFast("systematic_setup_failfast")


def _is_systematic_setup_failfast(error: BaseException) -> bool:
    if isinstance(error, _SystematicSetupFailFast):
        return True
    if isinstance(error, BaseExceptionGroup):
        return any(_is_systematic_setup_failfast(item) for item in error.exceptions)
    return False
...
def _submission_audit_projection(
    trial_dir: Path,
    *,
    spec: Mapping[str, Any],
    rewarded: bool,
    ...
    """Canonical protected/Git audit projection for collector and validators."""

    task = spec["task"]
    contamination = _contamination_audit(trial_dir, rewarded=rewarded)
    ...
    if not (
        result_classification == "errored" and runtime_entry_state == "not_started"
    ):
        return contamination, git_history
    return (
        {
...
def _row(
    *,
    ...
    contamination, git_history = _submission_audit_projection(
        trial_dir,
        spec=spec,
        rewarded=collector_pass,
...
def _terminalize_interruption(
    ...
    *,
    ...
    if _is_systematic_setup_failfast(error):
        reason = "systematic_setup_failfast"
...
def collect_job(
    ...
    *,
    ...
    summary_admission_ready = bool(
        isinstance(summary_gates, dict)
        and all(
            summary_gates.get(name) is True
            for name in (
                "job_terminal",
                "exact_inventory",
                "result_identity",
    ...
    summary.update(
        status_lattice(
            rows,
            tuple(str(spec["task"]["id"]) for spec in specs),
            submission_ready=summary_admission_ready,
        )
    )
...
async def run_baseline(
    ...
    *,
    ...
    if len(prepared.selected) >= 2:
        from harbor.trial.hooks import TrialEvent

        failfast_hook = _SystematicSetupFailFastHook(
            job_dir=bound.job.job_dir,
            selected_count=len(prepared.selected),
            concurrency=prepared.concurrency,
        )
        bound.job.add_hook(TrialEvent.AGENT_START, failfast_hook.agent_started)
        bound.job.add_hook(
            TrialEvent.END,
            failfast_hook,
        )
