...
def _directory_digests(path: Path) -> tuple[str, str]:
    """Return Harbor-compatible dirhash and an explicit source-file manifest hash."""

    source_rows: list[dict[str, object]] = []

    def visit(directory: Path, relative: Path) -> str | None:
        ...
        for entry in entries:
            ...
            if stat.S_ISLNK(metadata.st_mode):
                raise TB21Error("task_source_symlink")
            if stat.S_ISDIR(metadata.st_mode):
                child_hash = visit(entry, child_relative)
                if child_hash is None:
                    continue
                properties = (
                    f"dirhash:{child_hash}",
                    f"name:{entry.name}",
                )
            elif stat.S_ISREG(metadata.st_mode):
                raw = _read_regular(entry)
                ...
                raise TB21Error("task_source_special_file")
            descriptors.append("\0".join(sorted(properties)))
        if not descriptors:
            return None
        encoded = "\0\0".join(sorted(descriptors)).encode("utf-8")
        return hashlib.sha256(encoded).hexdigest()

    task_digest = visit(path, Path())
    if task_digest is None or not source_rows:
        raise TB21Error("task_source_empty")
    source_sha256 = hashlib.sha256(_canonical(source_rows)).hexdigest()
    return task_digest, source_sha256
...
def assert_tb21_pin(checkout: Path) -> None:
    if _git_head(checkout, "tb21_checkout_invalid") != TB21_SOURCE_COMMIT:
        raise TB21Error("tb21_pin_mismatch")
    try:
        ...
        raise TB21Error("tb21_checkout_invalid") from error
    if status:
        raise TB21Error("tb21_checkout_dirty")
...
def _load_dispatch(job_dir: Path) -> tuple[Mapping[str, Any], ...]:
    ...
    for spec in specs:
        ...
        if (
            spec.get("schema_version") != "nano-run-spec-alpha-1"
...
def _artifact_evidence(
    ...
    success_valid = bool(
        diagnostic and kind == "success_atif" and workspace.snapshot_usable
    )
    return _ArtifactEvidence(
...
def _row(
    *,
    ...
    strict_pass = bool(collector_pass and reliable)
...
def _interruption_value(
    ...
    *,
    ...
    if reason not in {"operator_interrupted", "runner_exception"}:
        raise TB21Error("job_not_terminal")
...
def collect_job(
    ...
    *,
    ...
    trajectories_for_started = sum(
        artifacts.trajectory_valid for _row_value, artifacts in started_rows
    )
    ...
    summary: dict[str, object] = {
        ...
        "gates": {
            ...
            "trajectories_for_runtime_starts": (
                trajectories_for_started == len(started_rows)
            ),
        ...
        "terminal_evidence": {
            ...
            "valid_trajectories_for_started": trajectories_for_started,
        },
        "duration_ms": {
...
def _prepare_inventory(
    ...
    try:
        ...
        raise TB21Error("harbor_pin_mismatch") from error
    assert_tb21_pin(source_checkout)
