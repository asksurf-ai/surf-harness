...
def _valid_context_checkpoint_rejection(data: object) -> bool:
    if (
        ...
        or isinstance(data, _DuplicateAwareDict)
        and bool(data.duplicate_keys)
        or set(data) != _CONTEXT_CHECKPOINT_REJECTED_KEYS
...
def _event_prefix_from_raw(
    ...
    for sequence, line in enumerate(raw.splitlines()):
        ...
        if (
            ...
            and requests.get(int(checkpoint_prepare["turn_index"]))
            in {"completed", "failed"}
            and event_type
        ...
        if event_type in {"run.completed", "run.failed"}:
            ...
            checkpoint_prepare_phase = (
                isinstance(observation, dict)
                and observation.get("phase") == "checkpoint_prepare"
            )
            if checkpoint_prepare_phase != (checkpoint_source is not None):
                return None, True
