...
def _valid_context_checkpoint(data: object) -> bool:
    return bool(
        ...
        and not (isinstance(data, _DuplicateAwareDict) and bool(data.duplicate_keys))
        and set(data) == _CONTEXT_CHECKPOINT_KEYS
        and data.get("policy_version") == _CONTEXT_CHECKPOINT_POLICY_VERSION
        and _is_sha256(data.get("source_history_sha256"))
        and _is_sha256(data.get("checkpoint_history_sha256"))
        and all(
            _nonnegative_integer(data.get(field)) is not None
            for field in (
                "source_history_items",
                "checkpoint_history_items",
                "provider_turn_count",
        ...
        and int(data["source_history_items"]) > 0
        and 0
        < int(data["checkpoint_history_items"])
        < int(data["source_history_items"])
        and int(data["provider_turn_count"]) > 0
        and int(data["observed_input_tokens"]) > 0
    )
...
def _event_prefix_from_raw(
    ...
    for sequence, line in enumerate(raw.splitlines()):
        ...
        if (
            ...
            or event.get("type") == "context.checkpointed"
        ...
        if event_type in {"run.completed", "run.failed"}:
            ...
            if checkpoint_expected_history_sha is not None:
                ...
                if (
                    ...
                    or media_receipt.get("history_sha256")
                    != checkpoint_expected_history_sha
                    or data.get("history_item_count") != 3
