...
def parse_git_history_baseline_receipt(
    ...
    if (
        ...
        or status not in {"created", "isolated", "preserved"}
        or access == GIT_HISTORY_ACCESS_NOT_REQUIRED
        and status not in {"created", "isolated"}
        or access == GIT_HISTORY_ACCESS_REQUIRED
        and status != "preserved"
        or topology_before not in {"zero", "root", "nested"}
        or topology_after not in {"root", "nested"}
        or not _repo_relative_path(path)
        or not all(
        ...
        or not _hex(value.get("root_commit_oid"), {40, 64})
        or not _hex(value.get("root_tree_oid"), {40, 64})
        or not (
