...
_DEADLINE_SEAM_VERSION = "nano-harbor-agent-phase-trial-lifecycle-seam-v6"
_VERIFIER_CLEANUP_SEAM_VERSION = "nano-trial-lifecycle-verifier-seam-v3"
_RESOLVED_TASK_CACHE_SEAM_VERSION = "nano-resolved-package-task-cache-seam-v1"
_POST_AGENT_DIAGNOSTIC_TIMEOUT_SEC = 150.0
_PRE_VERIFIER_PREPARATION_TIMEOUT_SEC = 150.0
...
def _install_verifier_cleanup_seam() -> None:
    ...
    async def run_verifier_with_cleanup(self: Any) -> None:
        agent = self.agent
        if getattr(agent, "SUPPORTS_TRIAL_LIFECYCLE_V1", False):
            proof_reader = getattr(agent, "verifier_safety_proof_v1", None)
            finalize = getattr(agent, "post_verifier_finalize_v1", None)
            if not callable(proof_reader) or not callable(finalize):
                raise DeadlineContractError("trial_lifecycle_hook_unavailable")
            proof = proof_reader()
            if type(proof) is not VerifierSafetyProof:
                raise DeadlineContractError("verifier_safety_proof_invalid")
            verifier_error: BaseException | None = None
            verifier_traceback: Any = None
            if proof.safe is not True:
                verifier_status = "skipped_unsafe"
                verifier_error = DeadlineContractError("verifier_skipped_unsafe")
                verifier_traceback = verifier_error.__traceback__
            else:
                try:
                    await current(self)
                except BaseException as error:
                    verifier_error = error
                    verifier_traceback = error.__traceback__
                    verifier_status = (
            ...
            try:
                ...
                if verifier_error is not None:
                    raise finalize_error from verifier_error
                raise
            if verifier_error is not None:
                raise verifier_error.with_traceback(verifier_traceback)
            return
        if not getattr(
            agent,
        ...
        try:
            ...
            verifier_disabled = getattr(verifier_config, "disable", False) is True
            prepare = getattr(agent, "prepare_shared_verifier_v1", None)
            if not verifier_disabled and callable(prepare):
                preparation_deadline_monotonic_ns = host_monotonic_ns() + int(
                    _PRE_VERIFIER_PREPARATION_TIMEOUT_SEC * 1_000_000_000
                )
                user = self.task.config.agent.user
                with self.agent_environment.with_default_user(user):
                    await prepare(
                        hard_deadline_monotonic_ns=(preparation_deadline_monotonic_ns),
                    )
            await current(self)
        except BaseException as error:
            primary_error = error
...
def install_harbor_deadline_seam() -> None:
    ...
    async def run_agent_phase_with_deadline(
        ...
        *,
        ...
        async def prepare_before_publication() -> None:
            if not getattr(
                agent,
                "SUPPORTS_BACKGROUND_VERIFIER_HANDOFF_V1",
                False,
                ...
                return
            verifier_config = getattr(getattr(self, "config", None), "verifier", None)
            if getattr(verifier_config, "disable", False) is True:
                return
            prepare = getattr(agent, "prepare_shared_verifier_v1", None)
            if not callable(prepare):
                raise DeadlineContractError(
                    "shared_verifier_preparation_hook_unavailable"
                )
            preparation_deadline_monotonic_ns = host_monotonic_ns() + int(
                _PRE_VERIFIER_PREPARATION_TIMEOUT_SEC * 1_000_000_000
            )
        ...
        if getattr(agent, "SUPPORTS_TRIAL_LIFECYCLE_V1", False):
            ...
            if primary_error is not None and not isinstance(primary_error, Exception):
                raise primary_error.with_traceback(primary_traceback)
            complete = getattr(agent, "complete_runtime_and_prove_safety_v1", None)
            if not callable(complete):
                raise DeadlineContractError("trial_lifecycle_hook_unavailable")
            with self.agent_environment.with_default_user(user):
                proof = await complete(
                    primary_error=primary_error,
                    snapshot_error=hook_error,
            ...
            if proof.safe is not True:
                self._record_exception(  # noqa: SLF001
                    DeadlineContractError("verifier_skipped_unsafe")
                )
            return
            ...
            if isinstance(primary_error, Exception) and hook_error is None:
                ...
                if callable(decide):
                    ...
                    if (
                        ...
                        await prepare_before_publication()
                        try:
                            raise primary_error.with_traceback(primary_traceback)
                        except Exception as restored:
                            self._record_exception(restored)  # noqa: SLF001
