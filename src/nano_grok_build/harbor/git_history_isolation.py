...
new_admin=$(mktemp -d .nano-git-new.XXXXXXXX)
trap 'test ! -e "$new_admin" || find "$new_admin" -depth -delete 2>/dev/null || true' EXIT HUP INT TERM
git init -q --bare "$new_admin"
printf '[core]\n\trepositoryformatversion = 0\n\tfilemode = true\n\tsymlinks = true\n\tignorecase = false\n\tbare = false\n\tlogallrefupdates = false\n\thooksPath = /dev/null\n[user]\n\tname = Nano Baseline\n\temail = baseline@invalid.local\n' > "$new_admin/config"
temp_name=${{new_admin#./}}
GIT_DIR="$new_admin" GIT_WORK_TREE="$workspace" GIT_INDEX_FILE="$new_admin/index" git add -f -A -- . ":(exclude)$temp_name"
root_tree=$(GIT_DIR="$new_admin" GIT_WORK_TREE="$workspace" GIT_INDEX_FILE="$new_admin/index" git write-tree)
root_commit=$(printf 'Trusted current baseline %s\n' "$run_hash" | GIT_AUTHOR_NAME='Nano Baseline' GIT_AUTHOR_EMAIL='baseline@invalid.local' GIT_COMMITTER_NAME='Nano Baseline' GIT_COMMITTER_EMAIL='baseline@invalid.local' GIT_AUTHOR_DATE='2000-01-01T00:00:00Z' GIT_COMMITTER_DATE='2000-01-01T00:00:00Z' GIT_DIR="$new_admin" GIT_WORK_TREE="$workspace" GIT_INDEX_FILE="$new_admin/index" git commit-tree "$root_tree")
GIT_DIR="$new_admin" git update-ref refs/heads/nano-baseline "$root_commit"
GIT_DIR="$new_admin" git symbolic-ref HEAD refs/heads/nano-baseline
GIT_DIR="$new_admin" git repack -Adq --unpack-unreachable=now
GIT_DIR="$new_admin" git prune --expire now
mv "$new_admin" .git
new_admin=
...
remote_count=$(git remote | wc -l | tr -d ' ')
alternate_count=0
test "$root_count" = 1 && test "$ref_count" = 1 && test "$remote_count" = 0
printf '{{"schema_version":"{HISTORY_BASELINE_SCHEMA}","policy_version":"{HISTORY_BASELINE_POLICY}","run_spec_sha256":"%s","capability_instruction_sha256":"%s","trusted_manifest_sha256":"%s","status":"created","source_commit_oid":null,"source_tree_oid":null,"root_commit_oid":"%s","root_tree_oid":"%s","preexisting_commit_count":0,"root_commit_count":%s,"ref_count":%s,"remote_count":%s,"alternate_count":%s,"old_metadata_removed":false}}\n' "$run_hash" "$instruction_hash" "$manifest_hash" "$root_commit" "$root_tree" "$root_count" "$ref_count" "$remote_count" "$alternate_count"
"""
...
async def isolate_preexisting_git_history(
    *,
    ...
    if before_topology == "zero" and access == GIT_HISTORY_ACCESS_REQUIRED:
        raise RuntimeError("git_history_baseline_failed")
    admitted_workspace = workspace
    if before_path not in {None, "."}:
        admitted_workspace = f"{workspace}/{before_path}"
    if before_topology == "zero":
        core_raw = await execute(
            _create_repo_script(workspace, run_spec_sha256, capability)
        )
    else:
        core_raw = await execute(
    ...
    if (
        ...
        or before_topology == "zero"
        and (after_topology != "root" or after_path != ".")
        or before_topology != "zero"
        and (after_topology != before_topology or after_path != before_path)
    ):
        raise RuntimeError("git_history_baseline_failed")
    combined = {
        **core,
