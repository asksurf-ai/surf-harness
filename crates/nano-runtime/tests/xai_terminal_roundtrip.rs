...
#[tokio::test]
async fn semantic_checkpoint_v5_prepares_typed_capsule_and_binds_four_item_reset() {
    let root = tempfile::tempdir().expect("test root");
    let contract = write_contract_with_max_provider_turns(root.path(), 64);
    let artifacts = root.path().join("artifacts");
    ...
    responses.push_back(phase_message_response(
        "response-capsule",
        &serde_json::to_string_pretty(&capsule).expect("capsule JSON"),
    ));
    responses.push_back(phase_message_response("response-candidate", "candidate"));
    responses.push_back(phase_message_response(
        "response-critic",
    ...
    let outcome = run_agent_with_deadline_and_review(
        ...
        &mut executor,
        &deadline,
        CompletionReviewPolicy::SemanticCheckpointV5,
    )
    .await
    ...
    let prepare = &provider.requests[15];
    assert_eq!(prepare.mode, ProviderRequestMode::FinalOnly);
    assert!(prepare.tools.is_empty());
    assert_eq!(prepare.history.len(), 33);
    assert!(prepare.history.iter().any(|item| matches!(
        item,
    ...
    assert!(prepare.history.iter().any(|item| matches!(
        item,
        HistoryItem::User { content }
            if content.contains("semantic_checkpoint_prepare_v1")
    ...
    assert_eq!(checkpoint["data"]["observed_input_tokens"], 300_003);
    assert_eq!(checkpoint["data"]["prepare_turn_index"], 15);
    assert_eq!(checkpoint["data"]["tail_reserve_ms"], 900_000);
    let prepare_request = parsed
        .iter()
        .find(|event| {
            event["type"] == "provider.requested"
                && event["data"]["budget_observation"]["phase"] == "checkpoint_prepare"
        })
        .expect("prepare request event");
    assert_eq!(
        prepare_request["data"]["checkpoint_source_history_sha256"],
        checkpoint["data"]["source_history_sha256"]
    );
    assert_eq!(
        prepare_request["data"]["media_history_receipt"]["history_sha256"],
...
#[tokio::test]
async fn semantic_checkpoint_v5_invalid_capsule_rejects_once_and_preserves_source_history() {
    let root = tempfile::tempdir().expect("test root");
    let contract = write_contract_with_max_provider_turns(root.path(), 64);
    let artifacts = root.path().join("artifacts");
    let deadline = semantic_checkpoint_phase_deadline(Instant::now() + Duration::from_secs(1_060));
    ...
    assert_eq!(outcome.record.terminal_status, TerminalStatus::Success);
    assert_eq!(provider.requests.len(), 19);
    assert_eq!(provider.requests[15].mode, ProviderRequestMode::FinalOnly);
    let resumed = &provider.requests[16];
    assert_eq!(resumed.mode, ProviderRequestMode::ActionOpen);
    assert_eq!(resumed.history.len(), 32);
...
#[tokio::test]
async fn semantic_checkpoint_v5_prepare_rejection_matrix_preserves_source_history() {
    let mut extra_field = semantic_checkpoint_capsule();
    extra_field["unexpected"] = json!(true);
    let cases = vec![
        (
            "empty",
            phase_message_response("response-empty-capsule", ""),
            "semantic_checkpoint_capsule_bytes_exceeded",
            false,
        ),
        (
            "oversize",
            phase_message_response("response-oversize-capsule", &"x".repeat(8193)),
            "semantic_checkpoint_capsule_bytes_exceeded",
            false,
        ),
        (
            ...
            phase_message_response(
                ...
                &serde_json::to_string(&extra_field).expect("extra-field capsule"),
            ),
            "semantic_checkpoint_capsule_json_invalid",
            false,
        ),
        (
            "tool-call",
            phase_named_calls_response(
                "response-prepare-tool-call",
                vec![("call-prepare-tool", "run_terminal_command", json!({}))],
            ),
    ...
    for (label, prepare_response, expected_reason, provider_failed) in cases {
        let root = tempfile::tempdir().expect("test root");
        let contract = write_contract_with_max_provider_turns(root.path(), 64);
        let artifacts = root.path().join("artifacts");
        let deadline =
...
#[tokio::test]
async fn semantic_checkpoint_v5_prepare_provider_failure_rejects_once_without_history_loss() {
    let root = tempfile::tempdir().expect("test root");
    let contract = write_contract_with_max_provider_turns(root.path(), 64);
    let artifacts = root.path().join("artifacts");
    let deadline = semantic_checkpoint_phase_deadline(Instant::now() + Duration::from_secs(1_060));
    ...
    .await
    .expect("prepare provider failure must fall back safely");
    ...
    assert!(resumed.history.iter().any(|item| matches!(
        item,
        HistoryItem::FunctionCall { call_id, .. } if call_id == "call-prepare-failure-0"
    )));
    let events = fs::read_to_string(artifacts.join("events.jsonl")).expect("events");
    ...
    assert_eq!(
        rejection["data"]["reason"],
        "semantic_prepare_transport_failed"
    );
    assert_eq!(rejection["data"]["request_emitted"], true);
    assert_eq!(rejection["data"]["response_received"], false);
    assert_eq!(events.matches("\"type\":\"provider.failed\"").count(), 1);
    assert_eq!(
        events
            .matches("\"type\":\"context.checkpoint_rejected\"")
...
#[tokio::test]
async fn semantic_checkpoint_v5_turn_lease_admits_one_soft_action_then_provisional_and_critic() {
    let root = tempfile::tempdir().expect("test root");
    ...
    responses.push_back(phase_message_response(
        "response-capsule",
        &serde_json::to_string(&capsule).expect("capsule JSON"),
    ));
    for turn in 0..7 {
...
#[tokio::test]
async fn semantic_checkpoint_v5_provisional_and_critic_failures_are_phase_safe() {
        ...
        [("provisional", 23, false), ("critic", 24, true)]
    {
        ...
        responses.push_back(phase_message_response(
            "response-capsule",
            &serde_json::to_string(&semantic_checkpoint_capsule()).expect("capsule JSON"),
        ));
        for turn in 0..7 {
