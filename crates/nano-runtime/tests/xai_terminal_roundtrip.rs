...
use nano_runtime::{
    CompletionReviewPolicy, DeadlineContext, TerminalExecutor, ToolExecutor, ToolResult,
    WorkspaceMode, run_agent, run_agent_with_deadline, run_agent_with_deadline_and_review,
};
            ...
            let input_schema = if *name == "run_terminal_command" {
                json!({
                    ...
                    "properties": {
                        ...
                        "timeout": {"minimum": 0, "type": ["integer", "null"]}
                    },
...
#[tokio::test]
async fn completion_challenge_provider_failure_stays_typed_without_synthetic_final() {
    let root = tempfile::tempdir().expect("test root");
    let contract = write_contract(root.path());
    let artifacts = root.path().join("artifacts");
    let deadline = review_phase_deadline(Instant::now() + Duration::from_secs(10));
    ...
    .await
    .expect_err("in-flight review failure must stay typed");
    ...
    assert_eq!(run["terminal_code"], "phase_script_exhausted");
}

#[tokio::test]
async fn completion_challenge_timeout_stays_typed_without_synthetic_final() {
    let root = tempfile::tempdir().expect("test root");
    let contract = write_contract(root.path());
    let artifacts = root.path().join("artifacts");
    ...
    .await
    .expect_err("in-flight review timeout must stay typed");

    assert_eq!(error.code(), "provider_completion_review_deadline_exceeded");
    assert_eq!(provider.requests.len(), 2);
    ...
    assert_eq!(events.matches("\"type\":\"assistant.final\"").count(), 0);
    assert_eq!(events.matches("\"type\":\"run.completed\"").count(), 0);
    assert_eq!(events.matches("\"type\":\"run.failed\"").count(), 1);
    assert!(!events.contains("\"text\":\"candidate\""));
    ...
    let run: Value =
        serde_json::from_slice(&fs::read(artifacts.join("run.json")).expect("run record"))
            .expect("run JSON");
    assert_eq!(run["terminal_status"], "deadline_failure");
    assert_eq!(
        run["terminal_code"],
    ...
    assert_eq!(run["provider_call_coverage"]["requested"], 2);
    assert_eq!(run["provider_call_coverage"]["completed"], 1);
    assert_eq!(run["provider_call_coverage"]["failed"], 0);
    assert_eq!(run["provider_call_coverage"]["in_flight"], 1);
    ...
    assert_eq!(terminal["type"], "run.failed");
...
async fn shell_success_nonzero_and_utf8_safe_truncation() {
    ...
    assert_eq!(
        success.output,
        concat!(
            "execution_evidence_v1 command=exit:0 timeout=false ",
    ...
    assert!(nonzero.output.starts_with(
        "execution_evidence_v1 command=exit:7 timeout=false \
stdout_trunc=false stderr_trunc=false effect=unobserved\n"
    ));
    assert!(nonzero.output.contains("exit: 7\n"));
    ...
    assert!(
        truncated.output.starts_with(
            "execution_evidence_v1 command=exit:0 timeout=false \
stdout_trunc=false stderr_trunc=false effect=unobserved\n"
        ),
        "unexpected bounded evidence: {:?}",
        truncated.output
    );
...
#[cfg(unix)]
#[tokio::test]
async fn shell_signal_and_timeout_have_typed_verdicts_without_changing_transport_outcome() {
    let workspace = tempfile::tempdir().expect("workspace");
    let signaled = execute(
        workspace.path(),
        json!({"command":"kill -TERM $$","description":"typed signal"}),
    )
    .await;
    assert_eq!(signaled.outcome, ToolOutcome::Succeeded);
    assert!(signaled.output.starts_with(
        "execution_evidence_v1 command=signal:15 timeout=false \
stdout_trunc=false stderr_trunc=false effect=unobserved\n"
    ));
    ...
    let timed_out = execute(
        ...
        json!({
            "command":"sleep 30",
            "description":"typed timeout",
            "timeout":100
        }),
    )
    .await;
    assert_eq!(timed_out.outcome, ToolOutcome::TimedOut);
    assert!(timed_out.output.starts_with(
...
fn rejects_background_and_invalid_but_ignores_unknown_schema_fields() {
    ...
    assert_eq!(
        executor
            .validate(&over_frozen_max)
            .expect_err("frozen 300000ms maximum")
            .output,
        "invalid_arguments"
    );
}
...
async fn mocked_xai_completed_response_dispatches_foreground_shell_once() {
    ...
    let events = fs::read_to_string(artifacts.join("events.jsonl")).expect("events");
    assert_eq!(events.matches("\"type\":\"tool.dispatched\"").count(), 1);
    let completed_tool = events
        .lines()
        .map(|line| serde_json::from_str::<Value>(line).expect("event JSON"))
        .find(|event| event["type"] == "tool.completed")
        .expect("tool completion event");
    assert!(
        completed_tool["data"]["output"]
            .as_str()
            .expect("tool output")
            .starts_with("execution_evidence_v1 command=exit:0")
    );
