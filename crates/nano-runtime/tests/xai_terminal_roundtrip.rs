...
#[tokio::test]
async fn semantic_checkpoint_v7_uses_one_bounded_late_review_when_fresh_critic_does_not_fit() {
    let root = tempfile::tempdir().expect("test root");
    let contract = write_contract(root.path());
    let artifacts = root.path().join("artifacts");
    let deadline = phase_deadline(Instant::now() + Duration::from_secs(270));
    let mut provider = PhaseProvider {
        responses: VecDeque::from([
            phase_message_response("response-candidate", "candidate"),
            phase_tool_response(),
            phase_message_response("response-final", "late-reviewed"),
        ]),
        requests: Vec::new(),
        action_return_at: None,
    };
    ...
    .await
    .expect("bounded late review run");

    assert_eq!(provider.requests.len(), 3);
    assert_eq!(provider.requests[0].mode, ProviderRequestMode::ActionOpen);
    assert_eq!(provider.requests[1].mode, ProviderRequestMode::ActionOpen);
    assert!(provider.requests[1].history.iter().any(|item| matches!(
        item,
        HistoryItem::User { content } if content.contains("late_acceptance_review_v1")
    )));
    assert!(!provider.requests[1].history.iter().any(|item| matches!(
        item,
        HistoryItem::User { content } if content.contains("fresh_completion_critic_v3")
    )));
    assert_eq!(provider.requests[2].mode, ProviderRequestMode::FinalOnly);
    assert!(provider.requests[2].tools.is_empty());
    assert!(provider.requests[2].history.iter().any(|item| matches!(
        item,
        HistoryItem::User { content } if content.contains("late_acceptance_review_final_v1")
    )));
    assert_eq!(executor.executions, 1);
    assert_eq!(outcome.record.provider_turn_count, 3);
    assert_eq!(outcome.record.tool_call_count, 1);
    let events = fs::read_to_string(artifacts.join("events.jsonl")).expect("events");
    assert!(events.contains("\"completion_review_policy\":\"semantic-checkpoint-v7\""));
    assert_eq!(
        events
            .matches("\"phase\":\"completion_late_review\"")
            .count(),
        2
    );
    assert!(events.contains("\"text\":\"late-reviewed\""));
    assert_eq!(events.matches("\"type\":\"assistant.final\"").count(), 1);
}

#[tokio::test]
async fn semantic_checkpoint_v7_late_review_provider_failure_preserves_unmutated_candidate() {
    let root = tempfile::tempdir().expect("test root");
    let contract = write_contract(root.path());
    let artifacts = root.path().join("artifacts");
    let deadline = phase_deadline(Instant::now() + Duration::from_secs(270));
    let mut provider = OneShotFailureProvider {
        responses: VecDeque::from([phase_message_response("response-candidate", "candidate")]),
        requests: Vec::new(),
        failure_at_request: 1,
        failure_code: "late_review_transport_failed",
        failure_emitted: false,
    };
    let mut executor = ImmediateExecutor::default();
    let spec = phase_spec(&contract, &artifacts);
    ...
    .await
    .expect("unchanged candidate is the safe late-review fallback");
    ...
    assert_eq!(outcome.record.tool_call_count, 0);
    let events = fs::read_to_string(artifacts.join("events.jsonl")).expect("events");
    assert!(events.contains("\"phase\":\"completion_late_review\""));
    assert!(events.contains("\"code\":\"late_review_transport_failed\""));
    assert!(events.contains("\"text\":\"candidate\""));
}
