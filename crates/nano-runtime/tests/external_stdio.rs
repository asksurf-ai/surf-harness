        ...
        task: TaskSpec {
            ...
            git_history_capability: GitHistoryCapability {
                ...
                canonical_instruction_sha256:
                    "20c408b474e069ab773f9634a147a61a7d8fd0feb3a20658fa915323a1d04ae4".to_owned(),
...
async fn foreground_owned_processes_terminated_projects_term_and_kill_evidence() {
    for (call_id, kill_sent, return_code) in [
        ...
        let mut evidence_profile = profile();
        evidence_profile.tools.model_tool_output_bytes_per_call = 1_024;
        let result = execute_foreground_v3_case_with_profile(
            call(call_id, "printf residual"),
            evidence_profile,
            move |value| {
        ...
        assert_eq!(
            result.output,
            format!(
                "execution_evidence_v1 command=exit:{return_code} timeout=false \
                 stdout_trunc=false stderr_trunc=false effect=unobserved\n\
...
async fn foreground_owned_processes_terminated_keeps_prefix_inside_output_cap() {
    ...
    tight_profile.tools.model_tool_output_bytes_per_call = 256;
    let result = execute_foreground_v3_case_with_profile(
    ...
    assert!(result.output.starts_with("execution_evidence_v1"));
    assert!(result.output.contains(FOREGROUND_RESIDUAL_SENTINEL));
    assert!(result.output.contains("output truncated"));
...
async fn foreground_owned_processes_terminated_is_live_v3_only() {
    ...
    assert!(live.output.starts_with("execution_evidence_v1"));
    assert!(live.output.contains(FOREGROUND_RESIDUAL_SENTINEL));
}
...
async fn foreground_owned_processes_terminated_isolated_from_ordinary_and_timeout_results() {
    ...
    assert_eq!(
        ordinary.output,
        concat!(
            "execution_evidence_v1 command=exit:9 timeout=false ",
            "stdout_trunc=false stderr_trunc=false effect=unobserved\n",
            "exit: 9\nordinary"
        )
    );
    ...
    assert_eq!(
        timed_out.output,
        concat!(
            "execution_evidence_v1 command=timeout timeout=true ",
            "stdout_trunc=false stderr_trunc=false effect=unobserved\n",
            "exit: killed (timeout)\n"
        )
    );
...
#[tokio::test]
async fn v3_foreground_nonzero_is_transport_success_with_typed_missing_evidence() {
    let result = execute_foreground_v3_case(call("call-nonzero-evidence", "false"), |value| {
        value["result"]["return_code"] = json!(1);
        value["result"]["stderr_base64"] = json!(BASE64.encode(b"assertion failed"));
        value["result"]["effect_observation_v1"] = Value::Null;
    })
    .await
    .expect("a settled nonzero command is not a transport failure");

    assert_eq!(result.outcome, ToolOutcome::Succeeded);
    assert!(result.output.starts_with(
        "execution_evidence_v1 command=exit:1 timeout=false \
stdout_trunc=false stderr_trunc=false effect=unobserved\n"
    ));
    assert!(result.output.ends_with("exit: 1\nassertion failed"));
}

#[tokio::test]
async fn v3_effect_observation_is_closed_and_model_visible_without_guessing() {
    let changed = execute_foreground_v3_case(
        FunctionCall {
            call_id: "call-write-evidence".to_owned(),
            name: "write".to_owned(),
            arguments_json: r#"{"file_path":"result.txt","content":"done"}"#.to_owned(),
        },
        |value| {
            value["result"]["process_disposition"] = json!("no_process");
            value["result"]["cleanup"]["attempted"] = json!(false);
            value["result"]["actor_receipt"] = Value::Null;
            value["result"]["effect_observation_v1"] = json!({"status":"changed"});
            value["result"]["stdout_base64"] = json!(BASE64.encode(b"written"));
        },
    )
    .await
    .expect("closed observed effect");
    assert!(changed.output.starts_with(
        "execution_evidence_v1 command=not_applicable timeout=false \
stdout_trunc=false stderr_trunc=false effect=changed\n"
    ));
    assert!(changed.output.ends_with("written"));

    let error = execute_foreground_v3_case(
        FunctionCall {
            call_id: "call-write-unknown-effect".to_owned(),
            name: "write".to_owned(),
            arguments_json: r#"{"file_path":"result.txt","content":"done"}"#.to_owned(),
        },
        |value| {
            ...
            value["result"]["cleanup"]["attempted"] = json!(false);
            value["result"]["actor_receipt"] = Value::Null;
            value["result"]["effect_observation_v1"] = json!({
                "status":"changed",
                "guessed_from_tool_name":true
            });
        },
    )
    .await
    .expect_err("unknown effect fields must fail closed");
