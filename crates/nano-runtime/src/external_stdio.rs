...
use nano_types::external_tool::{
    ...
    ExternalMediaPayload, ExternalMediaType, ExternalProcessDisposition,
    ExternalTerminalActorOriginV1, ExternalTerminalActorReceiptV1, ExternalTerminalActorSubtypeV1,
    ExternalToolDeadlineFields, ExternalToolMessageType, ExternalToolProtocolError,
    ExternalToolRequest, ExternalToolRequestV3, ExternalToolResponse, ExternalToolResponseDocument,
    ExternalToolSettlement, ExternalToolSettlementV3, READ_FILE_MEDIA_MAX_BYTES,
};
...
use crate::tool::{
    ToolExecutionBudget, ToolExecutionError, ToolExecutionFailureClass, ToolExecutor, ToolMedia,
    ToolResult, ToolRuntimeBudget, WorkspaceMode,
};
    ...
    W: AsyncWrite + Unpin,
{
    ...
    async fn execute_foreground(
        ...
        if let ExternalStdioProtocol::LiveV3(envelope) = self.protocol.clone() {
            return self
                .execute_foreground_v3(call, envelope, budget.semantic_deadline())
                .await;
        }
        let operation =
        ...
        let requested_timeout = match operation {
            ForegroundOperation::Terminal {
                ...
                .min(Duration::from_millis(
                    self.profile.tools.terminal_max_timeout_ms,
                ))
                .min(command_window)
            }
            ForegroundOperation::Terminal {
        ...
        let response_bytes = match tokio::time::timeout_at(
            tokio::time::Instant::from_std(budget.settlement_deadline()),
    ...
    async fn execute_foreground_v3(
        ...
        let operation =
            validate_foreground_call(call, &self.profile.tools, self.background_enabled)
                .map_err(|_| incomplete("external_stdio_validated_call_changed"))?;
        let semantic_cutoff = semantic_deadline.min(envelope.actor_done());
        if Instant::now() >= semantic_cutoff {
        ...
        let operation_timeout_ms = if matches!(
            ...
            let remaining_action = semantic_cutoff.saturating_duration_since(Instant::now());
            let Some(timeout_ms) =
        ...
        let (response, media_payload, runtime_budget, background_start_observation, actor_receipt) =
            match settlement {
    ...
    W: AsyncWrite + Unpin,
{
    ...
    async fn execute(
        ...
        let budget = match &self.protocol {
            ExternalStdioProtocol::LiveV3(envelope) => {
                ToolExecutionBudget::new(envelope.actor_done(), envelope.tool_settled())
            }
            ExternalStdioProtocol::LegacyV2 => ToolExecutionBudget::uniform(deadline),
        };
        self.execute_foreground(call, workspace, budget).await
    }
    ...
    let timeout_ms = match operation {
        ForegroundOperation::Terminal {
            ...
            requested_timeout_ms
                ...
                .min(profile.tools.terminal_max_timeout_ms),
        ),
        ForegroundOperation::Terminal {
            background: true, ..
        } => profile
...
fn finish_response(
    ...
    if foreground_owned_processes_terminated(projection_mode, call, &response) {
        return Ok(ToolResult {
            ...
            output: render_foreground_owned_processes_terminated(
                stdout,
                stderr,
    ...
    if call.name == "run_terminal_command"
        && response.process_disposition == ExternalProcessDisposition::ForegroundCleaned
    {
        let output = render_external_result(
            stdout,
            stderr,
            response.return_code,
            response.timed_out,
    ...
    if matches!(
        ...
        return match response.return_code {
            0 => {
                let mut result = ToolResult::succeeded(truncate_utf8(&output, max_model_bytes));
                result.runtime_budget = runtime_budget;
                Ok(result)
            }
            2 => Ok(ToolResult {
                execution_attempted: true,
                outcome: ToolOutcome::Rejected,
                output: truncate_utf8(&output, max_model_bytes),
                media: None,
                runtime_budget,
                actor_receipt: None,
            }),
    ...
    let output = String::from_utf8(stdout)
        .map_err(|_| incomplete("external_stdio_response_output_not_utf8"))?;
    let output = truncate_utf8(&output, max_model_bytes);
