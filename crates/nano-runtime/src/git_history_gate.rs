...
/// Render the bounded, task-neutral observation for a whole-call history denial.
///
/// The caller must use this only after rejecting the complete request before
/// dispatch. The observation deliberately accepts no request data, so it
/// cannot echo a command or imply that an allowed subset ran.
pub fn git_history_rejection_observation() -> &'static str {
    "git_history_not_required: No part of this tool call was executed, and the workspace is unchanged. Omit Git history operations and issue allowed current-tree inspection (ls, cat, grep, or git status) as a separate tool call."
}
...
mod tests {
    ...
    use super::{
        GIT_HISTORY_NOT_REQUIRED, deny_git_history_request, git_history_rejection_observation,
        terminal_reads_history,
    };
    ...
    #[test]
    fn mixed_safe_and_history_commands_are_denied_as_one_call() {
        for command in [
            "ls -la && git log -1",
            "cat Cargo.toml; git show HEAD~1:Cargo.toml",
            "git reflog | grep HEAD",
            "git status --short || git cat-file -p HEAD~1",
        ] {
            assert!(
                terminal(command, GitHistoryAccess::NotRequired),
                "the complete mixed call must be denied: {command}"
            );
        }
    }

    #[test]
    fn rejection_observation_is_bounded_generic_and_actionable() {
        let observation = git_history_rejection_observation();

        assert!(observation.starts_with(GIT_HISTORY_NOT_REQUIRED));
        assert!(observation.contains("No part of this tool call was executed"));
        assert!(observation.contains("workspace is unchanged"));
        assert!(observation.contains("separate tool call"));
        for current_tree_tool in ["ls", "cat", "grep", "git status"] {
            assert!(observation.contains(current_tree_tool));
        }
        assert!(observation.len() <= 512);
    }

    #[test]
    fn rejection_observation_cannot_return_a_mixed_calls_safe_subset() {
        let mixed = "printf SAFE_SUBSET_CANARY > marker && git log -1 && ls";

        assert!(terminal(mixed, GitHistoryAccess::NotRequired));
        assert!(!git_history_rejection_observation().contains("SAFE_SUBSET_CANARY"));
    }
