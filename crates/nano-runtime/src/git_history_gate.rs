...
fn command_segment_reads_history(tokens: &[String]) -> bool {
    ...
    if creates_git_executable_alias(&tokens[index..]) {
        return true;
    }
    loop {
...
fn creates_git_executable_alias(tokens: &[String]) -> bool {
    let Some(program) = tokens.first().map(|value| basename(value)) else {
        return false;
    };
    if !matches!(program, "cp" | "install" | "ln") {
        return false;
    }
    tokens.iter().skip(1).any(|value| {
        !value.starts_with('-')
            && value.contains('/')
            && basename(value).eq_ignore_ascii_case("git")
    })
}
...
mod tests {
    ...
    fn denies_history_revision_object_and_bounded_wrapper_forms() {
        for command in [
            ...
            "cat .git/logs/HEAD",
            "cp /usr/bin/git /tmp/g && /tmp/g log --all",
            "install /usr/bin/git /tmp/history-reader",
            "ln -s /usr/bin/git /tmp/g; /tmp/g cat-file -p HEAD~1",
        ] {
            assert!(
