...
/// Typed settled facts rendered directly, never reconstructed from tool text.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub struct ExecutionEvidenceV1 {
    pub command: CommandVerdictV1,
    pub timed_out: bool,
...
impl ExecutionEvidenceV1 {
    pub fn render_before(self, body: &str, max_bytes: usize) -> String {
        let command = match self.command {
            CommandVerdictV1::NotApplicable => "not_applicable".to_owned(),
            CommandVerdictV1::Exit(code) => format!("exit:{code}"),
            CommandVerdictV1::Signal(signal) => format!("signal:{signal}"),
            CommandVerdictV1::Timeout => "timeout".to_owned(),
            CommandVerdictV1::Unknown => "unknown".to_owned(),
        };
        let effect = match self.effect {
            EffectObservationV1::Changed => "changed",
            EffectObservationV1::Unchanged => "unchanged",
            EffectObservationV1::NotApplicable => "not_applicable",
            EffectObservationV1::Unobserved => "unobserved",
        };
        let preamble = format!(
            "execution_evidence_v1 command={command} timeout={} \
             stdout_trunc={} stderr_trunc={} effect={effect}\n",
            self.timed_out, self.stdout_truncated, self.stderr_truncated,
        );
        if preamble.len() >= max_bytes {
            return truncate_utf8(&preamble, max_bytes);
        }
        let rendered_body = truncate_utf8(body, max_bytes - preamble.len());
        let mut rendered = String::with_capacity(preamble.len() + rendered_body.len());
...
mod tests {
    ...
    use super::{
        CommandVerdictV1, EchoExecutor, EffectObservationV1, ExecutionEvidenceV1, ToolExecutor,
        ToolResult,
    };
    ...
    #[test]
    fn execution_evidence_preamble_is_bounded_and_keeps_the_body_after_it() {
        let evidence = ExecutionEvidenceV1 {
            command: CommandVerdictV1::Exit(1),
            timed_out: false,
            stdout_truncated: false,
            stderr_truncated: true,
            effect: EffectObservationV1::Unobserved,
        };
        let rendered = evidence.render_before("exit: 1\nbody", 256);
        assert!(rendered.starts_with(
            "execution_evidence_v1 command=exit:1 timeout=false \
stdout_trunc=false stderr_trunc=true effect=unobserved\n"
        ));
        assert!(rendered.ends_with("exit: 1\nbody"));

        let bounded = evidence.render_before(&"🙂".repeat(100), 128);
        assert!(bounded.len() <= 128);
        assert!(bounded.starts_with("execution_evidence_v1"));
