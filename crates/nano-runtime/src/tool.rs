...
/// Separate useful-work admission from the immutable settlement envelope.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
...
impl ToolExecutionBudget {
    pub fn new(semantic_deadline: Instant, settlement_deadline: Instant) -> Self {
        Self {
            semantic_deadline,
            settlement_deadline,
        }
    ...
    pub fn uniform(deadline: Instant) -> Self {
        Self::new(deadline, deadline)
    }

    pub fn semantic_deadline(self) -> Instant {
        self.semantic_deadline
    }

    pub fn settlement_deadline(self) -> Instant {
...
pub trait ToolExecutor {
    ...
    /// Execute with a semantic cutoff that may be earlier than settlement.
    async fn execute_with_budget(
        ...
        self.execute(call, workspace, budget.semantic_deadline())
            .await
    }
}
...
mod tests {
    ...
    use super::{EchoExecutor, ToolExecutionBudget, ToolExecutor, ToolResult};

    #[test]
    fn execution_budget_keeps_semantic_and_settlement_deadlines_distinct() {
        let now = Instant::now();
        let budget =
            ToolExecutionBudget::new(now + Duration::from_secs(5), now + Duration::from_secs(20));
        assert_eq!(budget.semantic_deadline(), now + Duration::from_secs(5));
        assert_eq!(budget.settlement_deadline(), now + Duration::from_secs(20));
    }
