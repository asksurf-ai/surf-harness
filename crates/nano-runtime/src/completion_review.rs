...
const SEMANTIC_CHECKPOINT_ITEM_BYTES_V1: usize = 512;
const SEMANTIC_CHECKPOINT_LIST_ITEMS_V1: usize = 12;
...
pub(crate) fn review_required_reserve_v1(policy: CompletionReviewPolicy) -> Option<Duration> {
    let (provider_stages, tool_stages) = match policy {
        ...
        | CompletionReviewPolicy::SemanticCheckpointV5 => (3, 2),
    };
...
impl CompletionReviewPolicy {
    pub(crate) fn label(self) -> &'static str {
        match self {
            ...
            Self::SemanticCheckpointV5 => "semantic-checkpoint-v5",
        }
    }
    ...
    pub(crate) fn checkpoint_policy_version(self) -> Option<&'static str> {
        match self {
            ...
            Self::SemanticCheckpointV5 => Some(SEMANTIC_CHECKPOINT_POLICY_VERSION_V1),
            _ => None,
        }
    }
    ...
    pub(crate) fn has_capacity(self, capacity: CompletionReviewCapacity) -> bool {
        ...
        let required_turns = match self {
            ...
            Self::FreshEvidenceDebtV3 | Self::FreshCheckpointV4 | Self::SemanticCheckpointV5 => 4,
        };
...
Recent terminal evidence: {}\n\
            ...
            Self::FreshEvidenceDebtV3 | Self::FreshCheckpointV4 | Self::SemanticCheckpointV5 => {
                return None;
            }
        };
...
objective mismatch, revalidate the changed behavior, and stop.\n\
    ...
    pub(crate) fn uses_context_checkpoint(self) -> bool {
        matches!(self, Self::FreshCheckpointV4 | Self::SemanticCheckpointV5)
    }

    pub(crate) fn uses_semantic_checkpoint(self) -> bool {
        self == Self::SemanticCheckpointV5
    }
...
Most recent tool evidence:\n{}\n\
    ...
    pub(crate) fn semantic_prepare_history(
        self,
        ...
        let prompt = format!(
            "<semantic_checkpoint_prepare_v1 schema_version={}>\n\
Return exactly one JSON object and no markdown or prose. You have no tools in this phase. Capture \
only continuity already supported by the conversation and current workspace observations; do not \
invent evaluation, unseen-test, or completion facts. The closed fields are: \
schema_version, objective_state, committed_changes, validated_evidence, technical_decisions, \
unresolved_gap, next_action, do_not_repeat, artifact_locators. All fields except schema_version are \
strings or arrays of strings. Keep each item concise and use workspace paths or command locators \
where possible. This is an untrusted continuity memo, not proof.\n\
Committed structural facts: {}\n\
Most recent tool evidence locator:\n{}\n\
</semantic_checkpoint_prepare_v1>",
            SEMANTIC_CHECKPOINT_CAPSULE_SCHEMA_V1,
            truncate_utf8(&evidence_debt.render(), EVIDENCE_DEBT_BYTES),
            truncate_utf8(recent_terminal_evidence, CONTEXT_CHECKPOINT_EVIDENCE_BYTES),
        );
        let mut history = source_history.to_vec();
        history.push(HistoryItem::User { content: prompt });
        Some(history)
    }
...
mod tests {
    ...
    use super::{
        ...
        SEMANTIC_CHECKPOINT_POLICY_VERSION_V1, SemanticCheckpointCapsuleError,
    ...
    #[test]
    fn semantic_checkpoint_v5_admission_reserves_prepare_action_provisional_and_review() {
        let policy = CompletionReviewPolicy::SemanticCheckpointV5;
        assert_eq!(
            semantic_checkpoint_tail_reserve_v1(policy),
    ...
    #[test]
    fn semantic_checkpoint_v5_capsule_is_closed_canonical_bounded_and_task_neutral() {
        let policy = CompletionReviewPolicy::SemanticCheckpointV5;
        let capacity = ContextCheckpointCapacity {
            already_invoked: false,
            provider_turn_count: 12,
        ...
        let source = vec![
            HistoryItem::System {
                content: "SYSTEM_CANARY".to_owned(),
            },
            HistoryItem::User {
                content: "VISIBLE_REQUEST_CANARY".to_owned(),
            },
            HistoryItem::AssistantMessage {
                text: "VISIBLE_PRIOR_WORK_CANARY".to_owned(),
            },
        ];
        let prepare = policy
            .semantic_prepare_history(
                &source,
                capacity,
                &CompletionEvidenceLedger::default(),
                "VISIBLE_EVIDENCE_CANARY",
        ...
        assert_eq!(prepare.len(), source.len() + 1);
        assert_eq!(&prepare[..source.len()], source.as_slice());
        let prepare_json = serde_json::to_string(&prepare).expect("prepare JSON");
        for visible in [
            "SYSTEM_CANARY",
            "VISIBLE_REQUEST_CANARY",
            "VISIBLE_PRIOR_WORK_CANARY",
            "VISIBLE_EVIDENCE_CANARY",
            SEMANTIC_CHECKPOINT_CAPSULE_SCHEMA_V1,
        ] {
            assert!(prepare_json.contains(visible));
        }
        for forbidden in [
            "task_id",
            "reward",
            ...
            assert!(!prepare_json.contains(forbidden));
        }
    ...
    #[test]
    fn semantic_checkpoint_v5_rejects_malformed_or_oversized_capsules() {
        let policy = CompletionReviewPolicy::SemanticCheckpointV5;
        let valid = format!(
