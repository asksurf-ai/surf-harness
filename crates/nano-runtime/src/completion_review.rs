...
const LATE_CANDIDATE_EXCERPT_BYTES_V1: usize = 384;
...
const LATE_ACCEPTANCE_REVIEW_PROVIDER_RESERVE_V1: Duration = Duration::from_secs(90);
...
const FRESH_CRITIC_SYSTEM_PROMPT_V3: &str = "\
...
const FRESH_CRITIC_SYSTEM_PROMPT_V4: &str = "\
You are an isolated completion critic. You have no tools and no access to the actor's hidden \
reasoning. Assess only the supplied request, candidate, and bounded execution evidence. Identify \
one acceptance claim with the weakest independent evidence. Prefer a claim that only passed a \
familiar input, depends on validation-set selection, or sits close to a quantitative threshold. \
For threshold or comparative claims, one self-selected sample, split, or warm measurement is weak \
evidence: propose a representative independent measurement that exposes the safety margin. For \
behavior intended to generalize across inputs, prefer one bounded change of scale, duration, \
distribution, or boundary conditions over repeating the same happy path. Do not seek unavailable \
evaluation data, solve the task, propose broad improvements, repeat a checklist, or invent facts. \
Return only a concise claim, reason, and one high-signal falsification check; say that existing \
evidence is sufficient when it is.";
...
pub(crate) const LATE_ACCEPTANCE_REVIEW_FINAL_PROMPT_V1: &str = "<late_acceptance_review_final_v1>\n\
The single bounded late-review tool batch is settled. Do not call tools again. Report the final \
state concisely and do not claim evidence that was not observed.\n\
</late_acceptance_review_final_v1>";
...
pub enum CompletionReviewPolicy {
    ...
    SemanticCheckpointV7,
    SemanticCheckpointV8,
}
...
#[derive(Clone, Copy, Debug)]
pub(crate) struct LateAcceptanceReviewCapacity {
    pub now: Instant,
    pub actor_done: Instant,
    pub last_send: Instant,
    pub provisional_final_nonempty: bool,
    pub review_already_invoked: bool,
    pub provider_turn_count: u64,
    pub max_provider_turns: u64,
...
pub(crate) fn late_acceptance_review_required_reserve_v1() -> Duration {
    LATE_ACCEPTANCE_REVIEW_PROVIDER_RESERVE_V1
        .saturating_add(REVIEW_BOUNDED_TOOL_RESERVE_V1)
        .saturating_add(FINAL_RESPONSE_LATENCY_RESERVE_V1)
}
...
pub(crate) fn review_required_reserve_v1(policy: CompletionReviewPolicy) -> Option<Duration> {
    let (provider_stages, tool_stages) = match policy {
        ...
        | CompletionReviewPolicy::SemanticCheckpointV8 => (3, 2),
    };
...
impl CompletionReviewPolicy {
    pub(crate) fn label(self) -> &'static str {
        match self {
            ...
            Self::SemanticCheckpointV7 => "semantic-checkpoint-v7",
            Self::SemanticCheckpointV8 => "semantic-checkpoint-v8",
        }
    }
    ...
    pub(crate) fn checkpoint_policy_version(self) -> Option<&'static str> {
        match self {
            ...
            Self::SemanticCheckpointV6
            | Self::SemanticCheckpointV7
            | Self::SemanticCheckpointV8 => Some(SEMANTIC_CHECKPOINT_POLICY_VERSION_V1),
            _ => None,
        }
    }
    ...
    pub(crate) fn has_capacity(self, capacity: CompletionReviewCapacity) -> bool {
        ...
        let required_turns = match self {
            ...
            Self::FreshEvidenceDebtV3
            | Self::FreshCheckpointV4
            | Self::SemanticCheckpointV6
            | Self::SemanticCheckpointV7
            | Self::SemanticCheckpointV8 => 4,
        };
...
Recent terminal evidence: {}\n\
            ...
            | Self::SemanticCheckpointV6
            | Self::SemanticCheckpointV7
            | Self::SemanticCheckpointV8 => return None,
        };
    ...
    pub(crate) fn fresh_context(
        ...
        let (version, system_prompt) = if self == Self::SemanticCheckpointV8 {
            ("v4", FRESH_CRITIC_SYSTEM_PROMPT_V4)
        } else {
            ("v3", FRESH_CRITIC_SYSTEM_PROMPT_V3)
        };
        let request = format!(
            "<fresh_completion_critic_{version}>\n\
Original request:\n{}\n\
...
Most recent tool evidence:\n{}\n\
    ...
    pub(crate) fn actor_prompt(self, critic_advice: &str) -> Option<String> {
        ...
        let version = if self == Self::SemanticCheckpointV8 {
            "v4"
        } else {
            "v3"
        };
        let risk_margin_instruction = if self == Self::SemanticCheckpointV8 {
            " If the gap concerns a threshold, sample selection, or cross-input behavior, the \
check must expose representative safety margin or a bounded changed-input counterexample rather \
than repeat the same chosen happy path."
        } else {
            ""
        };
        Some(format!(
            "<fresh_completion_critic_advice_{version}>\n\
...
objective mismatch, revalidate the changed behavior, and stop.\n\
            ...
            risk_margin_instruction,
        ))
    }

    pub(crate) fn has_late_acceptance_review_capacity(
        self,
        capacity: LateAcceptanceReviewCapacity,
    ) -> bool {
        if !matches!(
            self,
            Self::SemanticCheckpointV7 | Self::SemanticCheckpointV8
        ) || !capacity.provisional_final_nonempty
            || capacity.review_already_invoked
            || capacity.max_function_calls_per_response == 0
        {
            return false;
        }
        let enough_turns = capacity
        ...
        let enough_history = capacity
            .max_history_items
            .checked_sub(capacity.history_items)
            .is_some_and(|remaining| remaining >= 1);
        let enough_tool_calls = capacity
            .max_function_calls_per_run
            .checked_sub(capacity.function_call_count)
            .is_some_and(|remaining| remaining >= 1);
        let action_reserve = LATE_ACCEPTANCE_REVIEW_PROVIDER_RESERVE_V1
            .saturating_add(REVIEW_BOUNDED_TOOL_RESERVE_V1);
        let enough_action_time = capacity
            .actor_done
            .checked_duration_since(capacity.now)
            .is_some_and(|remaining| remaining > action_reserve);
        let enough_total_time = capacity
            .last_send
            .checked_duration_since(capacity.now)
            .is_some_and(|remaining| remaining > late_acceptance_review_required_reserve_v1());
        enough_turns
            && enough_history
            && enough_tool_calls
            && enough_action_time
    ...
    pub(crate) fn late_acceptance_review_prompt(
        self,
        candidate_final: &str,
        recent_terminal_evidence: &str,
        evidence_debt: &CompletionEvidenceLedger,
        ...
        let prompt = format!(
            "<late_acceptance_review_v1>\n\
The full isolated critic no longer fits, but one bounded same-context review does. Reassess one \
acceptance claim with the weakest independent evidence. Prefer exact boundaries, numerical \
tolerance, representative repeated performance, ordering, format, or end-to-end behavior over a \
familiar happy path. Do not seek unavailable evaluation evidence or preloaded history. Existing \
self-generated output is not an independent oracle. If committed evidence is sufficient, return \
the final now without tools. Otherwise emit at most one self-contained tool batch. The batch must \
either be read-only, or preserve the last known good artifact while it atomically selects and \
validates a complete replacement; there is no later exploration or correction stage. For \
comparative or threshold claims, avoid choosing from one warm or unrepresentative measurement.\n\
Candidate completion excerpt: {}\n\
Run facts: {}\n\
Recent terminal evidence: {}\n\
</late_acceptance_review_v1>",
            truncate_utf8(candidate_final, LATE_CANDIDATE_EXCERPT_BYTES_V1),
            truncate_utf8(&evidence_debt.render(), EVIDENCE_DEBT_BYTES),
            truncate_utf8(recent_terminal_evidence, FRESH_TERMINAL_EXCERPT_BYTES),
        );
        debug_assert!(prompt.len() <= MAX_PROMPT_BYTES);
    ...
    pub(crate) fn uses_semantic_checkpoint(self) -> bool {
        matches!(
            self,
            Self::SemanticCheckpointV6 | Self::SemanticCheckpointV7 | Self::SemanticCheckpointV8
        )
    }
...
mod tests {
    ...
    use super::{
        ...
        LATE_ACCEPTANCE_REVIEW_FINAL_PROMPT_V1, LateAcceptanceReviewCapacity, MAX_PROMPT_BYTES,
        ...
        late_acceptance_review_required_reserve_v1, recent_terminal_evidence,
        review_required_reserve_v1, semantic_checkpoint_action_turn_cutoff_v1,
        semantic_checkpoint_control_schema_v1, semantic_checkpoint_tail_reserve_v1,
        semantic_checkpoint_total_admission_reserve_v1,
    };
    ...
    #[test]
    fn semantic_checkpoint_v7_late_review_has_closed_budget_and_generic_prompt() {
        let policy = CompletionReviewPolicy::SemanticCheckpointV7;
        let now = Instant::now();
        let required = late_acceptance_review_required_reserve_v1();
        assert_eq!(required, Duration::from_secs(300));
        let at_cutoff = LateAcceptanceReviewCapacity {
            now,
            actor_done: now + Duration::from_secs(210),
            last_send: now + required,
            provisional_final_nonempty: true,
            review_already_invoked: false,
            provider_turn_count: 10,
            max_provider_turns: 12,
            history_items: 31,
            max_history_items: 32,
            function_call_count: 51,
            max_function_calls_per_response: 8,
            max_function_calls_per_run: 52,
        };
        assert!(!policy.has_late_acceptance_review_capacity(at_cutoff));
        let admitted = LateAcceptanceReviewCapacity {
            actor_done: now + Duration::from_secs(211),
            last_send: now + required + Duration::from_nanos(1),
            ..at_cutoff
        };
        assert!(policy.has_late_acceptance_review_capacity(admitted));
        assert!(
            !CompletionReviewPolicy::SemanticCheckpointV6
                .has_late_acceptance_review_capacity(admitted)
        );
        assert!(
            !policy.has_late_acceptance_review_capacity(LateAcceptanceReviewCapacity {
                max_provider_turns: 11,
                ..admitted
            })
        );
        assert!(
            !policy.has_late_acceptance_review_capacity(LateAcceptanceReviewCapacity {
                function_call_count: 52,
                ..admitted
            })
        );
        ...
        let prompt = policy
            .late_acceptance_review_prompt(
                ...
                "VISIBLE_TERMINAL_EVIDENCE",
                &CompletionEvidenceLedger::default(),
            )
            .expect("v7 late prompt");
        assert!(prompt.len() <= MAX_PROMPT_BYTES);
        assert!(prompt.contains("late_acceptance_review_v1"));
        assert!(prompt.contains("representative repeated performance"));
        assert!(prompt.contains("one bounded same-context review"));
        assert!(prompt.contains("CANDIDATE_COMPLETION"));
        assert!(prompt.contains("VISIBLE_TERMINAL_EVIDENCE"));
        for forbidden in ["task_id", "reward=", "historical answer", "terminal-bench"] {
            assert!(!prompt.contains(forbidden));
        }
        assert!(LATE_ACCEPTANCE_REVIEW_FINAL_PROMPT_V1.contains("Do not call tools again"));
    }
    ...
    #[test]
    fn semantic_v8_fresh_critic_prioritizes_margin_and_changed_input_risk() {
        let policy = CompletionReviewPolicy::SemanticCheckpointV8;
        let history = policy
            .fresh_context(
                "Produce a robust result.",
                "The result is ready.",
                "One local check passed.",
                &CompletionEvidenceLedger::default(),
            )
            .expect("v8 fresh critic context");
        let HistoryItem::System { content: system } = &history[0] else {
            panic!("isolated system prompt");
        };
        let HistoryItem::User { content: user } = &history[1] else {
        ...
        assert!(system.contains("quantitative threshold"));
        assert!(system.contains("validation-set selection"));
        assert!(system.contains("representative independent measurement"));
        assert!(system.contains("safety margin"));
        assert!(system.contains("scale, duration"));
        assert!(user.contains("fresh_completion_critic_v4"));

        let advice = policy
            .actor_prompt("The observed margin is too narrow.")
            .unwrap();
        assert!(advice.contains("fresh_completion_critic_advice_v4"));
        assert!(advice.contains("representative safety margin"));
        for forbidden in [
            "task_id",
            "benchmark",
            "reward",
            ...
            assert!(!system.contains(forbidden));
            assert!(!advice.contains(forbidden));
        }
    }
