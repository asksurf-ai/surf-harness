...
const EVIDENCE_DEBT_REQUEST_BYTES: usize = 4096;
...
pub(crate) fn review_required_reserve_v1(policy: CompletionReviewPolicy) -> Option<Duration> {
    let provider_stages = match policy {
        CompletionReviewPolicy::Disabled => return None,
        CompletionReviewPolicy::IndependentFalsificationV1
        | CompletionReviewPolicy::EvidenceDebtV2 => 2,
        CompletionReviewPolicy::FreshEvidenceDebtV3 => 3,
    };
    FINAL_RESPONSE_LATENCY_RESERVE_V1
        .checked_mul(provider_stages)
        .and_then(|provider| provider.checked_add(REVIEW_BOUNDED_TOOL_RESERVE_V1))
...
impl CompletionReviewPolicy {
    pub(crate) fn has_capacity(self, capacity: CompletionReviewCapacity) -> bool {
        ...
        let enough_history = u64::try_from(capacity.history_items)
            .ok()
            .is_some_and(|items| items < capacity.max_history_items);
    ...
    pub(crate) fn prompt(
        ...
        if matches!(self, Self::Disabled | Self::FreshEvidenceDebtV3) {
            return None;
        }
...
checklist.\n\
Original request (bounded exact replay): {}\n\
Run facts: {}\n\
Recent terminal evidence: {}\n\
</evidence_debt_review_v2>",
                truncate_utf8(original_request, EVIDENCE_DEBT_REQUEST_BYTES),
                truncate_utf8(&evidence_debt.render(), EVIDENCE_DEBT_BYTES),
                truncate_utf8(recent_terminal_evidence, TERMINAL_EXCERPT_BYTES),
            ),
            Self::FreshEvidenceDebtV3 => return None,
        };
    ...
    pub(crate) fn fresh_context(
        ...
        if self != Self::FreshEvidenceDebtV3 {
            return None;
        }
        let request = format!(
...
mod tests {
    ...
    use super::{
        ...
        CompletionReviewPolicy, FINAL_RESPONSE_LATENCY_RESERVE_V1, MAX_PROMPT_BYTES,
    ...
    #[test]
    fn fresh_critic_requires_three_turns_and_full_reserve() {
    ...
    fn evidence_debt_v2_is_bounded_exact_and_has_no_runtime_injected_task_identity() {
        ...
        let original = format!("ORIGINAL-{}-REQUEST_TAIL", "x".repeat(3000));
        ...
        assert!(prompt.contains("Original request (bounded exact replay)"));
        assert!(prompt.contains("ORIGINAL-"));
        assert!(prompt.contains("REQUEST_TAIL"));
