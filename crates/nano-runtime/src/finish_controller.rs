...
impl FinishController {
    ...
    /// Admit a host-frozen task-neutral checkpoint lease as an earlier soft
    /// finish boundary. The lease may only advance the state machine.
    pub fn observe_checkpoint_lease(&mut self, expired: bool) -> bool {
        if !expired || self.state != FinishState::ActionOpen {
            return false;
        }
...
mod tests {
    ...
    #[test]
    fn checkpoint_lease_only_advances_action_open_once() {
        let now = Instant::now();
        let mut controller = FinishController::new();
        assert!(!controller.observe_checkpoint_lease(false));
        assert_eq!(controller.state(), FinishState::ActionOpen);
        assert!(controller.observe_checkpoint_lease(true));
        assert_eq!(controller.state(), FinishState::SoftFinish);
        assert!(!controller.observe_checkpoint_lease(true));
        ...
        let facts = FinishNoticeFacts {
            ...
            remaining: Duration::from_secs(899),
        };
        assert!(controller.take_notice(facts).is_some());
        assert!(controller.close_actions());
        assert_eq!(controller.state(), FinishState::FinalOnly);
        assert!(!controller.observe_checkpoint_lease(true));
        assert_eq!(controller.state(), FinishState::FinalOnly);
