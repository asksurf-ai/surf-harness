...
use crate::git_history_gate::{GIT_HISTORY_NOT_REQUIRED, deny_git_history_request};
    ...
    loop {
        ...
        let provider_result = tokio::select! {
            ...
            () = cancellation.cancelled() => {
                return terminalize_provider_without_response(
                    &mut state,
                    turn_index,
...
async fn settle_tool_call<T: ToolExecutor>(
    ...
    if deny_git_history_request(
        ...
        return prepare_tool_completion(
            state,
            call,
            ToolResult::rejected(GIT_HISTORY_NOT_REQUIRED),
            window.runtime_deadline,
            window.max_provider_turns,
            window.output_limits,
            window.projected_tool_output_bytes,
...
fn terminalize_provider_without_response(
    state: &mut RunState<'_>,
    turn_index: u64,
    request_terminal_code: &str,
    run_terminal: TerminalOutcome,
) -> Result<AgentRunOutcome, AgentRunError> {
    state.provider_failed(None);
    if let Err(error) = state.append_operational(EventBody::ProviderFailed(ProviderFailed {
        turn_index,
        code: request_terminal_code.to_owned(),
    ...
    state.finalize_once(run_terminal)
}
    ...
    async fn git_history_denial_is_registered_recoverable_and_continues_to_current_state_tool() {
        ...
        let calls = [
            FunctionCall {
                ...
                name: "run_terminal_command".to_owned(),
                arguments_json: serde_json::json!({"command": "git show HEAD~1:src/app.c"})
                    .to_string(),
            },
            FunctionCall {
        ...
        assert_eq!(
            staging.completions[0].event.output,
        ...
        assert!(!events.contains("HEAD~1:src/app.c\n"));
    }
