...
use crate::completion_review::{
    ...
    SEMANTIC_CHECKPOINT_CAPSULE_SCHEMA_V1, SEMANTIC_CHECKPOINT_POLICY_VERSION_V1,
    ValidatedSemanticCheckpointCapsuleV1, recent_terminal_evidence, review_required_reserve_v1,
    semantic_checkpoint_action_turn_cutoff_v1, semantic_checkpoint_tail_reserve_v1,
};
    ...
    loop {
            ...
            && completion_challenge.phase == CompletionReviewPhase::NotIssued
        {
                ...
                && completion_review_policy.uses_semantic_checkpoint()
            {
                ...
                let Some(mut prepare_history) = completion_review_policy.semantic_prepare_history(
                    &history,
                    checkpoint_capacity,
                    &state.completion_evidence,
                    &terminal_evidence,
                ...
                let prepare_receipt = match apply_media_history_policy(&mut prepare_history) {
                    Ok(receipt) => receipt,
                    Err(_) => {
                        return state.finalize_once(TerminalOutcome::failure(
                            TerminalStatus::RuntimeFailure,
                            TerminalPhase::Runtime,
                            "semantic_checkpoint_prepare_history_failed",
                        ));
                    }
                };
                let Some(deadline) = deadline else {
                    return state.finalize_once(TerminalOutcome::failure(
                        TerminalStatus::RuntimeFailure,
                        TerminalPhase::Runtime,
                        "semantic_checkpoint_deadline_missing",
                    ));
                };
                let prepared = run_semantic_checkpoint_prepare(
                    &mut state,
                    provider,
                    SemanticCheckpointPrepareRequest {
                        spec,
                        deadline,
                        cancellation,
                        history: prepare_history,
                        media_history_receipt: prepare_receipt,
                        source_history_sha256: source_history_sha256.clone(),
                        policy: completion_review_policy,
                        max_arguments_bytes: profile.transport.max_function_arguments_bytes,
                    },
                )
                .await?;
                let (capsule, prepare_turn_index, prepare_history_sha256) = match prepared {
                    SemanticCheckpointPrepareResolution::Accepted {
                        capsule,
                        prepare_turn_index,
                        prepare_history_sha256,
                    } => (capsule, prepare_turn_index, prepare_history_sha256),
                    SemanticCheckpointPrepareResolution::Rejected => {
                        context_checkpoint_invoked = true;
                        continue;
                    }
                    SemanticCheckpointPrepareResolution::Terminal(outcome) => return Ok(*outcome),
                };
                ...
                let tail_reserve = semantic_checkpoint_tail_reserve_v1(completion_review_policy)
                ...
                let action_turn_cutoff =
                    semantic_checkpoint_action_turn_cutoff_v1(spec.provider.max_turns);
                let action_lease_ms = soft_cutoff
                    .and_then(|cutoff| cutoff.checked_duration_since(Instant::now()))
                    .map(|remaining| u64::try_from(remaining.as_millis()).unwrap_or(u64::MAX));
                if !completion_review_policy
                    .should_accept_semantic_checkpoint_after_prepare(post_prepare_capacity)
                ...
                let Some(mut checkpoint_history) = completion_review_policy
                ...
                let checkpoint_receipt = match apply_media_history_policy(&mut checkpoint_history) {
                    ...
                    Err(_) => {
                        return state.finalize_once(TerminalOutcome::failure(
                            TerminalStatus::RuntimeFailure,
                            TerminalPhase::Runtime,
                            "semantic_checkpoint_history_failed",
                        ));
                    }
                };
                state.append_operational(EventBody::ContextCheckpointed(
                    ContextCheckpointedV1 {
                        ...
                        provider_turn_count: state.provider_turn_count,
                        tool_call_count: state.tool_call_count,
                        observed_input_tokens: post_prepare_capacity.observed_input_tokens,
                        capsule_schema_version: Some(
                            SEMANTIC_CHECKPOINT_CAPSULE_SCHEMA_V1.to_owned(),
                        ),
                        capsule_sha256: Some(capsule.sha256().to_owned()),
                        capsule_bytes: Some(capsule.bytes()),
                        prepare_turn_index: Some(prepare_turn_index),
                        prepare_history_sha256: Some(prepare_history_sha256),
                        action_turn_cutoff: Some(action_turn_cutoff),
                        action_lease_ms,
                        tail_reserve_ms: Some(
                            u64::try_from(tail_reserve.as_millis()).unwrap_or(u64::MAX),
                        ),
                    },
                ))?;
                ...
                semantic_checkpoint_soft_cutoff = soft_cutoff;
                semantic_checkpoint_action_turn_cutoff = Some(action_turn_cutoff);
...
enum SemanticCheckpointPrepareResolution {
    Accepted {
        capsule: ValidatedSemanticCheckpointCapsuleV1,
        prepare_turn_index: u64,
        prepare_history_sha256: String,
    },
    Rejected,
    Terminal(Box<AgentRunOutcome>),
}

struct SemanticCheckpointPrepareRequest<'a> {
    spec: &'a RunSpec,
    deadline: &'a DeadlineContext,
...
fn append_semantic_checkpoint_rejection(
    ...
    state.append_operational(EventBody::ContextCheckpointRejected(
        ContextCheckpointRejectedV1 {
            policy_version: SEMANTIC_CHECKPOINT_POLICY_VERSION_V1.to_owned(),
            source_history_sha256: source_history_sha256.to_owned(),
            prepare_history_sha256: prepare_history_sha256.to_owned(),
            prepare_turn_index,
            provider_turn_count: state.provider_turn_count,
...
async fn run_semantic_checkpoint_prepare<P: Provider>(
    state: &mut RunState<'_>,
    provider: &mut P,
    request: SemanticCheckpointPrepareRequest<'_>,
) -> Result<SemanticCheckpointPrepareResolution, AgentRunError> {
    let SemanticCheckpointPrepareRequest {
        spec,
        deadline,
        cancellation,
        history,
    ...
    let prepare_turn_index = state.provider_turn_count;
    let prepare_history_sha256 = media_history_receipt.history_sha256().to_owned();
    let pending = match prepare_pending_provider_request(
        provider,
        spec,
        &[],
        ...
        ProviderRequestEvidence {
            ...
            budget_observation: provider_budget_observation(
                Some(deadline),
                ProviderBudgetPhaseV1::CheckpointPrepare,
                false,
            ),
            checkpoint_source_history_sha256: Some(&source_history_sha256),
        },
    ) {
        Ok(pending) => pending,
        Err(failure) => {
            ...
            return Ok(SemanticCheckpointPrepareResolution::Rejected);
        }
    };
    state.append_operational(EventBody::ProviderRequested(pending.projection))?;
    state.provider_requested();
    let provider_result = tokio::select! {
        biased;
        () = cancellation.cancelled() => {
            let outcome = state.finalize_once(TerminalOutcome::failure(
                TerminalStatus::Cancelled,
                TerminalPhase::Cancellation,
                "cooperative_cancelled",
            ))?;
            return Ok(SemanticCheckpointPrepareResolution::Terminal(Box::new(outcome)));
        }
        result = tokio::time::timeout_at(
    ...
    let completed = match provider_result {
        ...
        Err(_) => {
            let code = "provider_semantic_checkpoint_deadline_exceeded";
            state.provider_failed(None);
            append_provider_failure_event(
                state,
                prepare_turn_index,
    ...
    {
        let rejected = ProviderRejectedResponse {
            call_count: (failure.code() == "provider_call_limit_exceeded")
    ...
    let capsule_text = completed.final_text().unwrap_or_default().to_owned();
    let capsule = policy.parse_semantic_capsule(&capsule_text);
    state.provider_completed(completed.usage.as_ref());
    state.append_operational(EventBody::ProviderCompleted(ProviderCompleted {
        turn_index: prepare_turn_index,
        response_id: completed.response_id,
        model: completed.model,
        call_ids: Vec::new(),
        has_final_text: !capsule_text.is_empty(),
    ...
    match capsule {
        Ok(capsule) => Ok(SemanticCheckpointPrepareResolution::Accepted {
            capsule,
            prepare_turn_index,
            prepare_history_sha256,
        }),
        Err(error) => {
            ...
            Ok(SemanticCheckpointPrepareResolution::Rejected)
        }
    }
}
