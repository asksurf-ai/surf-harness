...
use nano_types::event::{
    AssistantFinal, ContextCheckpointedV1, EventBody, MediaHistoryRequestReceiptV1,
    ProviderBudgetObservationV1, ProviderBudgetPhaseV1, ProviderCallCoverage, ProviderCompleted,
    ProviderFailed, ProviderRequested, RUN_RECORD_SCHEMA, RunCompleted, RunFailed, RunRecord,
    RunRecordV3, RunStarted, TOOL_RECEIPT_V1_SCHEMA, TerminalPhase, TerminalStatus,
    ToolBudgetObservationV1, ToolCompleted as ToolCompletedEvent, ToolDispatched, ToolFailed,
    ToolFailureRecoverability, ToolReceiptV1, ToolRegistered, UsageState, UsageTotals,
    VersionedRunRecord, tool_receipt_identity_sha256,
};
...
async fn run_agent_bound_with_cancellation<P: Provider, T: ToolExecutor>(
    ...
    loop {
        ...
        if phase_split_enabled {
            ...
            if phase == ProviderPhase::FinalOnly {
            ...
            if let Some(text) = final_text.filter(|text| !text.is_empty()) {
                if phase_split_enabled
                    ...
                    && if completion_review_policy.uses_fresh_context() {
                        completion_review_policy.has_fresh_path_capacity(FreshCriticCapacity {
                            now: Instant::now(),
                            last_send,
        ...
        let (semantic_deadline, finish_admission_rejection) = match finish_controller
            .admit_action_batch(
                Instant::now(),
        ...
        phase = if completion_challenge.requires_final_only()
            || finish_controller.state() == FinishState::FinalOnly
        {
            ProviderPhase::FinalOnly
