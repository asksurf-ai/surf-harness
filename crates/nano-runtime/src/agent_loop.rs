...
use nano_types::event::{
    AssistantFinal, ContextCheckpointRejectedV1, ContextCheckpointedV1, EventBody,
    MediaHistoryRequestReceiptV1, ProviderBudgetObservationV1, ProviderBudgetPhaseV1,
    ProviderCallCoverage, ProviderCompleted, ProviderFailed, ProviderRequested, RUN_RECORD_SCHEMA,
    RunCompleted, RunFailed, RunRecord, RunRecordV3, RunStarted, TOOL_RECEIPT_V1_SCHEMA,
    TerminalPhase, TerminalStatus, ToolBudgetObservationV1, ToolCompleted as ToolCompletedEvent,
    ToolDispatched, ToolFailed, ToolFailureRecoverability, ToolReceiptV1, ToolRegistered,
    UsageState, UsageTotals, VersionedRunRecord, tool_receipt_identity_sha256,
};
...
use crate::completion_review::{
    ...
    ContextCheckpointCapacity, FINAL_RESPONSE_LATENCY_RESERVE_V1,
    LATE_ACCEPTANCE_REVIEW_FINAL_PROMPT_V1, LateAcceptanceReviewCapacity,
    REVIEW_BOUNDED_TOOL_RESERVE_V1, SEMANTIC_CHECKPOINT_CAPSULE_SCHEMA_V1,
    SEMANTIC_CHECKPOINT_CONTROL_TOOL_V1, SEMANTIC_CHECKPOINT_POLICY_VERSION_V1,
    ValidatedSemanticCheckpointCapsuleV1, recent_terminal_evidence, review_required_reserve_v1,
    semantic_checkpoint_action_turn_cutoff_v1, semantic_checkpoint_control_schema_v1,
    semantic_checkpoint_tail_reserve_v1,
};
...
use crate::finish_controller::{
    ActionAdmission, ActionRejection, FinishController, FinishNoticeFacts, FinishObservation,
    FinishState, FreshCriticCapacity, SOFT_FINISH_PROVIDER_TURN_V1,
};
use crate::foreground::truncate_utf8;
use crate::git_history_gate::{deny_git_history_request, git_history_rejection_observation};
use crate::protected_target::{PROTECTED_TARGET_PERMISSION_DENIED, match_protected_target};
use crate::tool::{
    ToolExecutionBudget, ToolExecutionError, ToolExecutionFailureClass, ToolExecutor, ToolResult,
    WorkspaceMode,
};
...
fn soft_finish_cutoff(last_send: Instant) -> Instant {
    let full_finish_reserve = FINAL_RESPONSE_LATENCY_RESERVE_V1
        .saturating_mul(2)
        .saturating_add(REVIEW_BOUNDED_TOOL_RESERVE_V1);
    last_send
        .checked_sub(full_finish_reserve)
        .unwrap_or(last_send)
}

fn soft_action_cutoff(last_send: Instant) -> Instant {
    last_send
        .checked_sub(FINAL_RESPONSE_LATENCY_RESERVE_V1)
        .unwrap_or(last_send)
}

fn completion_evidence_counter(evidence: &CompletionEvidenceLedger, key: &str) -> u64 {
    evidence
        .render()
        .split("; ")
        .find_map(|part| {
...
async fn run_agent_bound_with_cancellation<P: Provider, T: ToolExecutor>(
    ...
    let wrapped_user_request = contract.effective().wrap_user_query(&spec.task.instruction);
    let mut history = vec![
        HistoryItem::System {
            content: contract.effective().system_prompt.text.clone(),
        },
        HistoryItem::User {
            content: wrapped_user_request.clone(),
        },
    ];
    ...
    writer.append_operational(EventBody::RunStarted(RunStarted {
        ...
        completion_review_policy: Some(completion_review_policy.label().to_owned()),
        context_checkpoint_policy_version: completion_review_policy
            .checkpoint_policy_version()
            .map(str::to_owned),
        checkpoint_capsule_schema_version: completion_review_policy
            .checkpoint_capsule_schema_version()
            .map(str::to_owned),
    }))?;
    ...
    let mut finish_controller = FinishController::new();
    let finish_time_trigger_enabled = Instant::now() < soft_finish_cutoff(last_send);
    let mut completion_challenge = CompletionChallengeState::default();
    let mut finish_notice_issued = false;
    let mut fresh_critic_invoked = false;
    let mut late_acceptance_review_active = false;
    let mut context_checkpoint_invoked = false;
    let mut semantic_checkpoint_active = false;
    let mut semantic_checkpoint_soft_cutoff = None;
    let mut semantic_checkpoint_action_turn_cutoff = None;
    let mut inline_semantic_checkpoint_pending = None;
    ...
    loop {
        ...
        if phase_split_enabled {
            ...
            {
                let checkpoint_lease_expired = semantic_checkpoint_active
                    && (semantic_checkpoint_soft_cutoff.is_some_and(|cutoff| now >= cutoff)
                        || semantic_checkpoint_action_turn_cutoff
                            .is_some_and(|cutoff| state.provider_turn_count >= cutoff));
                finish_controller.observe_checkpoint_lease(checkpoint_lease_expired);
                let initial_exception_active = state.provider_turn_count == 0
                    && initial_provider_phase_at(now, actor_done, last_send, minimum_send_window)
                        == ProviderPhase::ActionOpen
                    && provider_phase_at(now, actor_done, last_send, minimum_send_window, false)
                        == ProviderPhase::FinalOnly;
                if !initial_exception_active
                    && (finish_time_trigger_enabled
                        || state.provider_turn_count >= SOFT_FINISH_PROVIDER_TURN_V1)
                {
                    finish_controller.observe(FinishObservation {
                        now,
                        soft_finish_cutoff: soft_finish_cutoff(last_send),
                        provider_turn_count: state.provider_turn_count,
                    });
                }
                phase = match finish_controller.state() {
                    FinishState::SoftFinish => ProviderPhase::ActionOpen,
                    FinishState::FinalOnly | FinishState::TerminalCommit => {
                        if semantic_checkpoint_active
                            && !fresh_critic_invoked
                            && completion_challenge.phase == CompletionReviewPhase::NotIssued
                        {
                            ProviderPhase::ProvisionalOnly
                        } else {
                            ProviderPhase::FinalOnly
                        }
                    }
                    FinishState::ActionOpen => {
                        if state.provider_turn_count == 0 {
            ...
            if matches!(
                phase,
                ProviderPhase::ProvisionalOnly | ProviderPhase::FinalOnly
            ) {
                finish_controller.close_actions();
            }
            if let Some(notice) = finish_controller.take_notice(FinishNoticeFacts {
                provider_turn_count: state.provider_turn_count,
                tool_call_count: state.tool_call_count,
                timed_out_count: completion_evidence_counter(
                    &state.completion_evidence,
                    "timed_out",
                ),
                rejected_count: completion_evidence_counter(&state.completion_evidence, "rejected"),
                unresolved_background_count: completion_evidence_counter(
                    &state.completion_evidence,
                    "unresolved_background",
                ),
                remaining: last_send.saturating_duration_since(now),
            }) {
                ...
                media_history_receipt = match apply_media_history_policy(&mut history) {
                    ...
                    Err(_) => {
                        return state.finalize_once(TerminalOutcome::failure(
                            ...
                            "finish_notice_history_failed",
                        ));
                    }
                };
                finish_notice_issued = true;
            }
        }
        if phase == ProviderPhase::ActionOpen
            && finish_controller.state() == FinishState::ActionOpen
            && completion_challenge.phase == CompletionReviewPhase::NotIssued
        {
            let checkpoint_capacity = ContextCheckpointCapacity {
                already_invoked: context_checkpoint_invoked,
                provider_turn_count: state.provider_turn_count,
                max_provider_turns: spec.provider.max_turns,
                observed_input_tokens: state.usage.observed_input_tokens().unwrap_or(0),
                history_items: u64::try_from(history.len()).unwrap_or(u64::MAX),
                unresolved_background_count: state
                    .completion_evidence
                    .unresolved_background_count(),
                remaining: last_send.saturating_duration_since(Instant::now()),
            };
            if completion_review_policy.should_checkpoint(checkpoint_capacity)
                && completion_review_policy.uses_semantic_checkpoint()
            {
                let source_history_sha256 = media_history_receipt.history_sha256().to_owned();
                let source_history_items = u64::try_from(history.len()).unwrap_or(u64::MAX);
                let terminal_evidence = recent_terminal_evidence(&history);
                let Some(notice) = completion_review_policy.semantic_inline_notice(
                    checkpoint_capacity,
                    &state.completion_evidence,
                    &terminal_evidence,
                ) else {
                    return state.finalize_once(TerminalOutcome::failure(
                        TerminalStatus::RuntimeFailure,
                        TerminalPhase::Runtime,
                        "semantic_checkpoint_policy_inconsistent",
                    ));
                };
                history.push(HistoryItem::User { content: notice });
                media_history_receipt = match apply_media_history_policy(&mut history) {
                    ...
                    Err(_) => {
                        return state.finalize_once(TerminalOutcome::failure(
                            TerminalStatus::RuntimeFailure,
                            TerminalPhase::Runtime,
                            "semantic_checkpoint_inline_history_failed",
                        ));
                    }
                };
                inline_semantic_checkpoint_pending = Some(InlineSemanticCheckpointPending {
                    source_history_sha256,
                    source_history_items,
                    request_history_sha256: media_history_receipt.history_sha256().to_owned(),
                    turn_index: state.provider_turn_count,
                });
            } else if completion_review_policy.should_checkpoint(checkpoint_capacity) {
                let source_history_sha256 = media_history_receipt.history_sha256().to_owned();
                let source_history_items = u64::try_from(history.len()).unwrap_or(u64::MAX);
                let terminal_evidence = recent_terminal_evidence(&history);
                let Some(mut checkpoint_history) = completion_review_policy.checkpoint_history(
                    &contract.effective().system_prompt.text,
                    ...
                    return state.finalize_once(TerminalOutcome::failure(
                        TerminalStatus::RuntimeFailure,
                        TerminalPhase::Runtime,
                        "context_checkpoint_policy_inconsistent",
                    ));
                };
                let checkpoint_history_items =
                    u64::try_from(checkpoint_history.len()).unwrap_or(u64::MAX);
                let checkpoint_receipt = match apply_media_history_policy(&mut checkpoint_history) {
                    Ok(receipt) => receipt,
                    Err(_) => {
                        return state.finalize_once(TerminalOutcome::failure(
                            TerminalStatus::RuntimeFailure,
                            TerminalPhase::Runtime,
                            "context_checkpoint_history_failed",
                        ));
                    }
                };
                if let Err(error) = state.append_operational(EventBody::ContextCheckpointed(
                    ContextCheckpointedV1 {
                        policy_version: CONTEXT_CHECKPOINT_POLICY_VERSION_V1.to_owned(),
                        source_history_sha256,
                        checkpoint_history_sha256: checkpoint_receipt.history_sha256().to_owned(),
                        source_history_items,
                        checkpoint_history_items,
                        provider_turn_count: state.provider_turn_count,
                        tool_call_count: state.tool_call_count,
                        observed_input_tokens: checkpoint_capacity.observed_input_tokens,
                ...
                media_history_receipt = checkpoint_receipt;
                context_checkpoint_invoked = true;
            }
        }
        if phase != ProviderPhase::ActionOpen
        ...
        if phase == ProviderPhase::ActionOpen && !phase_split_enabled {
        ...
        let action_provider_deadline =
            if review_request_active || finish_controller.state() == FinishState::SoftFinish {
                completion_review_provider_deadline(Instant::now(), actor_done)
            } else {
                actor_done
            };
        let provisional_request_active = phase == ProviderPhase::ProvisionalOnly;
        let mut inline_request_tools = tools.clone();
        if inline_semantic_checkpoint_pending.is_some() {
            inline_request_tools.push(semantic_checkpoint_control_tool_v1());
        }
        let (request_mode, request_tools, provider_deadline, timeout_code) = match phase {
            ProviderPhase::ActionOpen => (
                ProviderRequestMode::ActionOpen,
                inline_request_tools.as_slice(),
            ...
            ProviderPhase::FinalOnly => {
                ...
                (
                    ...
                    if late_acceptance_review_active {
                        &[] as &[FunctionTool]
                    } else {
                        tools.as_slice()
                    },
            ...
            ProviderPhase::ProvisionalOnly => {
                let Some(provisional_deadline) =
                    review_required_reserve_v1(completion_review_policy)
                        .and_then(|reserve| last_send.checked_sub(reserve))
                else {
                    return state.finalize_once(TerminalOutcome::failure(
                        TerminalStatus::RuntimeFailure,
                        TerminalPhase::Runtime,
                        "semantic_checkpoint_provisional_reserve_invalid",
                    ));
                };
                if Instant::now() >= provisional_deadline {
                    return state.finalize_once(TerminalOutcome::failure(
                        TerminalStatus::DeadlineFailure,
                        TerminalPhase::Deadline,
                        "provider_semantic_checkpoint_provisional_deadline_exceeded",
                    ));
                }
                (
                    ProviderRequestMode::FinalOnly,
        ...
        let pending = match prepare_pending_provider_request(
            ...
            ProviderRequestEvidence {
                ...
                budget_observation: provider_budget_observation(
                    deadline,
                    if provisional_request_active {
                        ProviderBudgetPhaseV1::CheckpointProvisional
                    } else if inline_semantic_checkpoint_pending.is_some() {
                        ProviderBudgetPhaseV1::CheckpointInline
                    } else if late_acceptance_review_active {
                        ProviderBudgetPhaseV1::CompletionLateReview
                    } else {
                        match request_mode {
                    ...
                    finish_notice_issued,
                ),
                checkpoint_source_history_sha256: inline_semantic_checkpoint_pending
                    .as_ref()
                    .map(|pending| pending.source_history_sha256.as_str()),
            },
        ) {
            Ok(pending) => pending,
            Err(failure) => {
            ...
            Err(_) => {
                if let Some(text) = completion_challenge.take_safe_fallback() {
                    ...
                    return record_provider_failure_with_final(
                        &mut state,
                        turn_index,
                        ProviderFailure::new(timeout_code),
                        &send_telemetry,
                        text,
                        ProviderRejectedResponse::default(),
                    );
        ...
        let no_response_tools = BTreeSet::new();
        let mut response_action_tools = known_tools.clone();
        if inline_semantic_checkpoint_pending.is_some() {
            response_action_tools.insert(SEMANTIC_CHECKPOINT_CONTROL_TOOL_V1);
        }
        let response_known_tools = if provisional_request_active {
            &no_response_tools
        } else {
            &response_action_tools
        };
        let response_call_limit = if provisional_request_active {
            0
        } else if inline_semantic_checkpoint_pending.is_some() {
            profile
                .scheduler
        ...
        if let Err(failure) = completed.validate_authoritative(
                ...
                && completion_challenge.provisional_final.is_none()
            {
                ...
                if let Some(checkpoint) = inline_semantic_checkpoint_pending.take() {
                    append_semantic_checkpoint_rejection(
                        &mut state,
                        &checkpoint,
                        failure.code(),
                        true,
        ...
        let action_call_count = calls
            .iter()
            .filter(|call| call.name != SEMANTIC_CHECKPOINT_CONTROL_TOOL_V1)
            .count();
        if request_mode == ProviderRequestMode::ActionOpen {
            let projected_total = state
                .tool_call_count
                .saturating_add(u64::try_from(action_call_count).unwrap_or(u64::MAX));
            if projected_total > profile.scheduler.max_function_calls_per_run {
                let rejected = ProviderRejectedResponse {
                    call_count: Some(u64::try_from(action_call_count).unwrap_or(u64::MAX)),
                    usage: completed.usage.clone(),
                };
                state.provider_failed(rejected.usage.as_ref());
        ...
        let checkpoint_calls = calls
            ...
            .cloned()
            .collect::<Vec<_>>();
        let action_calls = calls
            .iter()
            .filter(|call| call.name != SEMANTIC_CHECKPOINT_CONTROL_TOOL_V1)
            .cloned()
            .collect::<Vec<_>>();
        let inline_checkpoint_resolution = inline_semantic_checkpoint_pending.as_ref().map(|_| {
            if checkpoint_calls.len() != 1 {
                InlineSemanticCheckpointResolution::Rejected {
                    reason: if checkpoint_calls.is_empty() {
                ...
                match completion_review_policy
                    .parse_semantic_capsule(&checkpoint_calls[0].arguments_json)
                {
                    Ok(capsule) => InlineSemanticCheckpointResolution::Accepted(capsule),
                    Err(error) => InlineSemanticCheckpointResolution::Rejected {
                        reason: error.code(),
                        capsule_content: Some(checkpoint_calls[0].arguments_json.clone()),
                    },
                }
            }
        });
        ...
        if request_mode == ProviderRequestMode::FinalOnly && !provisional_request_active {
        ...
        if matches!(
            &inline_checkpoint_resolution,
            Some(InlineSemanticCheckpointResolution::Rejected { .. })
        ) && let (Some(pending), Some(resolution)) = (
            inline_semantic_checkpoint_pending.take(),
            inline_checkpoint_resolution.as_ref(),
        ) {
            let committed = commit_inline_semantic_checkpoint(
                &mut state,
                &pending,
                resolution,
                InlineSemanticCheckpointContext {
                    policy: completion_review_policy,
                    system_prompt: &contract.effective().system_prompt.text,
                    wrapped_user_request: &wrapped_user_request,
                    settled_history: &history,
                    last_send,
                    max_provider_turns: spec.provider.max_turns,
                },
            )?;
            debug_assert!(committed.is_none());
            context_checkpoint_invoked = true;
        }

        if action_calls.is_empty() && checkpoint_calls.is_empty() {
            if let Some(text) = final_text.filter(|text| !text.is_empty()) {
                let mut full_fresh_review_admitted = false;
                let mut late_acceptance_review_admitted = false;
                let review_admitted = phase_split_enabled
                    && completion_challenge.phase == CompletionReviewPhase::NotIssued
                    && if completion_review_policy.uses_fresh_context() {
                        ...
                        let capacity = FreshCriticCapacity {
                            now,
                            last_send,
                            provisional_final_nonempty: !text.is_empty(),
                            critic_already_invoked: fresh_critic_invoked,
                            provider_turn_count: state.provider_turn_count,
                            max_provider_turns: spec.provider.max_turns,
                            history_items: u64::try_from(history.len()).unwrap_or(u64::MAX),
                            max_history_items: profile.context.max_history_items,
                            function_call_count: state.tool_call_count,
                            max_function_calls_per_response: profile
                                .scheduler
                                .max_function_calls_per_response,
                            max_function_calls_per_run: profile
                                .scheduler
                                .max_function_calls_per_run,
                        };
                        full_fresh_review_admitted =
                            completion_review_policy.has_fresh_path_capacity(capacity);
                        if !full_fresh_review_admitted {
                            late_acceptance_review_admitted = completion_review_policy
                                .has_late_acceptance_review_capacity(
                                    LateAcceptanceReviewCapacity {
                                        now,
                                        actor_done,
                                        last_send,
                                        provisional_final_nonempty: !text.is_empty(),
                                        review_already_invoked: fresh_critic_invoked
                                            || late_acceptance_review_active,
                                        provider_turn_count: state.provider_turn_count,
                                        max_provider_turns: spec.provider.max_turns,
                                        history_items: u64::try_from(history.len())
                                            .unwrap_or(u64::MAX),
                                        max_history_items: profile.context.max_history_items,
                                        function_call_count: state.tool_call_count,
                                        max_function_calls_per_response: profile
                        ...
                        full_fresh_review_admitted || late_acceptance_review_admitted
                    } else {
                ...
                if review_admitted {
                    ...
                    if full_fresh_review_admitted {
                        ...
                        phase = ProviderPhase::ActionOpen;
                        continue;
                    }
                    if late_acceptance_review_admitted {
                        late_acceptance_review_active = true;
                        let Some(challenge) = completion_review_policy
                            .late_acceptance_review_prompt(
                                &text,
                                &terminal_evidence,
                                &state.completion_evidence,
                            )
        ...
        let soft_action_batch = finish_controller.state() == FinishState::SoftFinish;
        ...
        let review_tool_stage =
            completion_challenge.admit_tool_stage(action_calls.iter().map(|call| {
        ...
        let review_tool_deadline = if review_tool_stage.is_some() || soft_action_batch {
            completion_review_tool_deadline(Instant::now(), tool_settled)
        } else {
            tool_settled
        };
        let requested_semantic_deadline = if review_tool_stage.is_some() || soft_action_batch {
            review_tool_deadline.min(actor_done)
        } else {
            actor_done
        };
        let (semantic_deadline, finish_admission_rejection) = if review_was_active {
            (requested_semantic_deadline, None)
        } else {
            match finish_controller.admit_action_batch(
                Instant::now(),
                requested_semantic_deadline,
                soft_action_cutoff(last_send),
                tool_settled,
            ) {
                ActionAdmission::Admitted(envelope) => (envelope.semantic_deadline, None),
                ActionAdmission::Rejected(reason) => (
                    requested_semantic_deadline,
                    Some(match reason {
                        ActionRejection::ActionAlreadyAdmitted => "finish_action_already_admitted",
                        ActionRejection::CutoffReached => "finish_action_cutoff_reached",
                        ActionRejection::PhaseClosed => "finish_action_phase_closed",
                    }),
                ),
            }
        };
        let admission_rejection = admission_rejection.or(finish_admission_rejection);
        let mut staging = TurnBatchStaging::new(state.tool_output_bytes);
        if let Some(resolution) = &inline_checkpoint_resolution {
            for call in &checkpoint_calls {
                staging.push(stage_semantic_checkpoint_control(call, resolution));
            }
        }
        ...
        for call in action_calls {
            ...
            let settlement = settle_tool_call(
                ...
                ToolExecutionWindow {
                    ...
                    dispatch_cutoff: semantic_deadline,
        ...
        if let Some(review_tool_stage) = review_tool_stage {
            ...
            if late_acceptance_review_active || !review_stage_dispatched {
        ...
        {
            history.push(HistoryItem::User {
                content: if late_acceptance_review_active {
                    LATE_ACCEPTANCE_REVIEW_FINAL_PROMPT_V1.to_owned()
                } else {
                    COMPLETION_REVIEW_DECISION_PROMPT.to_owned()
                },
            });
            media_history_receipt = match apply_media_history_policy(&mut history) {
                Ok(receipt) => receipt,
        ...
        if soft_action_batch {
            finish_controller.settle_action_batch();
        }
        if let (Some(pending), Some(resolution)) = (
            inline_semantic_checkpoint_pending.take(),
            inline_checkpoint_resolution.as_ref(),
        ) {
            if let Some(committed) = commit_inline_semantic_checkpoint(
                &mut state,
                &pending,
                resolution,
                InlineSemanticCheckpointContext {
                ...
                media_history_receipt = committed.receipt;
                semantic_checkpoint_active = true;
                semantic_checkpoint_soft_cutoff = committed.soft_cutoff;
                semantic_checkpoint_action_turn_cutoff = Some(committed.action_turn_cutoff);
            }
            context_checkpoint_invoked = true;
        }
        phase = if completion_challenge.requires_final_only() {
            ProviderPhase::FinalOnly
        } else if completion_challenge.phase != CompletionReviewPhase::NotIssued {
            ProviderPhase::ActionOpen
        } else if finish_controller.state() == FinishState::FinalOnly {
            if semantic_checkpoint_active
                && !fresh_critic_invoked
                && completion_challenge.phase == CompletionReviewPhase::NotIssued
            {
...
struct ProviderRequestEvidence<'a> {
    ...
    checkpoint_source_history_sha256: Option<&'a str>,
}

#[derive(Debug)]
struct InlineSemanticCheckpointPending {
    source_history_sha256: String,
    source_history_items: u64,
    request_history_sha256: String,
    turn_index: u64,
}

#[derive(Debug)]
enum InlineSemanticCheckpointResolution {
    Accepted(ValidatedSemanticCheckpointCapsuleV1),
    Rejected {
        reason: &'static str,
        capsule_content: Option<String>,
    },
}

struct InlineSemanticCheckpointCommit {
    history: Vec<HistoryItem>,
    receipt: MediaHistoryPolicyReceiptV1,
    soft_cutoff: Option<Instant>,
    action_turn_cutoff: u64,
}

struct InlineSemanticCheckpointContext<'a> {
    policy: CompletionReviewPolicy,
    system_prompt: &'a str,
    wrapped_user_request: &'a str,
    settled_history: &'a [HistoryItem],
    last_send: Instant,
    max_provider_turns: u64,
}

fn semantic_checkpoint_control_tool_v1() -> FunctionTool {
    FunctionTool {
        name: SEMANTIC_CHECKPOINT_CONTROL_TOOL_V1.to_owned(),
        description: "Record a bounded continuity capsule while the normal action turn continues. This runtime-owned control call is not dispatched to the workspace.".to_owned(),
        parameters: semantic_checkpoint_control_schema_v1(),
    }
}

fn commit_inline_semantic_checkpoint(
    state: &mut RunState<'_>,
    pending: &InlineSemanticCheckpointPending,
    resolution: &InlineSemanticCheckpointResolution,
    context: InlineSemanticCheckpointContext<'_>,
) -> Result<Option<InlineSemanticCheckpointCommit>, AgentRunError> {
    let InlineSemanticCheckpointContext {
        policy,
        system_prompt,
        wrapped_user_request,
        settled_history,
    ...
    let capsule = match resolution {
        InlineSemanticCheckpointResolution::Accepted(capsule) => capsule,
        InlineSemanticCheckpointResolution::Rejected {
            reason,
            capsule_content,
        } => {
    ...
    let capacity = ContextCheckpointCapacity {
        ...
        max_provider_turns,
        observed_input_tokens: state.usage.observed_input_tokens().unwrap_or(0),
        history_items: u64::try_from(settled_history.len()).unwrap_or(u64::MAX),
        unresolved_background_count: state.completion_evidence.unresolved_background_count(),
        remaining: last_send.saturating_duration_since(Instant::now()),
    };
    let tail_reserve =
        semantic_checkpoint_tail_reserve_v1(policy).ok_or(AgentRunError::Incomplete {
            code: "semantic_checkpoint_tail_reserve_missing",
        })?;
    let soft_cutoff = last_send.checked_sub(tail_reserve);
    let action_turn_cutoff = semantic_checkpoint_action_turn_cutoff_v1(max_provider_turns);
    let action_lease_ms = soft_cutoff
        .and_then(|cutoff| cutoff.checked_duration_since(Instant::now()))
        .map(|remaining| u64::try_from(remaining.as_millis()).unwrap_or(u64::MAX));
    if !policy.should_accept_semantic_checkpoint_after_prepare(capacity)
        || action_turn_cutoff <= state.provider_turn_count
        || action_lease_ms.is_none_or(|value| value == 0)
    {
        append_semantic_checkpoint_rejection(
            state,
            pending,
            "semantic_checkpoint_capacity_exhausted",
            true,
            true,
            Some(capsule.canonical_json()),
        )?;
        return Ok(None);
    }
    let settled_evidence = recent_terminal_evidence(settled_history);
    let mut checkpoint_history = policy
        ...
        .ok_or(AgentRunError::Incomplete {
            code: "semantic_checkpoint_policy_inconsistent",
        })?;
    let checkpoint_history_items = u64::try_from(checkpoint_history.len()).unwrap_or(u64::MAX);
    let receipt = apply_media_history_policy(&mut checkpoint_history).map_err(|_| {
        AgentRunError::Incomplete {
            code: "semantic_checkpoint_history_failed",
        }
    })?;
    state.append_operational(EventBody::ContextCheckpointed(ContextCheckpointedV1 {
        policy_version: SEMANTIC_CHECKPOINT_POLICY_VERSION_V1.to_owned(),
        source_history_sha256: pending.source_history_sha256.clone(),
        checkpoint_history_sha256: receipt.history_sha256().to_owned(),
        source_history_items: pending.source_history_items,
        checkpoint_history_items,
        provider_turn_count: state.provider_turn_count,
        tool_call_count: state.tool_call_count,
        observed_input_tokens: capacity.observed_input_tokens,
        capsule_schema_version: Some(SEMANTIC_CHECKPOINT_CAPSULE_SCHEMA_V1.to_owned()),
        capsule_sha256: Some(capsule.sha256().to_owned()),
        capsule_bytes: Some(capsule.bytes()),
        prepare_turn_index: Some(pending.turn_index),
        prepare_history_sha256: Some(pending.request_history_sha256.clone()),
        action_turn_cutoff: Some(action_turn_cutoff),
        action_lease_ms,
        tail_reserve_ms: Some(u64::try_from(tail_reserve.as_millis()).unwrap_or(u64::MAX)),
    }))?;
    Ok(Some(InlineSemanticCheckpointCommit {
        history: checkpoint_history,
        receipt,
        soft_cutoff,
        action_turn_cutoff,
...
fn prepare_pending_provider_request<P: Provider>(
    ...
    let ProviderRequestEvidence {
        ...
        budget_observation,
        checkpoint_source_history_sha256,
    } = evidence;
    ...
    Ok(PendingProviderRequest {
        ...
        projection: ProviderRequested {
            ...
            budget_observation,
            checkpoint_source_history_sha256: checkpoint_source_history_sha256.map(str::to_owned),
        },
    })
}

fn append_semantic_checkpoint_rejection(
    state: &mut RunState<'_>,
    pending: &InlineSemanticCheckpointPending,
    reason: &str,
    request_emitted: bool,
    response_received: bool,
    capsule_content: Option<&str>,
) -> Result<(), EventWriteError> {
    let capsule_content_sha256 =
        capsule_content.map(|content| format!("{:x}", Sha256::digest(content.as_bytes())));
    let capsule_content_bytes =
        capsule_content.map(|content| u64::try_from(content.len()).unwrap_or(u64::MAX));
    let capsule_content_excerpt = capsule_content.map(|content| truncate_utf8(content, 2_048));
    state.append_operational(EventBody::ContextCheckpointRejected(
        ContextCheckpointRejectedV1 {
            policy_version: SEMANTIC_CHECKPOINT_POLICY_VERSION_V1.to_owned(),
            source_history_sha256: pending.source_history_sha256.clone(),
            prepare_history_sha256: pending.request_history_sha256.clone(),
            prepare_turn_index: pending.turn_index,
            provider_turn_count: state.provider_turn_count,
    ...
    let completed = match provider_result {
        ...
        Err(_) => {
            ...
            return record_provider_failure_with_final(
                state,
                turn_index,
                ProviderFailure::new("provider_completion_critic_deadline_exceeded"),
                &send_telemetry,
                candidate_final,
                ProviderRejectedResponse::default(),
            )
...
fn stage_semantic_checkpoint_control(
    call: &FunctionCall,
    resolution: &InlineSemanticCheckpointResolution,
) -> StagedToolCompletion {
    let output = match resolution {
        InlineSemanticCheckpointResolution::Accepted(_) => {
            "semantic checkpoint capsule accepted; reset waits for same-turn tool settlement"
        }
        InlineSemanticCheckpointResolution::Rejected { reason, .. } => reason,
    }
    .to_owned();
    StagedToolCompletion {
        event: ToolCompletedEvent {
        ...
        attachment: None,
        committed_output_bytes: u64::try_from(output.len()).unwrap_or(u64::MAX),
        tool_receipt: None,
...
fn commit_turn_history_batch(
    ...
    for completion in &staging.completions {
        if !completion.internal_control {
            state
...
async fn settle_tool_call<T: ToolExecutor>(
    ...
    if deny_git_history_request(
        &call.name,
        &call.arguments_json,
        state.spec.task.git_history_capability.git_history_access,
    ) {
        return prepare_tool_completion(
            state,
            call,
            ToolResult::rejected(git_history_rejection_observation()),
            window.runtime_deadline,
            window.max_provider_turns,
            window.output_limits,
            window.projected_tool_output_bytes,
                ...
                let execution = tokio::select! {
                    ...
                    result = tokio::time::timeout_at(
                        ...
                        executor.execute_with_budget(
                            call,
                            window.workspace,
                            ToolExecutionBudget::new(
                                window.dispatch_cutoff,
                                window.settlement_deadline,
                            ),
                        ),
                    ) => result,
                };
...
impl UsageAccumulator {
    fn observed_input_tokens(&self) -> Option<u64> {
        (!self.invalid && self.usage_present > 0).then_some(self.input_tokens)
    }
...
mod tests {
    ...
    use nano_types::run_spec::{
        ContractSpec, GIT_HISTORY_CAPABILITY_POLICY, GIT_HISTORY_CAPABILITY_SCHEMA,
        GitHistoryAccess, GitHistoryCapability, ProviderKind, ProviderSpec, RUN_SPEC_SCHEMA,
        RunSpec, TaskSpec,
    };
    use sha2::{Digest, Sha256};
            ...
            task: TaskSpec {
                ...
                git_history_capability: GitHistoryCapability {
                    ...
                    canonical_instruction_sha256:
                        "f296e1ab660c1821313f331bad8a4abbcba7eb8d49238c87e5152f0f50fb9af1"
                            .to_owned(),
                    trusted_manifest_sha256: "a".repeat(64),
    ...
    #[tokio::test]
    async fn git_history_denial_is_registered_recoverable_and_continues_to_current_state_tool() {
        let directory = tempfile::tempdir().expect("event directory");
        let writer = EventWriter::create(
            directory.path(),
            "run",
        ...
        let calls = [
            FunctionCall {
                call_id: "call-history".to_owned(),
                name: "run_terminal_command".to_owned(),
                arguments_json: serde_json::json!({
                    "command": "printf SAFE_SUBSET_CANARY > marker && git show HEAD~1:src/app.c"
                })
                .to_string(),
            },
            FunctionCall {
                call_id: "call-current-show".to_owned(),
                name: "run_terminal_command".to_owned(),
                arguments_json: serde_json::json!({"command": "git show HEAD"}).to_string(),
            },
            FunctionCall {
                call_id: "call-current".to_owned(),
                name: "run_terminal_command".to_owned(),
                arguments_json: serde_json::json!({"command": "git status --short"}).to_string(),
            },
        ];
        ...
        for call in &calls {
            ...
            .await
            .expect("history denial and current-state continuation must settle");
            staging.push(completion);
        }

        assert_eq!(executor.executions, 2);
        assert!(
            staging.completions[0]
                .event
                .output
                .starts_with("git_history_not_required:")
        );
        assert!(
            staging.completions[0]
                .event
                .output
                .contains("No part of this tool call was executed")
        );
        assert!(
            staging.completions[0]
                .event
                .output
                .contains("workspace is unchanged")
        );
        assert!(
            !staging.completions[0]
                .event
                .output
                .contains("SAFE_SUBSET_CANARY")
        );
        assert!(!staging.completions[0].dispatched);
        assert!(staging.completions[1].dispatched);
        assert!(staging.completions[2].dispatched);
        let events =
            std::fs::read_to_string(directory.path().join("events.jsonl")).expect("events");
        assert_eq!(events.matches("\"type\":\"tool.registered\"").count(), 3);
        assert_eq!(events.matches("\"type\":\"tool.dispatched\"").count(), 2);
        assert!(!events.contains("SAFE_SUBSET_CANARY > marker\n"));
    }

    #[tokio::test]
    async fn explicit_history_capability_dispatches_the_same_history_shape() {
        let directory = tempfile::tempdir().expect("event directory");
        let writer = EventWriter::create(
            directory.path(),
            "run",
        ...
        spec.task.git_history_capability.git_history_access = GitHistoryAccess::Required;
        spec.task.git_history_capability.supporting_span_sha256 = Some("c".repeat(64));
        let mut state =
            super::RunState::new(writer, &spec, "a".repeat(64), super::RunRecordEmission::V2);
        state.provider_turn_count = 1;
        state.tool_call_count = 1;
        ...
        let mut executor = CrossCutoffExecutor::default();
        let call = FunctionCall {
            call_id: "call-history-required".to_owned(),
            name: "run_terminal_command".to_owned(),
            arguments_json: serde_json::json!({"command": "git show HEAD~1:src/app.c"}).to_string(),
        };
        let completion = settle_tool_call(
            &mut state,
            &mut executor,
        ...
        .await
        .expect("bound explicit-history task must preserve normal dispatch");
        assert!(completion.dispatched);
    ...
    async fn p0b_multi_call_cutoff_commits_accepted_prefix_and_rejected_suffix() {
        ...
        let mut receipt =
            nano_provider_xai::apply_media_history_policy(&mut history).expect("receipt");
        let cancellation = super::RunCancellation::new();
        let actor_done = Instant::now() + Duration::from_millis(500);
        let tool_settled = Instant::now() + Duration::from_secs(2);
        let mut executor = CrossCutoffExecutor::default();
        let mut staging = TurnBatchStaging::new(0);
        for call in &calls {
            state.tool_call_count = state.tool_call_count.saturating_add(1);
