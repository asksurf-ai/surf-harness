...
fn validate_completion_review_policy_binding(
    ...
    let valid = match review_policy {
        ...
        Some("semantic-checkpoint-v5" | "semantic-checkpoint-v6" | "semantic-checkpoint-v7") => {
...
impl ProviderRequested {
    fn validate_checkpoint_binding(&self) -> Result<(), EventValidationError> {
        let checkpoint_phase = self.budget_observation.as_ref().and_then(|observation| {
            matches!(
                observation.phase,
                ProviderBudgetPhaseV1::CheckpointPrepare | ProviderBudgetPhaseV1::CheckpointInline
            )
            .then_some(observation.phase)
        });
        if checkpoint_phase.is_some() != self.checkpoint_source_history_sha256.is_some()
            || matches!(
                checkpoint_phase,
                Some(ProviderBudgetPhaseV1::CheckpointPrepare)
            ) && self.tool_count != 0
            || matches!(
                checkpoint_phase,
                Some(ProviderBudgetPhaseV1::CheckpointInline)
            ) && self.tool_count == 0
        {
            return Err(EventValidationError::new(
...
pub struct ContextCheckpointRejectedV1 {
    ...
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub capsule_content_sha256: Option<String>,
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub capsule_content_bytes: Option<u64>,
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub capsule_content_excerpt: Option<String>,
}
...
impl ContextCheckpointRejectedV1 {
    fn validate(&self) -> Result<(), EventValidationError> {
        ...
        match (
            &self.capsule_content_sha256,
            self.capsule_content_bytes,
            &self.capsule_content_excerpt,
        ) {
            (None, None, None) => Ok(()),
            (Some(sha256), Some(bytes), Some(excerpt))
                if self.response_received
                    && bytes >= u64::try_from(excerpt.len()).unwrap_or(u64::MAX)
                    && !excerpt.is_empty()
                    && excerpt.len() <= 2_048 =>
            {
            ...
            _ => Err(EventValidationError::new(
                "context_checkpoint_rejection_diagnostic_invalid",
            )),
        }
    }
}
...
mod tests {
    ...
    fn semantic_checkpoint_receipt_and_rejection_are_closed_and_hash_bound() {
        ...
        let rejection = base_event(EventBody::ContextCheckpointRejected(
            ContextCheckpointRejectedV1 {
                ...
                capsule_content_bytes: None,
                capsule_content_excerpt: None,
            },
        ));
        rejection.validate().expect("typed rejection receipt");
        let mut diagnosed = rejection.clone();
        let EventBody::ContextCheckpointRejected(diagnostic) = &mut diagnosed.body else {
            panic!("checkpoint rejection");
        };
        diagnostic.capsule_content_sha256 = Some("b".repeat(64));
        diagnostic.capsule_content_bytes = Some(3);
        diagnostic.capsule_content_excerpt = Some("bad".to_owned());
        diagnosed.validate().expect("bounded capsule diagnostic");
    ...
    fn completion_review_policy_binding_is_closed_and_semantic_schema_is_mandatory() {
        let mut event = base_event(EventBody::RunStarted(RunStarted {
            ...
            media_history_policy_version: None,
            media_history_policy_sha256: None,
            completion_review_policy: Some("semantic-checkpoint-v6".to_owned()),
        ...
        started.completion_review_policy = Some("semantic-checkpoint-v7".to_owned());
        event.validate().expect("v7 semantic policy provenance");

        let EventBody::RunStarted(started) = &mut event.body else {
            panic!("run.started");
        };
        started.completion_review_policy = Some("semantic-checkpoint-v5".to_owned());
        event.validate().expect("legacy semantic policy provenance");
    ...
    fn provider_budget_observation_is_typed_and_legacy_optional() {
        ...
        event.validate().expect("typed late review observation");
        let value = serde_json::to_value(&event).expect("late review request event");
        assert_eq!(
            value["data"]["budget_observation"]["phase"],
