...
pub const RUN_SPEC_SCHEMA: &str = "nano-run-spec-alpha-2";
pub const GIT_HISTORY_CAPABILITY_SCHEMA: &str = "nano-git-history-capability-v1";
pub const GIT_HISTORY_CAPABILITY_POLICY: &str = "nano-git-history-capability-policy-v3";
...
pub struct TaskSpec {
    ...
    pub git_history_capability: GitHistoryCapability,
}

#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
#[serde(deny_unknown_fields)]
pub struct GitHistoryCapability {
    pub schema_version: String,
    pub policy_version: String,
    pub git_history_access: GitHistoryAccess,
    pub canonical_instruction_sha256: String,
    pub trusted_manifest_sha256: String,
    pub supporting_span_sha256: Option<String>,
}

#[derive(Debug, Clone, Copy, PartialEq, Eq, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum GitHistoryAccess {
    Required,
    NotRequired,
}
...
impl RunSpec {
    ...
    pub fn validate(&self) -> Result<(), RunSpecError> {
        ...
        let capability = &self.task.git_history_capability;
        require_equal(
            "task.git_history_capability.schema_version",
            &capability.schema_version,
            GIT_HISTORY_CAPABILITY_SCHEMA,
        )?;
        require_equal(
            "task.git_history_capability.policy_version",
            &capability.policy_version,
            GIT_HISTORY_CAPABILITY_POLICY,
        )?;
        validate_sha256(
            "task.git_history_capability.canonical_instruction_sha256",
            &capability.canonical_instruction_sha256,
        )?;
        require_equal(
            "task.git_history_capability.canonical_instruction_sha256",
            &capability.canonical_instruction_sha256,
            &sha256_hex(self.task.instruction.as_bytes()),
        )?;
        require_equal(
            "task.git_history_capability.trusted_manifest_sha256",
            &capability.trusted_manifest_sha256,
            &self.task.digest,
        )?;
        match (
            capability.git_history_access,
            capability.supporting_span_sha256.as_deref(),
        ) {
            (GitHistoryAccess::Required, Some(span)) => {
                validate_sha256("task.git_history_capability.supporting_span_sha256", span)?
            }
            (GitHistoryAccess::NotRequired, None) => {}
            _ => return invalid("task.git_history_capability supporting span is invalid"),
        }
...
mod tests {
    ...
    use super::{
        ...
        RunSpec, RunSpecError, TaskSpec, sha256_hex,
    };
        ...
        RunSpec {
            ...
            task: TaskSpec {
                ...
                git_history_capability: GitHistoryCapability {
                    ...
                    canonical_instruction_sha256: sha256_hex(b"Do the task."),
    ...
    #[test]
    fn capability_is_bound_into_canonical_run_spec_bytes_and_hash() {
        let spec = valid_spec();
        ...
        assert_eq!(
            ...
            concat!(
                ...
                "\"policy_version\":\"nano-git-history-capability-policy-v3\",",
                "\"git_history_access\":\"not_required\",",
                "\"canonical_instruction_sha256\":\"c41d214df5a7fe54bd73a8a6e0bcb00ebbb1294150da17f325d66944241d4c17\",",
                "\"trusted_manifest_sha256\":\"aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa\",",
        ...
        assert_eq!(
            ...
            "360e2b7b449164c0e8a295be4bbdf0042ad5024db26bb02020ebdfb3be30ce6d"
        );
    }
    ...
    #[test]
    fn capability_missing_malformed_and_source_tamper_fail_closed() {
        let canonical = serde_json::to_value(valid_spec()).expect("serialize RunSpec");
        for mutation in ["missing", "malformed"] {
            ...
            if mutation == "missing" {
                value["task"]
                    .as_object_mut()
                    .expect("task object")
                    .remove("git_history_capability");
            } else {
                value["task"]["git_history_capability"] = serde_json::json!("invalid");
            }
            assert!(
                serde_json::from_value::<RunSpec>(value).is_err(),
                "{mutation}"
            );
        }

        for mutation in ["instruction", "manifest", "span"] {
            let mut spec = valid_spec();
            match mutation {
                "instruction" => spec.task.instruction.push_str(" changed"),
                "manifest" => {
                    spec.task.git_history_capability.trusted_manifest_sha256 = "b".repeat(64)
                }
                "span" => {
            ...
            assert!(matches!(
                spec.validate(),
                Err(RunSpecError::Invalid { reason })
                    if reason.contains("git_history_capability")
            ));
        }
    }
