...
/// A bounded actor observation of a direct tool's externally visible effect.
///
/// Bash deliberately omits this object because v1 does not inventory the
/// workspace around arbitrary commands. A required wire `null` therefore
/// means that the effect was unobserved, never that it was unchanged.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum ExternalEffectObservationStatusV1 {
    Changed,
    Unchanged,
    NotApplicable,
}
...
pub struct ExternalEffectObservationV1 {
    pub status: ExternalEffectObservationStatusV1,
}
...
pub struct ExternalToolCompletedResultV3 {
    ...
    pub census: ExternalProcessCensus,
    #[serde(deserialize_with = "deserialize_required_option")]
    pub effect_observation_v1: Option<ExternalEffectObservationV1>,
    pub media: Option<Box<ExternalMediaPayload>>,
...
mod tests {
    ...
    use super::{
        ...
        ExternalEffectObservationStatusV1, ExternalEffectObservationV1, ExternalEnvironmentPolicy,
        ExternalMediaPayload, ExternalMediaType, ExternalProcessCensus, ExternalProcessDisposition,
        ExternalToolCompletedResult, ExternalToolCompletedResultV3, ExternalToolFailure,
        ExternalToolLimits, ExternalToolMessageType, ExternalToolRecoverability,
        ExternalToolRequest, ExternalToolRequestV3, ExternalToolResponse,
        ExternalToolResponseDocument, ExternalToolSettlement, ExternalToolSettlementV3,
        ExternalToolWaitReason,
    };
    ...
    fn v3_completed_wait_metadata_is_required_correlated_and_no_v2_downgrade_exists() {
        ...
        assert!(value["result"]["effect_observation_v1"].is_null());

        let mut observed = value.clone();
        observed["result"]["effect_observation_v1"] = serde_json::json!({
            "status": "changed"
        });
        let observed = serde_json::from_value::<ExternalToolSettlementV3>(observed)
            .expect("closed effect observation");
        let ExternalToolSettlementV3::Completed { result, .. } = observed else {
            unreachable!("completed")
        };
        assert_eq!(
            result.effect_observation_v1,
            Some(ExternalEffectObservationV1 {
                status: ExternalEffectObservationStatusV1::Changed,
            })
        );

        let mut missing_effect = value.clone();
        missing_effect["result"]
            .as_object_mut()
            .expect("result object")
            .remove("effect_observation_v1");
        assert!(serde_json::from_value::<ExternalToolSettlementV3>(missing_effect).is_err());

        let mut unknown_effect = value.clone();
        unknown_effect["result"]["effect_observation_v1"] = serde_json::json!({
            "status": "changed",
            "guessed": true
        });
        assert!(serde_json::from_value::<ExternalToolSettlementV3>(unknown_effect).is_err());
