...
fn validate_runtime_profile(
    ...
    validate_model_visible_capability_parity(effective, profile)?;
    validate_runtime_media_capabilities(effective, profile)?;
    validate_runtime_profile_values(profile)
}

fn validate_model_visible_capability_parity(
    effective: &EffectiveContract,
    profile: &AgentProfile,
) -> Result<(), ContractError> {
    let terminal = &effective.tools[0];
    require_schema_u64(
        terminal,
        "timeout",
        "default",
        profile.tools.terminal_default_timeout_ms,
        "terminal schema default",
    )?;
    require_schema_u64(
        terminal,
        "timeout",
        ...
        "terminal schema maximum",
    )?;
    require_description_fragment(
        terminal,
        &format!(
            "default to {} ms",
            profile.tools.terminal_default_timeout_ms
    ...
    require_description_fragment(
        terminal,
        &format!("at most {} ms", profile.tools.terminal_max_timeout_ms),
        "terminal description maximum",
    )?;
    require_description_fragment(
        terminal,
        "including setsid and nohup descendants",
        "owned descendants",
    )?;

    let background_output = &effective.tools[7];
    require_schema_u64(
        background_output,
        "timeout_ms",
        "maximum",
        profile.tools.background_output_wait_max_ms,
        "background wait schema maximum",
    )?;
    require_description_fragment(
        background_output,
        "omitted or zero",
        "background wait nonblocking semantics",
    )?;
    require_description_fragment(
        background_output,
        "nonblocking",
    ...
    require_description_fragment(
        ...
        "background wait description unit",
    )?;

    for tool in &effective.tools {
        if tool.result_policy.max_model_output_bytes
            != profile.tools.model_tool_output_bytes_per_call
        {
            return invalid(format!(
                "result bytes for {} must equal profile model_tool_output_bytes_per_call",
                tool.provider_name
            ));
        }
    }
    ...
    let read_file = &effective.tools[1];
    require_description_fragment(read_file, "normal text files", "read_file text capability")?;
    require_description_fragment(read_file, "PNG and JPEG image files only", "media types")?;
    let media_description = read_file.description.to_ascii_lowercase();
    if ["pdf", "powerpoint", "pptx", "all image", "general image"]
        .iter()
        .any(|forbidden| media_description.contains(forbidden))
    {
        return invalid("media types must be PNG and JPEG only");
    }
    Ok(())
}

fn require_schema_u64(
    tool: &ToolContract,
    property: &str,
    keyword: &str,
    expected: u64,
    ...
    let actual = tool
        ...
        .and_then(|schema| schema.get(keyword))
...
fn require_description_fragment(
    tool: &ToolContract,
    expected: &str,
    field: &str,
) -> Result<(), ContractError> {
    ...
    {
        return invalid(format!("{field} must declare {expected}"));
    }
    Ok(())
}
...
mod tests {
        ...
        let tools = TOOL_ORDER
            ...
            .map(|(ordinal, name)| {
                let (description, input_schema) = match *name {
                    "run_terminal_command" => (
                        "Run a bash command and return its output. Foreground commands default to 120000 ms and accept at most 600000 ms. Runtime ownership and cleanup include every launched descendant, including setsid and nohup descendants. Model-visible output is capped at 65536 bytes per call.",
                        json!({
                            "additionalProperties": false,
                            "properties": {
                                "timeout": {
                                    "default": 120000,
                                    "description": "Foreground timeout in milliseconds; maximum 600000 ms.",
                                    "maximum": 600000,
                                    "minimum": 0,
                                    "type": ["integer", "null"]
                                }
                    ...
                    "read_file" => (
                        "Read normal text files and PNG and JPEG image files only. Model-visible output is capped at 65536 bytes per call.",
                        json!({
                            "additionalProperties": false,
                            "properties": {
                    ...
                    "get_terminal_command_output" => (
                        "Get output and status from background commands. Omitted or zero timeout_ms is nonblocking; a positive wait is capped at 30000 ms. Model-visible output is capped at 65536 bytes per call.",
                        json!({
                            "additionalProperties": false,
                            "properties": {
                                "timeout_ms": {
                                    "default": null,
                                    "description": "Wait in milliseconds; omitted or zero is nonblocking, maximum 30000 ms.",
                                    "maximum": 30000,
                                    "minimum": 0,
                                    "type": ["integer", "null"]
                                }
                    ...
                    _ => (
                        "Synthetic definition with model-visible output capped at 65536 bytes per call.",
                        json!({
                            "additionalProperties": false,
                            "properties": {},
                            "required": [],
    ...
    #[test]
    fn rejects_model_visible_capability_drift() {
        let cases = [
            ("terminal schema maximum", 0_u8),
            ("terminal description maximum", 1),
            ("background wait schema maximum", 2),
            ("background wait description unit", 3),
            ("result bytes", 4),
            ("owned descendants", 5),
            ("media types", 6),
        ];
        for (expected, case) in cases {
            let mut bundle = synthetic_bundle();
            bundle.effective = mutate_json(&bundle.effective, |effective| match case {
                0 => {
                    effective["tools"][0]["input_schema"]["properties"]["timeout"]["maximum"] =
                        json!(300_000)
                }
                1 => {
                    let description = effective["tools"][0]["description"]
                        .as_str()
                        .expect("terminal description")
                        .replace("600000 ms", "300000 ms");
                    effective["tools"][0]["description"] = json!(description);
                }
                2 => {
                    effective["tools"][7]["input_schema"]["properties"]["timeout_ms"]["maximum"] =
                        json!(600_000)
                }
                3 => {
                    let description = effective["tools"][7]["description"]
                        .as_str()
                        .expect("background description")
                        .replace("30000 ms", "30 seconds");
                    effective["tools"][7]["description"] = json!(description);
                }
                4 => {
                    effective["tools"][3]["result_policy"]["max_model_output_bytes"] = json!(40_000)
                }
                5 => {
                    let description = effective["tools"][0]["description"]
                        .as_str()
                        ...
                        .replace(
                            "including setsid and nohup descendants",
                            "except detached descendants",
                        );
                    effective["tools"][0]["description"] = json!(description);
                }
                6 => {
                    let description = effective["tools"][1]["description"]
                        .as_str()
                        .expect("read_file description")
                        .replace("PNG and JPEG image files only", "all image and PDF files");
                    effective["tools"][1]["description"] = json!(description);
                }
                _ => unreachable!(),
            });
            let effective_hash = sha256_hex(&bundle.effective);
            ...
            rebuild_manifest(&mut bundle);
            match load(&bundle) {
                Err(ContractError::InvalidContract { reason }) => assert!(
                    reason.contains(expected),
                    "{expected}: unexpected rejection: {reason}"
                ),
                other => panic!("{expected}: drift unexpectedly accepted: {other:?}"),
            }
        }

        let mut profile_drift = synthetic_bundle();
        profile_drift.profile = mutate_json(&profile_drift.profile, |profile| {
            profile["tools"]["background_output_wait_max_ms"] = json!(45_000);
        });
        rebuild_manifest(&mut profile_drift);
        assert!(matches!(
            load(&profile_drift),
            Err(ContractError::InvalidContract { reason })
                if reason.contains("background wait schema maximum")
        ));
    ...
    fn read_file_media_is_off_by_default_and_requires_semantic_safety_bounds() {
        ...
        incomplete_media.effective = mutate_json(&incomplete_media.effective, |effective| {
            effective["tools"][1]["input_schema"]["properties"]["target_file"]["type"] =
                json!("integer");
        });
        let incomplete_effective_hash = sha256_hex(&incomplete_media.effective);
        let incomplete_effective: Value = serde_json::from_slice(&incomplete_media.effective)
            .expect("parse incomplete media effective contract");
        incomplete_media.profile = mutate_json(&incomplete_media.profile, |profile| {
            profile["tools"]["read_file_media_enabled"] = json!(true);
            profile["provider"]["model"] = json!("grok-4.5");
            profile["context"]["max_request_body_bytes"] = json!(16 * 1024 * 1024);
            profile["contract_bindings"]["effective_contract_file_sha256"] =
                json!(incomplete_effective_hash);
            profile["contract_bindings"]["ordered_tools_value_sha256"] = json!(
                value_sha256(incomplete_effective.get("tools").expect("tools"))
                    .expect("tools hash")
            );
        });
    ...
    #[test]
    fn tracked_contract_has_exact_three_file_identity_and_capabilities() {
        ...
        assert_eq!(
            ...
            "9a3f9a20d2c999b09397d6c9d6a0de956836a1921efe0a337f6d9f9feef0c08a"
        );
        assert_eq!(
        ...
        let terminal = &contract.effective().tools[0];
        assert_eq!(
            terminal.input_schema["properties"]["timeout"]["maximum"],
            json!(600_000)
        );
        assert_eq!(
            contract.effective().tools[7].input_schema["properties"]["timeout_ms"]["maximum"],
            json!(30_000)
        );
        assert!(
            contract.effective().tools[1]
                .description
                .contains("PNG and JPEG")
        );
        assert!(!contract.effective().tools[1].description.contains("PDF"));
    }
}
