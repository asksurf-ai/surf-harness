...
def _mock_harbor_runtime(
    monkeypatch: pytest.MonkeyPatch, checkout: Path, running_python: Path
) -> None:
    monkeypatch.setattr(prelaunch.sys, "executable", str(running_python))
...
def test_runtime_admission_accepts_same_raw_symlink_invocation(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    checkout, lock, python, head = _runtime_checkout(tmp_path)
    selected = tmp_path / "selected-python"
    selected.symlink_to(python)
    _mock_harbor_runtime(monkeypatch, checkout, selected)

    receipt = prelaunch.admit_runtime(
        harbor_checkout=checkout.resolve(),
        runtime_python=selected.absolute(),
        runtime_python_sha256=_sha256(python),
        harbor_lock_sha256=_sha256(lock),
        expected_harbor_commit=head,
    )

    assert receipt["runtime_python"] == str(selected.absolute())
    assert receipt["runtime_python_resolved"] == str(python.resolve())
    assert receipt["runtime_python_sha256"] == _sha256(python)


def test_runtime_admission_rejects_prematurely_resolved_invocation(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    checkout, lock, python, head = _runtime_checkout(tmp_path)
    selected = tmp_path / "selected-python"
    ...
    with pytest.raises(prelaunch.PrelaunchError, match="^runtime_python_mismatch$"):
        prelaunch.admit_runtime(
            harbor_checkout=checkout.resolve(),
            runtime_python=selected.resolve(),
            runtime_python_sha256=_sha256(python),
            harbor_lock_sha256=_sha256(lock),
            expected_harbor_commit=head,
        )
...
def test_runtime_admission_rejects_distinct_symlinks_to_same_interpreter(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    checkout, lock, python, head = _runtime_checkout(tmp_path)
    selected = tmp_path / "selected-python"
    running = tmp_path / "running-python"
    selected.symlink_to(python)
    running.symlink_to(python)
    _mock_harbor_runtime(monkeypatch, checkout, running)
...
def test_runtime_admission_rejects_symlink_to_different_interpreter_target(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    checkout, lock, python, head = _runtime_checkout(tmp_path)
...
def test_runtime_admission_rejects_hash_mismatch_through_same_target_symlinks(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    checkout, lock, python, head = _runtime_checkout(tmp_path)
    selected = tmp_path / "selected-python"
    ...
    with pytest.raises(
        prelaunch.PrelaunchError, match="^runtime_python_hash_mismatch$"
    ):
        prelaunch.admit_runtime(
            harbor_checkout=checkout.resolve(),
            runtime_python=selected.absolute(),
            runtime_python_sha256="0" * 64,
            harbor_lock_sha256=_sha256(lock),
            expected_harbor_commit=head,
        )
