...
class _OfflineRemoteEnvironment:
    ...
    async def exec(self, command: str, *_args, **_kwargs) -> SimpleNamespace:
        ...
        if len(arguments) >= 3 and arguments[2] == "snapshot-start":
            control_dir = arguments[3]
            owner_token = arguments[8]
            lease = self._lease(owner_token, terminal=False)
            self._leases[control_dir] = lease
            return self._result(stdout=json.dumps(lease) + "\n")
        if len(arguments) >= 3 and arguments[2] == "snapshot-release":
            return self._result(stdout="released\n")
        if len(arguments) >= 3 and arguments[2] == "snapshot-wait":
            control_dir = arguments[3]
            owner_token = arguments[4]
        ...
        if len(arguments) >= 3 and arguments[2] == "snapshot-cancel":
            control_dir = arguments[3]
            owner_token = arguments[4]
            self._leases.pop(control_dir)
            return self._result(
        ...
        if len(arguments) >= 3 and arguments[2] == "snapshot-inspect":
