...
class _OfflineRemoteEnvironment:
    ...
    async def exec(self, command: str, *_args, **_kwargs) -> SimpleNamespace:
        ...
        if "nano-git-census." in command:
            return self._result(stdout=f"zero\t-\t{'a' * 64}\t{'b' * 64}\n")
...
def test_pinned_harbor_fake_exposes_exact_verifier_cleanup_seam(
    ...
    assert (
        ...
        == "nano-trial-lifecycle-verifier-seam-v3"
    )
...
def test_harbor_deadline_receipt_crosses_real_adapter_cli_bridge_actor_and_artifacts(
    ...
    monkeypatch.setattr(harbor_adapter, "RemoteTerminalActor", lambda _env: actor)
    agent.session_id = "session-live-e2e-001"
    agent.context_id = "context-live-e2e-001"
    ...
    async def scenario() -> bytes:
        ...
        private_receipt_path = control_root_for(logs_dir) / "runtime" / "deadline.json"
        public_receipt_path = logs_dir / "runtime" / "deadline.json"
        assert private_receipt_path.is_file()
        assert not public_receipt_path.exists()
    ...
    try:
        private_receipt_bytes = asyncio.run(scenario())
    except BaseException as error:
    ...
    assert receipt_bytes == private_receipt_bytes
