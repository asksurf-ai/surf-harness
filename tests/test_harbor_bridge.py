...
def test_harbor_context_persists_only_exact_provider_native_cost(
    ...
    monkeypatch.setattr(
        harbor_adapter, "publish_artifacts", lambda **_kwargs: publication
    )
    ...
    agent._artifact_publication_committed_v1 = False
    ...
    assert agent._artifact_publication_committed_v1 is True
    agent.populate_context_post_run(context)
...
def test_verifier_repeat_requires_in_memory_marker_commit_without_public_reread(
    tmp_path: Path,
    ...
    with pytest.raises(
        BridgeError,
        match="^git_history_publication_uncommitted$",
    ):
        asyncio.run(
    ...
    # This latch is set only by a successful marker-last publication. The
    # idempotent verifier repeat must not touch the now-deleted private tree or
    # reconstruct authority by reading mutable public evidence.
    agent._artifact_publication_committed_v1 = True
    asyncio.run(
        agent.prepare_shared_verifier_v1(
            hard_deadline_monotonic_ns=10_000_000_000,
        )
...
def test_validated_cost_survives_atif_publication_failure(
    ...
    assert agent._artifact_publication_committed_v1 is False
