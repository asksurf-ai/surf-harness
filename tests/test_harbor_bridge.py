...
def test_real_control_plane_is_removed_only_after_submission_phase(
    tmp_path: Path,
    ...
    public = tmp_path / "trial" / "agent"
    public.mkdir(parents=True)
    plane = ControlPlane.create(public, run_spec_sha256="a" * 64)
    lifecycle = TrialLifecycleCoordinator(
        root=plane.root,
        run_id="run-1",
        trial_id="trial-1",
    ...
    expected_bindings = lifecycle.receipt_binding_expectations(
        background_manifest_sha256="2" * 64,
    )
    lifecycle.prove_safety(
        identity_bound=True,
        receipt_validation=LifecycleReceiptValidation.compare(
            expected=expected_bindings,
            observed=expected_bindings,
        ),
        bridge_closed=True,
        child_process_closed=True,
        workspace_capture_complete=True,
        restore_valid=True,
        escrow_isolated=True,
        background_liveness_known=True,
        background_identity_bound=True,
        verifier_not_started=True,
        git_mode="restored",
    )

    agent = object.__new__(harbor_adapter.NanoGrokBuildAgent)
    agent._trial_lifecycle = lifecycle
    agent._control_plane = plane
    agent._post_verifier_publication_error = None
    agent._cleanup_git_history_escrow = lambda: None
    agent._publish_after_verifier = lambda: None

    async def cleanup_impl(*, hard_deadline_monotonic_ns: int) -> None:
        assert hard_deadline_monotonic_ns == 123

    agent._post_verifier_cleanup_impl = cleanup_impl

    asyncio.run(
        agent.post_verifier_finalize_v1(
            verifier_status="success",
            verifier_error=None,
            hard_deadline_monotonic_ns=123,
        )
    )

    assert lifecycle.phase is TrialPhase.SUBMISSION_READY
    assert not plane.root.exists()


def test_unsafe_terminal_and_closure_are_public_before_private_root_removal(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    ...
    from nano_grok_build.adapter.cleanup_closure import (
        ...
        BackgroundLeaseStateV1,
        HandlerCompletionStateV1,
        RuntimeCleanupFactsV1,
    )
    from nano_grok_build.adapter.control_plane import ControlPlane
    from nano_grok_build.harbor.trial_lifecycle import (
        LifecycleReceiptValidation,
    ...
    lifecycle.runtime_terminal(
        ...
        terminal_code="tool_settlement_deadline_exceeded",
        run_record_sha256="e" * 64,
        events_sha256="f" * 64,
        workspace_receipt_sha256=None,
        provider_ledger={
            "requested": 1,
            "completed": 0,
            "failed": 0,
            "cancelled": 0,
            "response_unobserved": 1,
            "outstanding": 0,
        },
        bridge_closed=False,
        child_process_closed=False,
    )
    ...
    lifecycle.prove_safety(
        ...
        bridge_closed=False,
        child_process_closed=False,
        workspace_capture_complete=False,
        restore_valid=False,
        escrow_isolated=True,
    ...
    closure = RuntimeCleanupFactsV1(
        run_id="run-1",
        trial_id="trial-1",
        attempt_id="attempt-0",
        run_spec_sha256="a" * 64,
        runtime_run_sha256="e" * 64,
        runtime_events_sha256="f" * 64,
        git_baseline_sha256="c" * 64,
        git_exposure_sha256="b" * 64,
        process_lease_sha256=None,
        background_manifest_sha256="2" * 64,
        handler_completion=HandlerCompletionStateV1.SETTLEMENT_DEADLINE,
        actor_dispatch=ActorDispatchStateV1.UNVERIFIED,
        actor_quiescent=False,
        remote_census_verified=False,
        remote_survivor_count=None,
        process_lease_sealed=False,
        background_state=BackgroundLeaseStateV1.UNVERIFIED,
        stdio_bridge_closed=False,
        runtime_child_closed=False,
    ).finalize(workspace_after_sha256=None)
    (plane.root / "runtime-cleanup-closure-v1.json").write_bytes(closure.to_bytes())

    observed_before_delete: list[bool] = []
    original_cleanup = ControlPlane.cleanup

    def cleanup_after_evidence(value: ControlPlane) -> None:
        observed_before_delete.append(
            (value.root / "phase-execution-failed-unsafe.json").is_file()
            and (value.public_root / "unsafe-execution-diagnostic-v1.json").is_file()
        )
        original_cleanup(value)

    monkeypatch.setattr(ControlPlane, "cleanup", cleanup_after_evidence)
    agent = object.__new__(harbor_adapter.NanoGrokBuildAgent)
    agent._trial_lifecycle = lifecycle
    agent._control_plane = plane
    agent._post_verifier_publication_error = None
    ...
    assert lifecycle.phase is TrialPhase.EXECUTION_FAILED_UNSAFE
    assert observed_before_delete == [True]
    assert not plane.root.exists()
    diagnostic = json.loads(
        (public / "unsafe-execution-diagnostic-v1.json").read_bytes()
    )
    assert diagnostic["terminal_phase"] == "EXECUTION_FAILED_UNSAFE"
    assert diagnostic["closure_receipt"]["verifier_permitted"] is False


def test_unsafe_diagnostic_publication_failure_retains_private_evidence(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    harbor_adapter = _load_harbor_adapter_for_workspace_receipt(monkeypatch)
    ...
    public = tmp_path / "unsafe-publication-failure" / "agent"
    public.mkdir(parents=True)
    plane = ControlPlane.create(public, run_spec_sha256="a" * 64)
    lifecycle = TrialLifecycleCoordinator(
        root=plane.root,
    ...
    agent._post_verifier_publication_error = None
    agent._cleanup_git_history_escrow = lambda: None
    agent._post_verifier_cleanup_impl = lambda **kwargs: asyncio.sleep(0)
    original_write = agent._write_immutable

    def fail_public_diagnostic(path: Path, content: bytes) -> None:
        if path.name == "unsafe-execution-diagnostic-v1.json":
            raise OSError("synthetic unsafe publication failure")
        original_write(path, content)

    agent._write_immutable = fail_public_diagnostic
    with pytest.raises(OSError, match="synthetic unsafe publication failure"):
        asyncio.run(
            agent.post_verifier_finalize_v1(
                verifier_status="error",
                verifier_error=None,
    ...
    assert plane.root.is_dir()
    assert (plane.root / "runtime-cleanup-closure-v1.json").is_file()
    assert (plane.root / "phase-execution-failed-unsafe.json").is_file()
    assert not (public / "unsafe-execution-diagnostic-v1.json").exists()


def _cleanup_race_handoff_agent(
    harbor_adapter: object,
    logs: Path,
    actor: object,
) -> object:
    ...
    agent._run_spec = {
        ...
        "run_id": "run-1",
        "trial_id": "cleanup-race-fixture",
        "attempt_id": "attempt-0",
        "task": {
            "id": "cleanup-race-fixture",
            "digest": "a" * 64,
            "instruction": "exercise terminal cleanup",
            "git_history_capability": {
                "schema_version": "nano-git-history-capability-v1",
                "policy_version": "nano-git-history-capability-policy-v1",
                "git_history_access": "absent",
                "canonical_instruction_sha256": "b" * 64,
                "trusted_manifest_sha256": "c" * 64,
                "supporting_span_sha256": "d" * 64,
            },
        },
        "contract": {
            "id": "synthetic-v1",
        ...
        "artifact_dir": str(runtime.resolve()),
        "agent_timeout_sec": 60,
    }
    agent._process_lease_v1 = None
    agent._background_manifest_handoff_v1 = None
    agent._bridge_closure_receipt_v1 = None
    agent._trial_lifecycle = None
    return agent


def _cleanup_race_safe_bridge_error(harbor_adapter: object) -> BridgeError:
    error = BridgeError("terminal_actor_cleanup_unverified")
    error.cleanup_receipt = BridgeClosureReceiptV1(
        handler_completion=harbor_adapter.HandlerCompletionStateV1.COMPLETED,
        actor_dispatch=harbor_adapter.ActorDispatchStateV1.REVOKED,
...
def test_cleanup_race_persists_reconciled_empty_lease_before_safety(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    harbor_adapter = _load_harbor_adapter_for_workspace_receipt(monkeypatch)
    ...
    class Actor:
        ...
        async def background_manifest(self) -> list[dict[str, object]]:
        ...
        @staticmethod
        def seal_process_lease_v1(rows: list[dict[str, object]]) -> ProcessLeaseV1:
            assert rows == []
            return ProcessLeaseV1(())

    logs = tmp_path / "cleanup-race-reconciled"
    actor = Actor()
    agent = _cleanup_race_handoff_agent(harbor_adapter, logs, actor)

    finalization_error = asyncio.run(
        agent._finalize_handoff(
            outcome=BridgeOutcome(request_count=15, stderr=b""),
            original_error=_cleanup_race_safe_bridge_error(harbor_adapter),
        )
    )

    assert finalization_error is None
    assert actor.cleanup_calls == 1
    assert actor.manifest_calls == 2
    manifest = json.loads((logs / "runtime-background-manifest.json").read_bytes())
    assert manifest["schema_version"] == "nano-background-manifest-v1"
    assert manifest["tasks"] == []
    lease_path = logs / "runtime-process-lease-v1.json"
    assert lease_path.is_file()
    assert hashlib.sha256(lease_path.read_bytes()).hexdigest() == (
        agent._process_lease_v1.canonical_sha256
    )
    assert agent._background_manifest_handoff_v1 is not None
    (logs / "git-history-baseline.json").write_bytes(b"{}\n")
    (logs / "git-history-exposure-v1.json").write_bytes(b"{}\n")
    (logs / "workspace-receipt.json").write_bytes(b"{}\n")
    facts = agent._derive_runtime_cleanup_facts_v1()
    background = agent._git_history_background_lifecycle_state(logs)
    assert facts.quiescence_proven is True
    assert (
        facts.process_lease_sha256
        == hashlib.sha256(lease_path.read_bytes()).hexdigest()
    )
    assert background.registered_count == 0
    assert background.live_count == 0


def test_unreconciled_background_failure_cannot_claim_cleanup_verified(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    harbor_adapter = _load_harbor_adapter_for_workspace_receipt(monkeypatch)
    ...
    class Actor:
        ...
        @staticmethod
        def seal_process_lease_v1(rows: list[dict[str, object]]) -> ProcessLeaseV1:
            raise AssertionError(f"unsafe rows cannot be sealed: {rows!r}")

    logs = tmp_path / "cleanup-race-unreconciled"
    actor = Actor()
    agent = _cleanup_race_handoff_agent(harbor_adapter, logs, actor)
    finalization_error = asyncio.run(
        agent._finalize_handoff(
    ...
    assert manifest["cleanup_attempted"] is True
    assert manifest["cleanup_verified"] is False
    assert agent._process_lease_v1 is None
    assert not (logs / "runtime-process-lease-v1.json").exists()


def test_cleanup_facts_reject_memory_only_process_lease(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    harbor_adapter = _load_harbor_adapter_for_workspace_receipt(monkeypatch)

    logs = tmp_path / "memory-only-lease"
    agent = _cleanup_race_handoff_agent(harbor_adapter, logs, object())
    (logs / "git-history-baseline.json").write_bytes(b"{}\n")
    (logs / "git-history-exposure-v1.json").write_bytes(b"{}\n")
    (logs / "runtime-background-manifest.json").write_bytes(b"{}\n")
    agent._process_lease_v1 = ProcessLeaseV1(())
    agent._bridge_closure_receipt_v1 = BridgeClosureReceiptV1(
        handler_completion=harbor_adapter.HandlerCompletionStateV1.COMPLETED,
        actor_dispatch=harbor_adapter.ActorDispatchStateV1.REVOKED,
        actor_quiescent=True,
        remote_census_verified=True,
    ...
    assert facts.process_lease_sealed is False
    assert facts.process_lease_sha256 is None
    assert facts.quiescence_proven is False


def test_background_failure_manifest_rejects_verified_unavailability(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    ...
    logs = tmp_path / "contradictory-background"
    logs.mkdir()
    harbor_adapter = _load_harbor_adapter_for_workspace_receipt(monkeypatch)
    spec = _cleanup_race_handoff_agent(harbor_adapter, logs, object())._run_spec
    ...
    with pytest.raises(ArtifactError, match="background_manifest_invalid"):
        validate_background_manifest(logs_dir=logs, run_spec=spec)


def test_harbor_context_persists_only_exact_provider_native_cost(
    ...
    publication = SimpleNamespace(
        ...
        provider_cost=SimpleNamespace(
            exact_ticks=123_000_000,
            observed_ticks=123_000_000,
            ...
            source="validated_run_record.provider_cost_ticks",
        ),
        background_manifest=SimpleNamespace(
            sha256="a" * 64,
            task_count=0,
            status="complete",
            failure_code=None,
        ...
        marker_path=tmp_path / "agent-run.json",
        usage_coverage={
            "requested": 1,
            "completed": 1,
            "failed": 0,
    ...
    runtime.mkdir()
    (runtime / "run.json").write_bytes(b"synthetic\n")
    validated = SimpleNamespace(
        provider_cost=publication.provider_cost,
        usage_coverage=publication.usage_coverage,
    )
    monkeypatch.setattr(
        harbor_adapter, "validate_runtime_artifacts", lambda **_kwargs: validated
    )
    publish_calls = 0
    ...
    def forbidden_publish(**_kwargs):
        ...
        publish_calls += 1
        raise AssertionError("pre-verifier sync cannot publish")

    monkeypatch.setattr(harbor_adapter, "publish_artifacts", forbidden_publish)

    agent = object.__new__(harbor_adapter.NanoGrokBuildAgent)
    agent.logs_dir = tmp_path
    agent._instruction = "Create the sentinel."
    agent._run_spec = {
        "run_id": "run-1",
        "trial_id": "trial-1",
        "attempt_id": "attempt-0",
    ...
    agent._control_plane = None
    agent._post_run_context = None
    agent.logger = SimpleNamespace(
        error=lambda *_args: None, exception=lambda *_args: None
    )
    ...
    first_metadata = context.metadata
    agent.populate_context_post_run(context)

    assert publish_calls == 0
    assert agent._post_run_context is context
    assert context.cost_usd == pytest.approx(0.0123)
    assert context.metadata == first_metadata
    assert context.metadata["provider_cost"] == {
        "source": "validated_run_record.provider_cost_ticks",
        "ticks_per_usd": 10_000_000_000,
        "observed_ticks": 123_000_000,
        "requested_calls": 1,
...
def test_verifier_preparation_repeat_has_no_publication_latch_dependency(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    harbor_adapter = _load_harbor_adapter_for_workspace_receipt(monkeypatch)
    ...
    agent._git_history_rehydration_outcome = ("restored", None)
    asyncio.run(
        agent.prepare_shared_verifier_v1(
            hard_deadline_monotonic_ns=10_000_000_000,
        )
    )
    asyncio.run(
        agent.prepare_shared_verifier_v1(
...
@pytest.mark.parametrize(
    ("has_escrow", "live_count", "expected", "restore_calls"),
    [
        (False, 1, "not_applicable", 0),
        (True, 1, "deferred_background_active", 0),
        (True, 0, "restored", 1),
    ],
)
def test_git_history_restore_obeys_bound_live_background_state(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
    has_escrow: bool,
    live_count: int,
    ...
    harbor_adapter = _load_harbor_adapter_for_workspace_receipt(monkeypatch)
    agent = object.__new__(harbor_adapter.NanoGrokBuildAgent)
    agent._git_history_rehydration_outcome = None
    agent._git_history_escrow = object() if has_escrow else None
    agent._process_lease_v1 = ProcessLeaseV1((object(),))
    agent._git_history_baseline_receipt = {"status": "isolated"}
    agent._actor = object()
    agent._artifact_source_dir = lambda: tmp_path
    state = harbor_adapter._BackgroundLifecycleState(
        manifest_sha256="a" * 64,
        liveness_sha256="b" * 64,
        registered_count=1,
        live_count=live_count,
    )
    agent._git_history_background_lifecycle_state = lambda _root: state
    written: list[dict[str, object]] = []
    agent._write_git_history_rehydration_receipt = lambda **kwargs: written.append(
        kwargs
    )
    agent._validate_lifecycle_receipt_bindings = lambda **_kwargs: None
    calls: list[int] = []

    async def restore(**kwargs):
        calls.append(kwargs["background_process_count"])
        return "restored"

    monkeypatch.setattr(harbor_adapter, "rehydrate_git_history_for_verifier", restore)
    monkeypatch.setattr(harbor_adapter, "host_monotonic_ns", lambda: 100)
    asyncio.run(
        agent._rehydrate_git_history_for_verifier(
            hard_deadline_monotonic_ns=1_000_000_100,
        )
    )

    assert len(calls) == restore_calls
    assert calls in ([], [0])
    assert written[0]["restore_result"] == expected
    assert written[0]["background_state"] is state
    assert agent._git_history_rehydration_outcome == (expected, None)


@pytest.mark.parametrize("receipt_failure", ("identity_mismatch", "unreadable"))
def test_persisted_lifecycle_receipt_failure_is_unsafe_and_blocks_verifier(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
    receipt_failure: str,
) -> None:
    harbor_adapter = _load_harbor_adapter_for_workspace_receipt(monkeypatch)
    monkeypatch.setattr(harbor_adapter, "rust_run_spec_sha256", lambda _spec: "a" * 64)
    lifecycle = harbor_adapter.TrialLifecycleCoordinator(
        root=tmp_path,
        run_id="run-1",
        trial_id="trial-1",
        attempt_id="attempt-0",
    ...
    lifecycle.runtime_terminal(
        terminal_status="deadline_failure",
        terminal_code="agent_deadline_exceeded",
        run_record_sha256="e" * 64,
        events_sha256="f" * 64,
        workspace_receipt_sha256="1" * 64,
    ...
    receipt = {
        "schema_version": harbor_adapter.REHYDRATION_SCHEMA,
        "policy_version": harbor_adapter.LIFECYCLE_POLICY,
        "run_spec_sha256": "a" * 64,
        "baseline_receipt_sha256": expected["baseline_receipt"],
        "exposure_receipt_sha256": expected["exposure_receipt"],
        "run_record_sha256": expected["run_record"],
        "events_sha256": expected["events"],
        "workspace_receipt_sha256": expected["workspace_receipt"],
        "background_manifest_sha256": expected["background_manifest"],
        "provider_ledger_sha256": expected["provider_ledger"],
    }
    receipt_path = tmp_path / harbor_adapter.REHYDRATION_RECEIPT
    if receipt_failure == "identity_mismatch":
        receipt["baseline_receipt_sha256"] = "9" * 64
        receipt_path.write_bytes(harbor_adapter.canonical_json(receipt))
    else:
    ...
    agent._run_spec = {}
    agent._artifact_source_dir = lambda: tmp_path
    agent._lifecycle_receipt_validation = None

    validation = agent._validate_lifecycle_receipt_bindings(
        background_manifest_sha256="2" * 64,
    )
    proof = lifecycle.prove_safety(
        identity_bound=True,
    ...
    assert lifecycle.verifier_permitted is False

    asyncio.run(lifecycle.finalize_once(lambda: asyncio.sleep(0)))
    assert lifecycle.phase is harbor_adapter.TrialPhase.EXECUTION_FAILED_UNSAFE


@pytest.mark.parametrize(
    ("projection", "expected_cost", "expected_lower_bound"),
    [
        (
            {
                "exact_ticks": 17_537_972_000,
                "observed_ticks": 17_537_972_000,
                "requested_calls": 39,
                "covered_calls": 39,
                "coverage": "complete",
                "is_complete": True,
...
def test_validated_cost_survives_atif_publication_failure(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
    projection: dict[str, object],
    expected_cost: float | None,
    expected_lower_bound: bool,
) -> None:
    harbor_adapter = _load_harbor_adapter_for_workspace_receipt(monkeypatch)
    ...
    runtime = tmp_path / "runtime"
    runtime.mkdir()
    (runtime / "run.json").write_bytes(b"validated by synthetic seam\n")
    ...
    coverage = {
        "requested": projection["requested_calls"],
        "completed": projection["covered_calls"],
        "failed": 0,
        "in_flight": (
            int(projection["requested_calls"]) - int(projection["covered_calls"])
        ),
        "usage_present": projection["covered_calls"],
        "usage_absent": 0,
        "usage_covered": projection["covered_calls"],
        "cost_present": projection["covered_calls"],
        "cost_absent": 0,
        "state": projection["coverage"],
    }
    validated = SimpleNamespace(provider_cost=cost, usage_coverage=coverage)
    validation_calls: list[tuple[Path, object]] = []

    def validate_runtime_artifacts(*, runtime_dir, run_spec):
    ...
    def fail_atif(**kwargs):
        assert kwargs["validated_runtime"] is validated
        raise RuntimeError("atif_failed_provider_call")

    monkeypatch.setattr(harbor_adapter, "publish_artifacts", fail_atif)
    agent = object.__new__(harbor_adapter.NanoGrokBuildAgent)
    agent.logs_dir = tmp_path
    agent._instruction = "Build the requested artifact."
    agent._run_spec = {
        "run_id": "run-1",
        "trial_id": "trial-1",
        "attempt_id": "attempt-0",
    ...
    assert validation_calls == [(runtime, agent._run_spec)]
    assert context.cost_usd == expected_cost
    assert context.metadata["provider_cost"]["is_lower_bound"] is (expected_lower_bound)
    assert (
        context.metadata["provider_cost"]["observed_ticks"]
        == projection["observed_ticks"]
    )
    assert agent._post_run_context is context


def test_real_harbor_finally_sync_refuses_marker_after_prepare_failure(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """Model SingleStepTrial._run_agent's unconditional finally sync exactly."""
    ...
    (runtime / "run.json").write_bytes(b"validated by synthetic seam\n")
    cost = SimpleNamespace(
        exact_ticks=10_000_000_000,
        observed_ticks=10_000_000_000,
        ticks_per_usd=10_000_000_000,
        requested_calls=1,
        covered_calls=1,
        coverage="complete",
    ...
    monkeypatch.setattr(
        harbor_adapter,
        "validate_git_history_publication_admission",
        lambda **_kwargs: "started",
        raising=False,
    )
    publish_calls = 0

    def publish_marker(**_kwargs):
        nonlocal publish_calls
        publish_calls += 1
        (tmp_path / "agent-run.json").write_bytes(b"forbidden marker\n")
        raise AssertionError("publication must not run after failed preparation")

    monkeypatch.setattr(harbor_adapter, "publish_artifacts", publish_marker)
    prepare_error = BridgeError("git_history_rehydration_invalid")
    agent = object.__new__(harbor_adapter.NanoGrokBuildAgent)
    agent.logs_dir = tmp_path
    agent._instruction = "Build the requested artifact."
    agent._run_spec = {
    ...
    agent._control_plane = None
    agent._post_run_context = None
    agent._git_history_rehydration_outcome = (None, prepare_error)
    agent.logger = SimpleNamespace(
        error=lambda *_args: None,
        exception=lambda *_args: None,
    )
    ...
    async def real_harbor_shape() -> None:
        try:
            raise prepare_error
        finally:
            # Harbor 0.20 calls this from `_run_agent()` even when the wrapped
            # `_run_agent_phase()` raised during pre-publication preparation.
            agent.populate_context_post_run(context)

    with pytest.raises(BridgeError, match="^git_history_rehydration_invalid$"):
        asyncio.run(real_harbor_shape())

    assert publish_calls == 0
    assert not (tmp_path / "agent-run.json").exists()
    assert agent._post_run_context is context
...
@pytest.mark.parametrize(
    "task_shape",
    ["local-fixture-a", "local-fixture-b", "local-fixture-c"],
)
def test_live_bridge_joins_same_handler_past_one_second_until_signed_settlement(
    tmp_path: Path,
    task_shape: str,
) -> None:
    ...
    deadline = RunDeadlineV1.mint(
        source="test_host_phase",
        agent_timeout_ms=76_000,
        now_monotonic_ns=now_ns,
    )
    receipt = RunDeadlineReceiptV1.bind(
        deadline=deadline,
        run_id=f"run-{task_shape}",
        trial_id=f"trial-{task_shape}",
        attempt_id="attempt-0",
        run_spec_sha256="a" * 64,
    )
    value = live_request_value(background=False)
    value.update(
        {
            ...
            "last_send_monotonic_ns": receipt.cutoffs.last_send_monotonic_ns,
            "runtime_final_monotonic_ns": (receipt.cutoffs.runtime_final_monotonic_ns),
            "cleanup_start_monotonic_ns": (receipt.cutoffs.cleanup_start_monotonic_ns),
            "hard_deadline_monotonic_ns": (receipt.cutoffs.hard_deadline_monotonic_ns),
    ...
    raw = json.dumps(value, separators=(",", ":")).encode()
    child = tmp_path / f"live-{task_shape}.py"
    child.write_text(
        "import json,sys\n"
        f"raw={raw!r}\n"
        "sys.stdout.buffer.write(raw+b'\\n');sys.stdout.buffer.flush()\n"
    ...
    class DelayedSettlementHandler(Handler):
        def __init__(self) -> None:
            super().__init__()
            self.contained = False
            self.cleanup_observed_containment = False
        ...
        async def execute(self, request):
            ...
            try:
                ...
                # Deliberately exceeds the removed one-second shortcut while
                # remaining well inside the signed ten-second settlement span.
                await asyncio.sleep(1.1)
                self.contained = True
                raise

        async def cleanup_active_until(self, hard_deadline_monotonic_ns):
            assert hard_deadline_monotonic_ns == (
                receipt.cutoffs.hard_deadline_monotonic_ns
            )
            self.cleanup_observed_containment = self.contained
            return self.contained

    handler = DelayedSettlementHandler()
    with pytest.raises(
        BridgeError,
        ...
        asyncio.run(
            asyncio.wait_for(
                run_stdio_bridge(
                    [sys.executable, str(child)],
                    handler,
                    task_image=task_shape,
                    trial_family=task_shape,
    ...
    closure = BridgeClosureReceiptV1.from_mapping(caught.value.cleanup_receipt)
    assert handler.contained is True
    assert handler.cleanup_observed_containment is True
    assert closure.handler_completion.value == "settlement_deadline"
    assert closure.remote_survivor_count == 0
    assert closure.process_safe is True
...
def test_harbor_proxy_clears_git_redirection_for_every_actor_shell(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    from types import ModuleType, SimpleNamespace
    ...
    git("config", "user.email", "environment@example.invalid")
    secret = "EXTERNAL_ALTERNATE_ORACLE_BYTES_841d"
    (workspace / "parser.py").write_text(secret + "\n")
    git("add", "parser.py")
    git("commit", "-qm", "old oracle")
    old_blob = git("rev-parse", "HEAD:parser.py").stdout.strip()
    (workspace / "parser.py").write_text("current baseline\n")
    git("add", "parser.py")
    git("commit", "-qm", "current baseline")
    external_objects = tmp_path / "external-objects"
    shutil.copytree(workspace / ".git" / "objects", external_objects)
    ...
    artifact_dir.mkdir()
    capability = compile_git_history_capability("Fix the current parser.", "a" * 64)

    async def scenario() -> None:
        await isolate_preexisting_git_history(
            actor=actor,
            artifact_dir=artifact_dir,
            capability=capability,
            run_spec_sha256="b" * 64,
        )
        environment_probe = await actor.exec_snapshot(
            "env | LC_ALL=C sort", timeout_sec=5
        )
        for name in injected:
            assert f"{name}=" not in environment_probe.stdout
        old_bytes = await actor.exec_snapshot(
            f"git cat-file -p {old_blob}", timeout_sec=5
        )
        assert old_bytes.return_code != 0
        assert secret not in old_bytes.stdout
        assert secret not in old_bytes.stderr

    asyncio.run(scenario())


def test_harbor_setup_isolates_real_disposable_repo_before_capture(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    from types import ModuleType, SimpleNamespace

    from nano_grok_build.adapter.control_plane import control_root_for
    from nano_grok_build.harbor.git_history_capability import (
        compile_git_history_capability,
    )
    from nano_grok_build.harbor.git_history_isolation import (
    ...
    secret = "PREEXISTING_SETUP_ORACLE_BYTES_07d6"
    (workspace / "parser.py").write_text(secret + "\n")
    git("add", "parser.py")
    git("commit", "-qm", "old oracle")
    (workspace / "parser.py").write_text("current baseline\n")
    ...
    instruction = "Fix the current parser."
    capability = compile_git_history_capability(instruction, "a" * 64)
    events: list[str] = []

    class OrderedActor:
        async def setup(self) -> None:
            events.append("actor_setup")

        def snapshot_workspace_root(self) -> str:
            return str(workspace.resolve())

        async def exec_snapshot(self, command: str, *, timeout_sec: float) -> object:
            if "nano-git-census" in command:
                events.append("topology")
            elif "tar -cf" in command:
                events.append("escrow_archive")
            elif "nano-git-guard" in command:
                events.append("guard")
            elif "status=isolated" in command:
                events.append("isolate")
            else:
                events.append("escrow_remote_delete")
            result = subprocess.run(
                ["/bin/bash", "-c", command],
                cwd=workspace,
                check=False,
        ...
        async def download_snapshot(
            ...
            events.append("escrow_download")
            shutil.copyfile(source_path, target_path)

    actor = OrderedActor()
    monkeypatch.setattr(
        harbor_adapter, "RemoteTerminalActor", lambda _environment: actor
    )

    async def ordered_capture(target: object, _policy: object) -> object:
        events.append("capture_before")
        assert getattr(target, "actor") is actor
        assert (control_root / HISTORY_BASELINE_RECEIPT).is_file()
        exposure = json.loads(
            (control_root / "git-history-exposure-v1.json").read_bytes()
        )
        assert exposure["schema_version"] == "nano-git-history-exposure-v1"
        assert exposure["archive_state"] == "controller_private"
        assert exposure["agent_dispatch_started"] is False
        assert exposure["agent_remote_archive_absent"] is True
        assert git("rev-list", "--all", "--count").stdout.strip() == "1"
        assert secret not in git("log", "-p", "--all").stdout
        return SimpleNamespace(status="complete")

    monkeypatch.setattr(harbor_adapter, "capture_before", ordered_capture)
    agent = object.__new__(harbor_adapter.NanoGrokBuildAgent)
    agent.logs_dir = logs_dir
    agent.session_id = "session"
    agent.context_id = "context"
    agent._run_spec = {
        "schema_version": "nano-run-spec-alpha-2",
        "run_id": "run-setup-order",
        "trial_id": "trial-setup-order",
        "attempt_id": "attempt-0",
        "task": {
            "id": "synthetic",
            "digest": "a" * 64,
            "instruction": instruction,
            "git_history_capability": capability,
        },
        "contract": {
            "id": "synthetic-v1",
        ...
        "workspace_dir": "/workspace",
        "artifact_dir": str((control_root / "runtime").resolve()),
        "agent_timeout_sec": 60,
    }

    asyncio.run(agent.setup(object()))  # type: ignore[arg-type]

    assert events == [
        "actor_setup",
        "topology",  # escrow admission
        "escrow_archive",
        "escrow_download",
        "escrow_remote_delete",
        "topology",  # isolation topology/manifest before
        "isolate",  # admitted-repo transform
        "topology",  # isolation topology/manifest after
        "guard",  # exact post-isolation semantic state
        "capture_before",
    ]
...
def test_harbor_success_hands_off_background_but_diagnostic_failure_cleans(
    ...
    async def bridge(*args, **kwargs):
        assert kwargs["task_image"] == "task"
        assert kwargs["trial_family"] == "task"
    ...
    receipt_error = BridgeError(
        ...
        cleanup_receipt={
            "schema_version": "external-bridge-cleanup-receipt-v1",
            "failed_component": "cleanup",
        },
    )
    receipt_payload = json.loads(
        agent(tmp_path / "receipt", Actor())._stderr_receipt(None, receipt_error)
    )
    assert receipt_payload["bridge_failure_receipt"]["return_code"] == 17
    assert receipt_payload["bridge_cleanup_receipt"]["failed_component"] == "cleanup"
    assert "private diagnostic" not in json.dumps(receipt_payload)
    ...
    async def cancelled_bridge(*args, **kwargs):
        ...
        try:
            ...
            # This injected bridge has no child or remote dispatch.  Supply the
            # structural proof the production bridge now always emits.
            error.cleanup_receipt = {
                "schema_version": "external-bridge-closure-v2",
                "handler_completion": "completed",
                "actor_dispatch": "revoked",
                "actor_quiescent": True,
                "remote_census_verified": True,
                "remote_survivor_count": 0,
                "stdio_bridge_closed": True,
    ...
    assert cancel_actor.cleaned is False
    assert cancel_actor.sealed is True
    assert cancel_actor.liveness_calls == 1
    ...
    assert cancelled_manifest["tasks"][0]["task_id"] == task_id
    assert (cancel_logs / "runtime-background-liveness-v1.json").is_file()
    ...
    async def isolate_fixture(**kwargs):
        receipt = {"status": "fixture"}
        harbor_adapter.NanoGrokBuildAgent._write_immutable(
            Path(kwargs["artifact_dir"]) / "git-history-baseline.json",
            harbor_adapter.canonical_json(receipt),
        )
        return receipt

    async def no_escrow_fixture(**_kwargs):
        return None
