...
def test_harbor_setup_isolates_real_disposable_repo_before_capture(
    ...
    assert events == [
        "actor_setup",
        "isolate",  # topology/manifest before
        "isolate",  # admitted-repo transform
        "isolate",  # topology/manifest after
        "capture_before",
    ]
...
def test_harbor_success_hands_off_background_but_diagnostic_failure_cleans(
    ...
    async def isolate_fixture(**_kwargs):
...
@pytest.mark.parametrize(
    ("exact_cost", "observed_cost", "expected_cost", "is_lower_bound"),
    [
        (0.0123, 0.0123, 0.0123, False),
        (None, 0.01, None, True),
    ],
)
def test_post_run_projects_exact_cost_and_labels_partial_lower_bound(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
    ...
    publication_context: dict[str, int | float] = {
        "n_input_tokens": 13,
        "n_cache_tokens": 2,
        "n_output_tokens": 5,
    }
    if exact_cost is not None:
        publication_context["cost_usd"] = exact_cost
    publication = SimpleNamespace(
        ...
        marker_path=Path("agent-run.json"),
        usage_coverage={"requested": 2, "cost_present": 1},
    ...
    agent._instruction = "Finish the task."
    ...
    agent._run_spec = {
        ...
        "artifact_dir": str((tmp_path / "runtime").resolve()),
    }
    ...
    assert context.metadata["provider_cost_usd_observed_lower_bound"] == pytest.approx(
        observed_cost
    )
    assert context.metadata["provider_cost_usd_is_lower_bound"] is is_lower_bound
