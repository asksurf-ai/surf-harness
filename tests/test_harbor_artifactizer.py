...
def test_private_source_publication_is_allowlisted_marker_last_and_conflict_closed(
    ...
    rehydration_bytes = b'{"schema_version":"synthetic-rehydration-proof-v1"}\n'
    (plane.root / "git-history-rehydration-v1.json").write_bytes(rehydration_bytes)
    ...
    assert (
        public / "git-history-rehydration-v1.json"
    ).read_bytes() == rehydration_bytes
    ...
    assert (
        publication.marker_path.stat().st_mtime_ns
        >= (public / "git-history-rehydration-v1.json").stat().st_mtime_ns
    )
...
def test_v2_success_keeps_atif_and_publishes_v2_usage_marker(tmp_path: Path) -> None:
    ...
    assert publication.provider_cost.exact_ticks == 17
    assert publication.provider_cost.observed_ticks == 17
    assert publication.provider_cost.requested_calls == 1
    assert publication.provider_cost.covered_calls == 1
    assert publication.provider_cost.coverage == "complete"
    assert publication.provider_cost.is_complete is True
    assert publication.provider_cost.is_lower_bound is False
    assert (
        publication.provider_cost.source == "validated_run_record.provider_cost_ticks"
    )
...
def test_v2_missing_cost_coverage_never_invents_exact_harbor_cost(
    tmp_path: Path,
) -> None:
    logs = tmp_path / "agent"
    spec = run_spec(logs)
    ...
    assert publication.provider_cost.exact_ticks is None
    assert publication.provider_cost.observed_ticks is None
    assert publication.provider_cost.requested_calls == 1
    assert publication.provider_cost.covered_calls == 0
    assert publication.provider_cost.coverage == "unavailable"
    assert publication.provider_cost.is_complete is False
    assert publication.provider_cost.is_lower_bound is False


def test_provider_cost_projection_preserves_partial_and_zero_call_semantics() -> None:
    partial = artifactizer._provider_cost_projection(
        {
            "requested": 2,
            "completed": 1,
            "failed": 0,
    ...
    assert partial.exact_ticks is None
    assert partial.observed_ticks == 17
    assert partial.coverage == "partial"
    assert partial.is_complete is False
    assert partial.is_lower_bound is True

    zero = artifactizer._provider_cost_projection(
        {
            "requested": 0,
            "completed": 0,
            "failed": 0,
    ...
    assert zero.exact_ticks is None
    assert zero.observed_ticks is None
    assert zero.coverage == "unavailable"
    assert zero.is_complete is False
    assert zero.is_lower_bound is False


def test_bound_runtime_validation_is_reused_without_second_event_scan(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    logs = tmp_path / "agent"
    ...
    write_success_runtime_v2(logs, spec)
    validation_calls = 0
    original = artifactizer._validate_run_and_events

    def counted(runtime_dir, expected_spec):
        nonlocal validation_calls
        validation_calls += 1
        return original(runtime_dir, expected_spec)

    monkeypatch.setattr(artifactizer, "_validate_run_and_events", counted)
    validated = artifactizer.validate_runtime_artifacts(
        runtime_dir=logs / "runtime",
        run_spec=spec,
    )
    publication = publish_artifacts(
        ...
        model_name="synthetic-model",
        require_harbor_validator=False,
        validated_runtime=validated,
    )

    assert validation_calls == 1
    assert publication.provider_cost is validated.provider_cost
    assert publication.provider_cost.exact_ticks == 17


def test_required_lifecycle_admission_fails_before_marker_publication(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    logs = tmp_path / "agent"
    ...
    def reject(**_kwargs):
        raise RuntimeError("git_history_lifecycle_missing")
    ...
    with pytest.raises(ArtifactError, match="^git_history_publication_invalid$"):
        publish_artifacts(
            logs_dir=logs,
            run_spec=spec,
            instruction="Create the sentinel.",
            ...
            model_name="synthetic-model",
            require_harbor_validator=False,
            require_git_history_lifecycle=True,
        )
    assert not (logs / "agent-run.json").exists()
