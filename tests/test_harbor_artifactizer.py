...
def test_v2_success_keeps_atif_and_publishes_v2_usage_marker(tmp_path: Path) -> None:
    ...
    assert publication.context == {
        ...
        "n_cache_tokens": 2,
        "n_output_tokens": 5,
        "cost_usd": pytest.approx(0.0000000017),
    }
...
def test_context_checkpoint_receipt_is_closed_hash_bound_and_monotonic() -> None:
    valid = {
        "policy_version": "fresh-context-checkpoint-v1",
        "source_history_sha256": "a" * 64,
        "checkpoint_history_sha256": "b" * 64,
        "source_history_items": 32,
        "checkpoint_history_items": 3,
        "provider_turn_count": 15,
        "tool_call_count": 15,
        "observed_input_tokens": 300_000,
    }
    artifactizer._validate_v2_event_data("context.checkpointed", valid)

    for patch in (
        {"policy_version": "fresh-context-checkpoint-v2"},
        {"source_history_sha256": "A" * 64},
        {"checkpoint_history_items": 32},
        ...
        invalid = {**valid, **patch}
        with pytest.raises(ArtifactError):
            artifactizer._validate_v2_event_data("context.checkpointed", invalid)


def test_semantic_checkpoint_receipts_and_prepare_source_are_closed() -> None:
    semantic = {
        ...
        "prepare_history_sha256": "d" * 64,
        "action_turn_cutoff": 20,
        "action_lease_ms": 299_000,
        "tail_reserve_ms": 900_000,
    }
    artifactizer._validate_v2_event_data("context.checkpointed", semantic)

    for patch in (
        {"capsule_bytes": 8193},
        {"prepare_turn_index": 13},
        {"action_turn_cutoff": 13},
        {"action_lease_ms": 0},
        {"tail_reserve_ms": 899_999},
    ):
        with pytest.raises(ArtifactError, match="event_data_invalid"):
            artifactizer._validate_v2_event_data(
                "context.checkpointed", {**semantic, **patch}
            )

    prepare = {
        "turn_index": 12,
        "history_item_count": 33,
        "tool_count": 0,
        "function_output_call_ids": ["call-old"],
        ...
        "checkpoint_source_history_sha256": "a" * 64,
    }
    artifactizer._validate_v2_event_data("provider.requested", prepare)
    inline = copy.deepcopy(prepare)
    inline["tool_count"] = 9
    inline["budget_observation"]["phase"] = "checkpoint_inline"
    artifactizer._validate_v2_event_data("provider.requested", inline)
    with pytest.raises(ArtifactError, match="event_data_invalid"):
        artifactizer._validate_v2_event_data(
            "provider.requested", {**inline, "tool_count": 0}
        )
    for field in ("checkpoint_source_history_sha256", "budget_observation"):
        invalid = {key: value for key, value in prepare.items() if key != field}
        with pytest.raises(ArtifactError, match="event_data_invalid"):
            artifactizer._validate_v2_event_data("provider.requested", invalid)
    ...
    artifactizer._validate_v2_event_data("context.checkpoint_rejected", rejection)
    rejection_diagnostic = {
        **rejection,
        "capsule_content_sha256": "e" * 64,
        "capsule_content_bytes": 3,
        "capsule_content_excerpt": "bad",
    }
    artifactizer._validate_v2_event_data(
        "context.checkpoint_rejected", rejection_diagnostic
    )
    for patch in (
        {"capsule_content_bytes": 2},
        {"capsule_content_excerpt": ""},
        {"capsule_content_sha256": None},
    ):
        with pytest.raises(ArtifactError):
            artifactizer._validate_v2_event_data(
                "context.checkpoint_rejected", {**rejection_diagnostic, **patch}
            )
    with pytest.raises(ArtifactError, match="event_data_fields_invalid"):
        artifactizer._validate_v2_event_data(
            "context.checkpoint_rejected",
            {**rejection, "capsule_content_sha256": "e" * 64},
        )
    with pytest.raises(ArtifactError, match="event_data_invalid"):
        artifactizer._validate_v2_event_data(
            "context.checkpoint_rejected",
            {**rejection, "prepare_turn_index": 13},
        )


def test_completion_review_provenance_is_closed_and_paired() -> None:
    base = {
        "task_id": "task",
    ...
    semantic = {
        **base,
        "completion_review_policy": "semantic-checkpoint-v6",
        "context_checkpoint_policy_version": "semantic-context-checkpoint-v1",
        "checkpoint_capsule_schema_version": "semantic-checkpoint-capsule-v1",
    }
    artifactizer._validate_v2_event_data("run.started", semantic)
    artifactizer._validate_v2_event_data(
        "run.started",
        {**semantic, "completion_review_policy": "semantic-checkpoint-v7"},
    )
    artifactizer._validate_v2_event_data(
        "run.started",
        {**semantic, "completion_review_policy": "semantic-checkpoint-v8"},
    )
    artifactizer._validate_v2_event_data(
        "run.started",
        {**semantic, "completion_review_policy": "semantic-checkpoint-v5"},
    )
    with pytest.raises(ArtifactError, match="event_data_invalid"):
        artifactizer._validate_v2_event_data(
            "run.started",
            {
                ...
                if key != "checkpoint_capsule_schema_version"
            },
        )


def test_semantic_checkpoint_relation_binds_prepare_source_and_four_item_reset(
    tmp_path: Path,
) -> None:
    spec = run_spec(tmp_path)
    spec["provider"]["max_turns"] = 64
    spec_sha = rust_run_spec_sha256(spec)
    started = {
        "task_id": "synthetic-task",
        "contract_id": "synthetic-v1",
        "profile_id": "synthetic-profile-v1",
        ...
        "media_history_policy_version": "rolling-media-history-latest-suffix-v1",
        "media_history_policy_sha256": MEDIA_HISTORY_POLICY_SHA256,
        "completion_review_policy": "semantic-checkpoint-v5",
        "context_checkpoint_policy_version": "semantic-context-checkpoint-v1",
        "checkpoint_capsule_schema_version": "semantic-checkpoint-capsule-v1",
    }
    request_zero = {
        "turn_index": 0,
        "history_item_count": 32,
        "tool_count": 8,
        "function_output_call_ids": ["call-old"],
        "media_history_receipt": {"history_sha256": "0" * 64},
    }
    prepare = {
        "turn_index": 1,
        "history_item_count": 33,
        "tool_count": 0,
        "function_output_call_ids": ["call-old"],
        "media_history_receipt": {"history_sha256": "d" * 64},
        "budget_observation": {"phase": "checkpoint_prepare"},
        "checkpoint_source_history_sha256": "a" * 64,
    }
    checkpoint = {
        "policy_version": "semantic-context-checkpoint-v1",
        ...
        "provider_turn_count": 2,
        "tool_call_count": 0,
        "observed_input_tokens": 250_100,
        "capsule_schema_version": "semantic-checkpoint-capsule-v1",
        "capsule_sha256": "c" * 64,
        "capsule_bytes": 1024,
        "prepare_turn_index": 1,
        "prepare_history_sha256": "d" * 64,
    ...
    events = [
        {"seq": 0, "type": "run.started", "data": started},
        {"seq": 1, "type": "provider.requested", "data": request_zero},
        {
            "seq": 2,
            "type": "provider.completed",
            "data": {"turn_index": 0, "usage": {"input_tokens": 250_000}},
        },
        {"seq": 3, "type": "provider.requested", "data": prepare},
        {
            "seq": 4,
            "type": "provider.completed",
            "data": {"turn_index": 1, "usage": {"input_tokens": 100}},
        },
        {"seq": 5, "type": "context.checkpointed", "data": checkpoint},
        {
            "seq": 6,
            "type": "provider.requested",
            "data": {
                ...
                "tool_count": 8,
                "function_output_call_ids": [],
                "media_history_receipt": {"history_sha256": "b" * 64},
            },
        },
        {
            "seq": 7,
            "type": "provider.completed",
            "data": {"turn_index": 2, "usage": {"input_tokens": 10}},
        },
        {"seq": 8, "type": "assistant.final", "data": {"text": "done"}},
        {"seq": 9, "type": "run.completed", "data": {"code": "completed"}},
    ]
    record = {
        "run_spec_sha256": spec_sha,
        "deadline_receipt_sha256": None,
        "provider_call_coverage": {"completed": 3, "failed": 0},
        "provider_turn_count": 3,
        "tool_call_count": 0,
        "terminal_status": "success",
        "terminal_phase": None,
    }
    artifactizer._validate_v2_event_relations(events, record, spec, {})

    inline_events = copy.deepcopy(events)
    inline_events[0]["data"]["completion_review_policy"] = "semantic-checkpoint-v6"
    inline_events[3]["data"]["tool_count"] = 9
    inline_events[3]["data"]["budget_observation"]["phase"] = "checkpoint_inline"
    inline_events[5:5] = [
        {
            "type": "tool.registered",
            "data": {
                "call_id": "call-inline-action",
                "provider_name": "run_terminal_command",
                "known": True,
                "arguments_json": "{}",
            },
    ...
    for seq, event in enumerate(inline_events):
        event["seq"] = seq
    inline_events[8]["data"]["tool_call_count"] = 1
    inline_record = {**record, "tool_call_count": 1}
    artifactizer._validate_v2_event_relations(inline_events, inline_record, spec, {})

    late_events = copy.deepcopy(inline_events)
    late_events[0]["data"]["completion_review_policy"] = "semantic-checkpoint-v7"
    late_events[-4]["data"]["budget_observation"] = {"phase": "completion_late_review"}
    artifactizer._validate_v2_event_relations(late_events, inline_record, spec, {})

    late_v8_events = copy.deepcopy(late_events)
    late_v8_events[0]["data"]["completion_review_policy"] = "semantic-checkpoint-v8"
    artifactizer._validate_v2_event_relations(late_v8_events, inline_record, spec, {})

    mislabeled_late = copy.deepcopy(late_events)
    mislabeled_late[0]["data"]["completion_review_policy"] = "semantic-checkpoint-v6"
    with pytest.raises(ArtifactError, match="completion_late_review_relation_invalid"):
        artifactizer._validate_v2_event_relations(
            mislabeled_late, inline_record, spec, {}
        )

    mislabeled_inline = copy.deepcopy(inline_events)
    mislabeled_inline[0]["data"]["completion_review_policy"] = "semantic-checkpoint-v5"
    with pytest.raises(
        ArtifactError, match="context_checkpoint_prepare_relation_invalid"
    ):
        artifactizer._validate_v2_event_relations(
            mislabeled_inline, inline_record, spec, {}
        )

    unresolved = copy.deepcopy(inline_events)
    unresolved[5:8] = [{"type": "assistant.final", "data": {"text": "early"}}]
    for seq, event in enumerate(unresolved):
        event["seq"] = seq
    with pytest.raises(ArtifactError, match="context_checkpoint_resolution_missing"):
        artifactizer._validate_v2_event_relations(unresolved, inline_record, spec, {})

    corrupted = copy.deepcopy(events)
    corrupted[5]["data"]["prepare_history_sha256"] = "e" * 64
    with pytest.raises(
        ArtifactError, match="context_checkpoint_prepare_relation_invalid"
    ):
        artifactizer._validate_v2_event_relations(corrupted, record, spec, {})

    wrong_lease = copy.deepcopy(events)
    wrong_lease[5]["data"]["action_turn_cutoff"] = 39
    with pytest.raises(
        ArtifactError, match="context_checkpoint_prepare_relation_invalid"
    ):
        artifactizer._validate_v2_event_relations(wrong_lease, record, spec, {})
...
def test_harbor_cost_projection_keeps_partial_cost_as_lower_bound() -> None:
    exact, lower_bound = artifactizer._harbor_cost_projection(
        {
            "requested": 2,
            "completed": 2,
            "failed": 0,
    ...
    assert lower_bound == pytest.approx(0.0123)
