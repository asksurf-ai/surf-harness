...
class MailmanShapeForegroundRemote:
    """Expose exit-76 evidence, then one later same-owner containment result."""

    def __init__(
        self,
        *,
        later_cleanup_verified: bool,
        run_transport_lost: bool = False,
    ) -> None:
        self.later_cleanup_verified = later_cleanup_verified
        self.run_transport_lost = run_transport_lost
        self.exec_calls: list[tuple[str, str | None, float | None, object]] = []
        self.uploads: list[str] = []
    ...
    async def exec(self, command, cwd=None, timeout_sec=None, user=None):
        self.exec_calls.append((command, cwd, timeout_sec, user))
        if command.startswith("/bin/bash -c ") or command.startswith("mkdir -p "):
            return SimpleNamespace(return_code=0, stdout="", stderr="")
        if "/actor.sh run " in command:
            if self.run_transport_lost:
                raise TimeoutError("synthetic post-dispatch transport loss")
            return SimpleNamespace(
                return_code=76,
                stdout="",
                stderr="foreground cleanup was not yet verified",
            )
        if command.startswith("if test -f ") and "/meta.json" in command:
            return SimpleNamespace(return_code=0, stdout="", stderr="")
        if "/actor.sh cleanup " in command:
            return SimpleNamespace(
                return_code=0 if self.later_cleanup_verified else 73,
                stdout=("cleanup-ok\n" if self.later_cleanup_verified else ""),
                stderr="",
            )
        raise AssertionError(f"unexpected remote command: {command}")
    ...
    async def download_file(self, source_path, target_path):
        ...
        payload = {
            ...
            "stdout.bin": b"foreground setup completed\n",
            "stderr.bin": b"",
        }[name]
        Path(target_path).write_bytes(payload)
...
def test_snapshot_transport_exposes_only_canonical_bounded_operations(
    ...
    source = (tmp_path / "host-escrow").resolve()
    source.write_bytes(b"escrow")
    asyncio.run(actor.upload_snapshot(source, "/tmp/nano-restore.tar"))
    assert environment.uploads == [(source, "/tmp/nano-restore.tar")]


def test_snapshot_upload_rejects_symlink_and_noncanonical_remote_path(
    tmp_path: Path,
) -> None:
    ...
    for local, remote in [
        (link, "/tmp/archive"),
        (source, "relative/archive"),
        (source, "/tmp/../archive"),
    ]:
        with pytest.raises(BridgeError, match="^terminal_actor_snapshot_path_invalid$"):
            asyncio.run(actor.upload_snapshot(local, remote))
...
def test_snapshot_owned_recovers_one_transient_terminal_json_parse_failure() -> None:
    class TransientTerminalJsonRemote(OwnedSnapshotRemote):
        failed_once = False
        ...
        async def exec(self, command, cwd=None, timeout_sec=None, user=None):
            ...
            if located is not None and located[0] == "snapshot-cancel":
                self.exec_calls.append((command, cwd, timeout_sec, user))
                token = parts[located[1] + 2]
                return SimpleNamespace(
                    return_code=0,
                    stdout=json.dumps(self._terminal(token), separators=(",", ":"))
                    + "\n",
                    stderr="",
                )
            return await super().exec(
    ...
    environment = TransientTerminalJsonRemote()
    actor = _owned_snapshot_actor(environment)
...
def test_snapshot_owned_repeated_terminal_json_parse_failure_remains_fatal() -> None:
    class InvalidTerminalJsonRemote(OwnedSnapshotRemote):
        async def exec(self, command, cwd=None, timeout_sec=None, user=None):
            parts = shlex.split(command)
            located = self._mode(parts)
            if located is not None and located[0] in {
                "snapshot-wait",
                "snapshot-cancel",
            }:
                self.exec_calls.append((command, cwd, timeout_sec, user))
    ...
    actor = _owned_snapshot_actor(InvalidTerminalJsonRemote())
...
def test_foreground_actor_publishes_canonical_unsafe_meta_before_exit_76() -> None:
    cleanup_guard = 'if [ "$cleanup_verified" != "true" ]; then'
    guard_offset = _ACTOR.index(cleanup_guard)
    exit_offset = _ACTOR.index("  exit 76", guard_offset)
    unsafe_branch = _ACTOR[guard_offset:exit_offset]

    assert ".meta.json.tmp" in unsafe_branch
    assert "meta.json" in unsafe_branch
    assert 'sync "$request_dir/.meta.json.tmp"' in unsafe_branch
    assert 'mv "$request_dir/.meta.json.tmp" "$request_dir/meta.json"' in (
        unsafe_branch
    )


@pytest.mark.parametrize(
    ("run_transport_lost", "expected_origin", "expected_primary"),
    [
        (False, "protocol", "run_response_nonzero"),
        (True, "transport", "run_transport_timeout"),
    ],
)
@pytest.mark.parametrize("later_cleanup_verified", [True, False])
def test_cleanup_race_preserves_primary_cleanup_fatal_and_refines_only_closure(
    later_cleanup_verified: bool,
    run_transport_lost: bool,
    expected_origin: str,
    expected_primary: str,
) -> None:
    environment = MailmanShapeForegroundRemote(
        later_cleanup_verified=later_cleanup_verified,
    ...
    failure = caught.value.failure
    assert failure.code == "terminal_actor_cleanup_unverified"
    assert failure.cleanup_verified is later_cleanup_verified
    assert failure.census_verified is (True if later_cleanup_verified else False)
    assert failure.actor_receipt is not None
    assert failure.actor_receipt.origin.value == expected_origin
    assert failure.actor_receipt.primary_subtype.value == expected_primary
    assert failure.actor_receipt.recovery_subtype is not None
    assert failure.actor_receipt.recovery_subtype.value == "cleanup_unverified"
    assert failure.actor_receipt.cleanup_verified is later_cleanup_verified
    assert failure.actor_receipt.census_verified is (
        True if later_cleanup_verified else False
    )
    encoded = json.loads(encode_tool_response(tool_request, failure))
    assert encoded["failure"]["code"] == "terminal_actor_cleanup_unverified"
    cleanup_calls = [
        command
        for command, *_ in environment.exec_calls
        if "/actor.sh cleanup " in command
    ...
    if later_cleanup_verified:
        assert actor.settlement_state_v1() == {
            "schema_version": "terminal-actor-settlement-state-v1",
        ...
        assert actor.settlement_state_v1()["actor_quiescent"] is False
...
def test_background_launch_uses_uuid_renderer_and_pinned_timeout_semantics(
    ...
    for timeout in (None, 0, 1234):
        ...
        assert result.effect_observation_v1 is None
...
def test_write_create_overwrite_empty_and_preserve_mode(tmp_path: Path) -> None:
    ...
    assert created.effect_observation_v1.status.value == "changed"
    target.chmod(0o640)
    ...
    assert overwritten.effect_observation_v1.status.value == "changed"
    ...
    assert unchanged.return_code == 0
    assert unchanged.effect_observation_v1.status.value == "unchanged"


def test_read_only_effect_is_not_applicable(
    tmp_path: Path,
) -> None:
    (tmp_path / "notes.txt").write_text("hello", encoding="utf-8")
    read = asyncio.run(
        LocalFileActor(tmp_path).execute(
            request(tmp_path, "read_file", {"target_file": "notes.txt"})
        )
    )
    assert read.effect_observation_v1.status.value == "not_applicable"


def test_v3_effect_observation_wire_is_required_nullable_and_closed(
    tmp_path: Path,
) -> None:
    original = request(
        tmp_path,
    ...
    live = _v10_absolute_terminal_request(
        original,
        actor_done_monotonic_ns=103_000_000_000,
    )
    result = asyncio.run(LocalFileActor(tmp_path).execute(live))
    encoded = json.loads(encode_tool_response(live, result))

    assert encoded["result"]["effect_observation_v1"] == {"status": "changed"}
    assert set(encoded["result"]["effect_observation_v1"]) == {"status"}
...
def test_search_replace_one_zero_multiple_and_replace_all(tmp_path: Path) -> None:
    ...
    assert multiple.effect_observation_v1 is None
