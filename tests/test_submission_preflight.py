...
def _git_access_finding() -> dict[str, object]:
    return {
        "schema_version": tb21.git_history_audit.FINDING_SCHEMA,
        "call_id": "history-call",
        "tool_name": "run_terminal_command",
        "access_kind": "git_history_command",
        "attempted": True,
        "dispatched": True,
        "bytes_returned": True,
        "history_required": False,
        ...
        "evidence_sources": [{"kind": "atif", "sha256": "8" * 64}],
    }


def _git_audit_with_finding(*, submission_blocking: object) -> dict[str, object]:
    finding = _git_access_finding()
    blocking = submission_blocking is True
    return {
        **_clean_git_audit(),
        "findings": [finding],
        "counts": {
...
@pytest.mark.parametrize(
    ("submission_blocking", "expected_code"),
    [
        (False, None),
        (True, "git_history_oracle_access"),
        (None, "git_history_evidence_unavailable"),
        (0, "git_history_evidence_unavailable"),
        ("false", "git_history_evidence_unavailable"),
    ],
)
def test_git_audit_honors_exact_lifecycle_reduced_aggregate(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
    submission_blocking: object,
    expected_code: str | None,
) -> None:
    audit = _git_audit_with_finding(submission_blocking=submission_blocking)
    monkeypatch.setattr(
        preflight.tb21.git_history_audit, "audit_trial", lambda *_a, **_k: audit
    )
    issues = preflight._Issues()

    projected = preflight._audit_git_history(
        tmp_path,
        instruction="generic fixture",
        capability=None,
        trusted_manifest_sha256="1" * 64,
    ...
    assert projected["findings"] == [_git_access_finding()]
    assert {item["code"] for item in issues.values()} == (
        set() if expected_code is None else {expected_code}
    )
...
def test_git_audit_rejects_incoherent_reduced_counts(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
    mutation: dict[str, object],
) -> None:
    audit = _git_audit_with_finding(submission_blocking=False)
    audit["counts"] = {**audit["counts"], **mutation}
    monkeypatch.setattr(
        preflight.tb21.git_history_audit, "audit_trial", lambda *_a, **_k: audit
    )
    issues = preflight._Issues()

    preflight._audit_git_history(
        tmp_path,
        instruction="generic fixture",
        capability=None,
        trusted_manifest_sha256="1" * 64,
...
def test_nonblocking_git_audit_rejects_malformed_finding_structure(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
    field: str | None,
    value: object,
) -> None:
    finding: object = _git_access_finding()
    if field is None:
        finding = None
    else:
    ...
    counts = {
        "findings": 1,
        "attempted": int(
            isinstance(finding, dict) and finding.get("attempted") is True
        ),
        "dispatched": int(
            isinstance(finding, dict) and finding.get("dispatched") is True
        ),
        "bytes_returned": int(
            isinstance(finding, dict) and finding.get("bytes_returned") is True
        ),
        "causal_reuse": int(
            isinstance(finding, dict) and finding.get("causal_reuse") is True
        ),
        "warnings": 1,
        "blocking": 0,
    }
...
def _synthetic_job(tmp_path: Path) -> tuple[Path, preflight.ReleaseExpectation]:
    ...
    profile_id = "nano-v1-grok-4-6-high-v1"
...
def test_clean_job_is_read_only_and_receipt_is_byte_stable(
    ...
    assert first["execution_status"] == "COMPLETE"
    assert first["verifier_status"] == "TERMINAL"
    assert first["submission_status"] == "SUBMISSION_READY"
...
def test_real_writer_collector_candidate_preflight_pipeline(
    ...
    assert rows[str(specs[2]["trial_id"])]["contamination_audit_state"] == "available"
...
def test_real_raw_admission_runtime_result_matrix(
    ...
    if classification == "errored":
        ...
        result["verifier"] = {
            "started_at": "2026-08-03T00:00:00.500000Z",
            "finished_at": "2026-08-03T00:00:00.750000Z",
        }
        result["exception_info"] = {
            "exception_type": "BridgeError",
...
def test_ordinary_terminal_error_remains_in_denominator_and_is_not_replaced(
    ...
    rows[0].update(
        {
            "verifier_result_kind": "timed_out",
            "reward": None,
...
def test_errored_audit_not_applicable_requires_durable_not_started(
    ...
    if expected_pass:
        ...
        assert receipt["status"] == "failed"
        if durable_not_started and audit_unavailable:
            assert _codes(receipt) == set()
            assert receipt["execution_status"] == "EXECUTION_INCOMPLETE"
            assert receipt["verifier_status"] == "INCOMPLETE"
        else:
            assert {
                "protected_evidence_unavailable",
                "git_history_evidence_unavailable",
