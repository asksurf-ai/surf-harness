...
def _review_codes(receipt: dict[str, object]) -> set[str]:
    return {
        str(flag["code"]) for job in receipt["jobs"] for flag in job["review_flags"]
    }
...
def test_errored_audit_not_applicable_requires_durable_not_started(
    ...
    if expected_pass:
        ...
        assert {
            "protected_evidence_unavailable",
            "git_history_evidence_unavailable",
        } <= _review_codes(receipt)
        assert receipt["review_required"] is False


def test_nonrewarded_missing_trajectory_discloses_unavailable_audits(
    tmp_path: Path,
    valid_artifacts: None,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
...
def test_protected_target_projection_only_blocks_actual_access(
    ...
    reviews = preflight._ReviewFlags()
    ...
    codes = {str(row["code"]) for row in reviews.values()}
...
def test_actual_git_history_bytes_require_review_without_blocking_preflight(
    tmp_path: Path,
    valid_artifacts: None,
) -> None:
    reward = 1.0
    ...
    assert receipt["status"] == "passed", _codes(receipt)
    assert receipt["submit_ready"] is True
    assert "git_history_oracle_access" not in _codes(receipt)
    assert "git_history_oracle_access" in _review_codes(receipt)
    assert receipt["review_required"] is True


def test_split_protected_target_attempt_requires_review_without_blocking(
    tmp_path: Path,
    valid_artifacts: None,
) -> None:
    job, expectation = _synthetic_job(tmp_path)
    ...
    assert receipt["status"] == "passed", _codes(receipt)
    assert receipt["submit_ready"] is True
    assert "protected_target_strong" not in _codes(receipt)
    assert "protected_target_strong" in _review_codes(receipt)
    assert receipt["review_required"] is True
