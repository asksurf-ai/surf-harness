...
def test_generated_remote_script_prunes_excluded_names_for_every_file_kind(
    tmp_path: Path,
) -> None:
    if shutil.which("timeout") is None:
        pytest.skip("GNU timeout unavailable")
    ...
    workspace.mkdir()
    stage.mkdir()
    (workspace / "dataset").write_bytes(b"excluded regular file\n")
    (workspace / "CACHES").mkdir()
    (workspace / "CACHES" / "nested.txt").write_bytes(b"excluded subtree\n")
    policy = workspace_snapshot.SnapshotPolicy(
        max_files=10,
        max_total_bytes=100,
        max_file_bytes=100,
    ...
    assert inventory.safe_contents == {}
    assert inventory.content_omissions == {}
    assert inventory.manifest["scan_complete"] is True
