...
async def _run_setup_cohort(*, prepared: Any, bound: Any) -> list[dict[str, object]]:
...
async def run(args: argparse.Namespace) -> dict[str, object]:
    ...
    if len(inventory) != TB21_TASK_COUNT or any(
        _DIGEST_IMAGE.fullmatch(task.docker_image) is None for task in inventory
    ):
    ...
    from nano_grok_build.harbor.prelaunch import admit_prelaunch
