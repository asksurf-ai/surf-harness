...
The Harbor host explicitly selects `fresh-checkpoint-v4`. Its completion review
is the unchanged Fresh Evidence-Debt V3 path. Separately, after at least 12
settled provider turns, 250,000 observed input tokens, and 32 history items, it
may replace the conversation once with the original system and user messages
plus a bounded continuity notice. The gate also requires no unresolved
background task, at least 12 provider turns of capacity, and more than 720
seconds before the last-send cutoff. It never reads task identity, reward,
verifier output, or previous benchmark results.

The workspace and runtime state are not reset. The notice treats recent tool
output as a locator rather than proof and asks the model to re-inspect the
minimum current state it needs. A `context.checkpointed` event records the
policy version, old and new history hashes, exact item counts, provider/tool
counters, and observed input tokens. The next provider request must bind the
new three-item history exactly; duplicate, mistimed, or counter-mismatched
events invalidate the runtime prefix.
