...
The Harbor host explicitly selects `semantic-checkpoint-v8`. Its isolated
completion critic prioritizes representative safety margin, validation-selection
risk, and changed-input counterexamples while retaining the same bounded review
shape. Separately, after at least
12 settled provider turns, 250,000 observed input tokens, and 32 history items,
the next ordinary action request adds one runtime-owned checkpoint function.
Its arguments are a closed, field-bounded semantic capsule, while ordinary
workspace functions remain available in the same response. The gate also
requires no unresolved background task, 13 provider turns of capacity, and
more than 990 seconds before the last-send cutoff. It never reads task identity,
reward, verifier output, or previous benchmark results.
...
`run.started` records positive policy/schema provenance. The inline request
binds the uncompacted source hash. Its control function is never dispatched or
counted as an external tool; ordinary same-response calls settle first.
`context.checkpointed` then binds request, capsule, settled-evidence, reset,
counter, and lease facts. Rejections carry bounded content hashes and ATIF
publishes bounded audit receipts. Artifactizer and collector validators require
the next request to bind the new four-item history; duplicate, mistimed,
unbound, or counter-inconsistent chains invalidate the runtime prefix.

When the complete 600-second isolated critic no longer fits, V7 may admit one
same-context late acceptance review. Admission requires more than 210 seconds
of actor time, 300 seconds before last-send, two provider turns, and remaining
history/tool capacity. The review binds a bounded excerpt of the provisional
completion, may emit one bounded tool batch, and then forces a tools-disabled
FinalOnly request. The typed
`completion_late_review` phase makes this path independently auditable. An
unchanged provisional final remains a safe transport-failure fallback; a
potentially mutated workspace cannot reuse that old final as current truth.
