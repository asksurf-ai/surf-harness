...
- Keep the V7 checkpoint, reserve-aware late review, deadlines, tools, and
  artifact contract unchanged.
- Version the complete isolated critic as V4 and prioritize claims with narrow
  threshold margin, validation-selection risk, or only happy-path evidence.
- Require its single falsification proposal to expose representative independent
  margin or a bounded changed-input counterexample without seeking hidden data.
- Record `semantic-checkpoint-v8` through runtime, CLI, artifactizer, and
  collector provenance while retaining V7 replay semantics.
- Treat the 0.5.3 raw-positive paired result as NO-GO because its legacy control
  had only 11/12 direct ATIF; no top-up or post-hoc gate relaxation is allowed.
