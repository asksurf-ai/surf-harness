...
- Move the exact provider and release identity from `xai/grok-4.5` to
  `xai/grok-4.6`, retaining reasoning effort `high`, service tier `default`,
  one attempt, and retry zero.
- Keep the V0.6.3 prompt, tools, limits, runtime loop, lifecycle, isolation,
  rehydration, and scoring behavior unchanged.
- Bind fresh owner approval to the exact V0.6.4 profile and mark the
  deterministic public-export manifest ready; full release gates and
  publication remain separate later steps.

## 0.6.3

- Publish canonical terminal-actor failure evidence before the remote cleanup
  branch exits, preserving the original tool failure separately from final
  containment facts.
- Bind the final zero-survivor census and background state to one durable,
  reread process lease before structural verifier admission.
- Reconcile a later same-owner safe census without accepting missing,
  contradictory, mismatched, or survivor-bearing evidence.
- Keep V0.5.7 model/profile/contract/provider-request/scoring bytes unchanged;
  add no provider request, task/reward route, retry, canary, or top-up path.

## 0.6.1

- Replace compound cleanup-error string inference with hash-bound structural
  process, bridge, actor, stdio, and runtime-child closure evidence.
- Wait through the signed tool-settlement boundary before classifying cleanup,
  and preserve unsafe evidence before any cleanup of private control material.
- Deduplicate byte-equal result/stdout and logs/stdout terminal envelopes
  while rejecting contradictions and unequal duplicates.
- Keep V0.5.7 model/profile/contract/provider-request/scoring bytes unchanged;
  add no provider request, task/reward route, retry, canary, or top-up path.

## 0.6.0

- Add a hash-chained per-trial lifecycle coordinator with explicit execution,
  verifier-safety, verifier-terminal, submission, unsafe, and cancellation
  phases.
- Preserve the original Harbor verifier exactly once for ordinary runtime,
  publication, cleanup, and cost-evidence errors; only typed workspace,
  process, bridge, and identity hazards may suppress it.
- Replace provider `in_flight` inference with a terminal provider ledger and
  bind persisted verifier-safety evidence to setup, runtime, workspace, Git,
  background-process, bridge, and provider-terminal identities.
- Keep V0.5.7 model/profile/contract/provider-request/scoring bytes unchanged;
  add no provider request, task/reward route, retry, canary, or top-up path.

## 0.5.9

- Prepare and mint the immutable Git-history lifecycle receipt before Harbor's
  marker-last artifact publication and private-control cleanup.
- Require durable lifecycle proof and successful preparation before a started
  trial may publish its terminal marker; preparation failure leaves marker and
  verifier absent while preserving validated exact cost projection.
- Bind live managed-service liveness into restore/defer decisions and keep
  repeated verifier preparation idempotent after publication cleanup.
- Add no provider request, prompt byte, task/reward route, retry, canary trial,
  or scoring-policy change.

## 0.5.8

- Project validated provider-native integer cost ticks into Harbor only when
  call coverage is complete; partial subtotals remain explicit lower-bound
  metadata and are never published as exact scalar cost.
- Recover one malformed snapshot terminal JSON frame only through the existing
  same-lease, identity-bound termination and zero-census proof, without
  re-executing the snapshot command or widening any semantic failure case.
- Advance to `nano-git-history-capability-policy-v4`: typed rejected ATIF results
  are zero-byte non-dispatch, Git history is classified from actual parsed argv,
  and the canonical supplied-bundle merge instruction is hash-bound `required`.
- Add no provider request, prompt byte, task-ID/reward route, checkpoint, critic,
  or late review.

## 0.5.7

- Keep pre-existing Git history in a host-only escrow while the agent runs, then
  restore the exact original Git admin database immediately before a shared
  verifier only after workspace evidence has been sealed.
- Skip restoration when an agent-owned background process survives or the
  isolated repository's semantic Git state changed; never expose escrow bytes
  through model tools, prompts, trajectories, or workspace artifacts.
- Return the scoring loop to the 0.4.6 path by removing the rejected 0.5.6
  4,096-byte exact-request replay. No provider call, task-name route, reward
  input, verifier identifier, or hidden commit special case is added.

## 0.5.6

- Advance the admission compiler to `nano-git-history-capability-policy-v3` so
  explicit branch-switch plus lost-work recovery requests receive the required
  Git-history capability instead of encouraging wrapper bypasses.
- Replay the bounded exact original request (up to 4,096 bytes) in the existing
  same-context evidence-debt review so late acceptance criteria are not replaced
  by a 128-byte prefix.
- Classify explicit branch-switch plus lost-work language as requiring Git history,
  and reject direct Git executable alias creation when history access is denied.
- Add no provider call, checkpoint, fresh critic, task identifier, reward input,
  verifier input, or task-specific runtime route.

## 0.5.5

- Reset the scoring path to the 0.4.6 runtime lineage and add one bounded,
  task-neutral actor-native acceptance reminder to the initial contract prompt.
- Prefix ordinary tool results with typed command, timeout, truncation, and
  observed-effect evidence without adding a provider request or history item.
- Keep terminal transport settlement independent from the real command verdict;
  a settled nonzero exit remains transport-success with explicit failure evidence.
- Report bash effects as unobserved, read-only effects as not applicable, and
  direct file changed/unchanged status only from a bounded byte comparison.
- Block public export and scored execution until the changed exact contract bytes
  receive fresh owner approval and an immutable public-source binding.
...
History capability records retain schema `nano-git-history-capability-v1`. The
compiler policy advanced from V1 to V2 in 0.4.0 and to V3 in 0.5.6. Historical
records remain immutable; new runs fail closed on a mixed policy.
