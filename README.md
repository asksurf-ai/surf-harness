...
Version `0.4.5` retains the exact, owner-approved task-neutral
workspace-integrity contract, immutable task-image binding, Git admission, and
provider-zero setup fail-fast semantics. External-bridge failures now preserve
their typed result, subprocess return code, phase, task identity, and cleanup
evidence, and remote cleanup requires a post-exit census. No leaderboard score is
claimed until five complete, public, audited jobs pass the upstream submission
checks.
