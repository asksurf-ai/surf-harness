...
The current `0.5.4` source is an evaluation candidate for a fresh bounded
provider-on paired gate. It is not yet a K=5 candidate. The 0.5.3 paired gate
was raw-positive but failed its pre-registered control-artifact completeness
gate. Results become submission candidates
only after the provider-on gate and trajectory audit described in
...
| Evidence-preserving semantic checkpoint | A strict runtime-owned function shares the next normal action response with ordinary workspace calls; typed events bind source, capsule, settled evidence, reset, and budget lease | Long trajectories retain direction without a second latency-sensitive provider call or workspace authority in the control function. |
| Reserve-aware late review | When the full isolated critic cannot fit, one same-context acceptance review may use one bounded tool batch before a tools-disabled final | Useful signed runtime is not discarded, while the provisional fallback and terminal reserve remain explicit. |
