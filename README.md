...
The current `0.5.1` source is an evaluation candidate authorized for one fresh,
complete K=5 cohort after its deterministic gates passed. No uplift or score is
claimed before that cohort completes and is audited. The last evaluated tag is
[`v0.5.0`](https://github.com/asksurf-ai/surf-harness/tree/v0.5.0), whose paired
audit did not establish uplift. Results become submission candidates only after
the public-job and trajectory audit described in [Submitting](docs/SUBMITTING.md).
...
| Evidence-preserving semantic checkpoint | One tools-disabled prepare request creates a closed, byte-bounded continuity capsule; typed events bind source, prepare, capsule, reset, and budget-lease hashes | Long trajectories retain implementation direction without treating actor-authored memory as proof or consuming the final review reserve. |
