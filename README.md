...
Version `0.5.8` reuses the exact owner-approved `0.5.5` contract artifacts and
adds exact provider-cost publication, one same-lease snapshot framing recovery,
and typed Git-history audit evidence. It remains a scored candidate until the
independent release gate passes. The last evaluated public source remains the immutable
