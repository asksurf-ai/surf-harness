...
**An audit-first Grok 4.6 terminal-agent harness for Harbor.**

Surf Harness turns `xai/grok-4.6` into a bounded coding agent for isolated
...
Version `0.6.4` changes only the release and model identity from V0.6.3 and
keeps the V0.5.7 scoring path unchanged. Its lifecycle coordinator records
process, bridge, actor, stdio, and child closure as independent structural axes,
strictly deduplicates equal terminal transport copies, and binds the final
terminal-actor census to a durable process lease. Ordinary runtime and
publication errors preserve native verifier truth; only structurally unproven
safety suppresses verification. It remains a scored candidate until the independent
release gate passes. Fresh owner approval now binds the exact Grok 4.6 profile,
and the deterministic V0.6.4 public-export manifest is marked ready. The last
evaluated public source remains the immutable
[`v0.4.6`](https://github.com/asksurf-ai/surf-harness/tree/v0.4.6) tag. This
repository does not claim a Terminal-Bench score; results become submission
candidates only after the full public-job and trajectory audit described in
[Submitting](docs/SUBMITTING.md).
