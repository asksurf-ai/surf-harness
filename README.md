...
Version `0.4.4` retains the exact, owner-approved task-neutral
workspace-integrity contract and immutable task-image binding. A clean workspace
with no repository is bound as `absent_clean` without assuming task-image Git;
root, nested, required, and ambiguous Git topologies remain fail-closed. A local
provider-zero fail-fast stops systematic setup failures after two distinct trials.
No leaderboard score is claimed until five complete, public, audited jobs pass
the upstream submission checks.
