...
# This is an evaluation cohort, never a runtime task branch.  The selection is
# frozen from the V9 error/integrity audit and is not exposed to the agent.
CANARY_TASK_IDS = (
...
def _policy_for_stage(stage: str, checksums: Mapping[str, str]) -> StagePolicy:
    if stage == "paired-canary":
        if any(task_id not in checksums for task_id in CANARY_TASK_IDS):
            raise CandidateError("canary_task_identity_invalid")
        return StagePolicy(
            name=stage,
            task_ids=CANARY_TASK_IDS,
            expected_jobs=3,
            min_trusted_raw=None,
            advances_to="full-one",
        )
    all_tasks = tuple(checksums)
    if stage == "full-one":
        return StagePolicy(
            name=stage,
            task_ids=all_tasks,
    ...
    raise CandidateError("candidate_stage_invalid")


def canary_plan() -> dict[str, object]:
    """Return the frozen evidence order; perform no execution or network I/O."""
    ...
    return {
        ...
        "stages": [
            {
                ...
                "provider_calls": 0,
                "order": [
                    "actor-settlement-background-integrity",
                    "final-admission-completion-review",
                    "bn-fit-modify-verifier-setup-reproduction",
                ],
                "required_outcomes": [
                    "zero-containment-regression",
                    "zero-integrity-contamination",
                    "rewarded-atif-eligible",
                ],
            },
            {
                "id": "targeted-c2",
        ...
        "boundary": (
            "post-run evidence audit is not execution-time filesystem or "
            "egress isolation"
        ),
    }
...
def _validate_row(
    ...
    if (
        row.get("schema_version") != tb21.ROW_SCHEMA
        or row.get("task") != spec["task"]["id"]  # type: ignore[index]
        ...
        or (row.get("strict_pass") and not row.get("collector_pass"))
        or (row.get("strict_pass") and not row.get("reliable"))
...
def _validate_row_evidence(
    ...
    if (
        ...
        or row.get("strict_pass") is not expected_strict
    ...
    contamination = tb21._contamination_audit(
        trial_dir,
...
def _load_job(
    ...
    for spec in specs:
        ...
        if (
            ...
            raise CandidateError("candidate_trial_result_invalid")
        _validate_row_evidence(trial_dir, spec, row)
        ...
        if reward is not None and reward > 0 and not stop_caused_cancellation:
            row["_atif_eligible"] = _eligible_rewarded_atif(
                trial_dir, spec, row, evidence
            )
        else:
...
def _metric(numerator: int, denominator: int, available: bool) -> dict[str, object]:
    return {
        "numerator": numerator,
...
def audit_jobs(
    ...
    *,
    ...
    raw = sum(effective_pass(row) for row in mechanism_rows)
    trusted_raw = sum(
    ...
    return {
        ...
        "release_advance_authorized": False,
        "provider_free": True,
        ...
        "external_prerequisites": {
            "evaluated_by_this_command": False,
            "required": [
                "native-linux-fault-gates",
                "paired-control-regression-review",
            ],
        },
...
def parse_args(arguments: list[str] | None = None) -> argparse.Namespace:
    ...
    audit.add_argument(
        "--stage",
        choices=("paired-canary", "full-one", "leaderboard"),
        required=True,
    )
...
def main(arguments: list[str] | None = None) -> int:
    ...
    try:
        if args.command == "plan":
            ...
            checksums = load_official_checksums(
                ROOT / tb21.OFFICIAL_TASK_CHECKSUMS_PATH
            )
            policy = _policy_for_stage(args.stage, checksums)
            report = audit_jobs(
                tuple(args.job_dir),
                policy=policy,
