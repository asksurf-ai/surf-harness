...
def run_cold_build(root: Path, uv: Path) -> dict[str, int | str]:
    ...
    with tempfile.TemporaryDirectory(prefix="nano-cold-build.") as tmp:
        ...
        if source_status:
            raise RuntimeError("cold build source has uncommitted tracked changes")
        source_head = _run(["git", "rev-parse", "HEAD"], source_root).stdout.strip()
        source_tree = _run(
            ["git", "rev-parse", "HEAD^{tree}"], source_root
        ).stdout.strip()
        _run(
            [
                "git",
        ...
        _run(["git", "checkout", "--detach", "--quiet", source_head], checkout)
        cloned_head = _run(["git", "rev-parse", "HEAD"], checkout).stdout.strip()
        cloned_tree = _run(["git", "rev-parse", "HEAD^{tree}"], checkout).stdout.strip()
        if (cloned_head, cloned_tree) != (source_head, source_tree):
            raise RuntimeError("cold checkout Git identity mismatch")
